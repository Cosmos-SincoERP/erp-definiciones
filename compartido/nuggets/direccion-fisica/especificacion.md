...
**Producidos por el custodio (Datos de Referencia)** a partir de las fuentes heredadas del servicio de Direcciones v1.0 (rescatadas del historial del repositorio, commit del replanteamiento de jun-2026). Al ser insumos que solo consume este Nugget, viven embebidos en `compartido/nuggets/direccion-fisica/datos/` (mismo criterio que `roles-contacto` en el Nugget `Contacto`) — no se duplican en `datos-referencia/catalogos/`, que reserva ese directorio para los catálogos base transversales (países, monedas, divisiones territoriales, tipos de documento):
