...
sequenceDiagram
    autonumber
    participant OXP as OXP (consumidor)
    participant EO as EO (dueño)
    participant Adm as Administrador EO

    Note over OXP: Llega un documento imputado a una unidad<br/>que no existe en la copia local de OXP
    OXP->>OXP: Imputa contra su copia local → la unidad no existe

    par Corrección (integridad del hecho)
        OXP->>OXP: DIFIERE solo la parte que requiere la unidad;<br/>registra el resto y NO se detiene
    and Visibilidad (no bloqueante)
        OXP-->>EO: Señal de demanda<br/>(informativa, fire-and-forget, dedup [SI07])
        EO->>EO: Proyecta en la bandeja de sugerencias [SI11]
        EO-->>Adm: Muestra la sugerencia
    end

    Adm->>EO: Decide crear — acto deliberado<br/>(F1: CrearUnidad → ActivarUnidad)
    EO-->>OXP: UnidadActivada<br/>(evento de ciclo de vida, a todos los consumidores)
    OXP->>OXP: Actualiza copia local → el DIFERIDO se resuelve solo<br/>(consistencia eventual, sin consultar a EO en caliente)

    Note over OXP,EO: Causación completa contra la unidad real<br/>(coincide exacto con Contabilidad)
```

**Lo que el diagrama hace explícito** (el bloque `par` muestra los dos caminos que arrancan juntos y son independientes):
