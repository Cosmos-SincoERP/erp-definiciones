...
| `anexo-orquestacion-creacion.md` | *(superado por `[D15]`)* | Describía el patrón de creación desde consumidores vía BFF + estado `Borrador`. Ese patrón fue reemplazado por el replanteamiento #46 (copia local + diferir + señal informativa, `[D15]`). El destino del anexo se resuelve en el cierre del #46. |
...
| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una unidad que nunca operó se descarta (F8). Aplica al rechazo manual del administrador sobre un borrador suyo, al abandono por inactividad (proceso automático `[SI05]`) o a la cascada cuando el grupo padre se inactiva con borradores colgando (F10). Cierra el `[PD3]` del anexo de orquestación. |
