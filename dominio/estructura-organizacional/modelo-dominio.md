...
| `codigo` | VO `Codigo` | Identificador plano de texto libre, único por tenant entre grupos y unidades. Inmutable. | longitud según el rango configurado por el tenant (`[R10]`, por defecto 2-12) |
| `nombre` | VO `Nombre` | Denominación descriptiva del grupo. | texto |
| `padreId` | Referencia | Identificador del grupo padre. Null cuando el grupo es un **grupo de primer nivel** (sin padre) — un tenant puede tener varios, cada uno con su propio árbol independiente (`[D16]`). Ver el comportamiento calculado `esDePrimerNivel()`. | uuid o null |
...
| `codigo` | VO `Codigo` | Identificador plano de texto libre, único por tenant, inmutable. | longitud según el rango configurado por el tenant (`[R10]`) |
...
- Grupos de primer nivel (`esDePrimerNivel()`): 1..N por tenant — cada uno encabeza su **propio árbol independiente** y la frontera de consolidación es el tenant (`[D16]`).
