...
| `padreId` | Referencia | Identificador del grupo padre. Null cuando el grupo es un **tope** (sin padre) — un tenant puede tener varios topes; la estructura es un bosque (`[D16]`). "Ser tope" es una propiedad derivada de `padreId == null`, no una marca almacenada. | uuid o null |
| `nivel` | Entero (calculado, no almacenado) | Profundidad del grupo en la jerarquía vigente. Se proyecta desde `[SI02]` (proyección de jerarquía vigente). No se appendea en eventos ni se persiste en el agregado. | 0 para grupos tope |
...
| `puedeInactivarse()` | `estado == Activo` | Validación previa a F10. Si retorna false, el comando `InactivarGrupo` se rechaza. (La protección especial del raíz se retiró con `[D16]`: inactivar un grupo tope es un F10 normal — cascada con confirmación del impacto, `[R21]`.) |
...
- Grupos tope (`padreId == null`): 1..N por tenant — la estructura es un **bosque**; cada tope es la raíz de su propio árbol y la frontera de consolidación es el tenant (`[D16]`).
...
| **Precondiciones** | Código único en el tenant (`[I09]`); si se indica `padreId`, el grupo padre existe y está en `Activo` (`[R07]`) y la posición no introduce ciclos (`[I11]`); sin `padreId`, el grupo nace como **tope** del bosque (`[D16]`); formato del código (`[R10]`). |
| **Información capturada** | `grupoId`, `codigo`, `nombre`, `padreId` (null cuando el grupo se crea como tope, sin padre), `usuarioId`, `timestamp`. |
