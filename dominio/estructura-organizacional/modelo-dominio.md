...
| `FechaEfectiva` | Eventos de reestructuración y de transición de jerarquía | Momento a partir del cual una versión de la jerarquía o una reestructuración rige. No puede ser anterior a la última versión vigente de jerarquía de las unidades involucradas (`[I08]`, validable localmente). Su coherencia con la actividad transaccional es responsabilidad del administrador, no un bloqueo (`[R25]` relajada, issue #56). |
...
> **Nota — `[SI10]` retirada (issue #56).** Existió una "proyección local de última imputación por unidad" que validaba que la `fechaEfectiva` de una reestructuración no fuera anterior a la última transacción imputada a la unidad (`[I08]`/`[R25]`). Se retiró al relajar esa validación: con transacciones inmutables y reportes de vista actual/histórica, la comparabilidad ya está cubierta sin restringir la fecha efectiva, y la coherencia de la fecha pasa a ser responsabilidad del administrador. Se conserva la numeración de `[SI11]`/`[SI12]` para no romper referencias. El detalle vive en el historial de git.
...
| **Precondiciones** | Todas las unidades origen en `Activa` o `Suspendida` (`[R23]`, `[I14]`); destino en `Activa` y distinto del conjunto origen (`[R22]`, `[R24]`); fecha efectiva no anterior a la última versión vigente de jerarquía (`[I08]`, validable localmente). La coherencia con la actividad transaccional es responsabilidad del administrador, no un bloqueo (`[R25]` relajada, issue #56). |
...
| **Precondiciones** | Unidad origen en `Activa` o `Suspendida` (`[R23]`); al menos dos destinos, todos en `Activa`, distintos del origen (`[R22]`, `[R24]`); fecha efectiva no anterior a la última versión vigente de jerarquía (`[I08]`, validable localmente). La coherencia con la actividad transaccional es responsabilidad del administrador, no un bloqueo (`[R25]` relajada, issue #56). |
...
| `I08` | **Fecha efectiva coherente con la jerarquía vigente.** En F12, F13 y F14, la `fechaEfectiva` no puede ser anterior a la última versión vigente de jerarquía de las unidades involucradas — validable **localmente** por Estructura Organizacional (es dueña de la jerarquía). La coherencia con la **actividad transaccional** (no reestructurar sobre periodos ya con movimientos) **dejó de ser un bloqueo** (issue #56): es responsabilidad del administrador que ejecuta la reestructuración, porque las transacciones/asientos son inmutables y los reportes ofrecen vista actual/histórica — EO no importa la última imputación ni la valida. | Local | `UnidadOrganizacional` | `[R25]` |
...
| **1.6** | **2026-06-19** | **Consistencia del modelo de comunicación — relajación de la validación de fecha efectiva (issue #56).** Se **retira `[SI10]`** (proyección local de última imputación) y con ella el acoplamiento que obligaba a EO a importar las imputaciones de todos los consumidores. **`[I08]` reformulada**: de *fecha efectiva no anterior al historial* (Eventual/Cross-domain, validada contra imputación externa) a **coherente con la jerarquía vigente** (Local — validable por EO, dueña de la jerarquía); la coherencia con la actividad transaccional pasa a ser **responsabilidad del administrador**, no un bloqueo (transacciones inmutables + reportes vista actual/histórica). Limpiadas las referencias a `[SI10]`/imputación (tabla comando↔SI, `[SI07]`, `[D15]`, sub-sección 3.8, VO `FechaEfectiva`, precondiciones F12/F13). Se conserva la numeración de `[SI11]`/`[SI12]` (hueco en SI10). Conteos: **11 sugerencias de implementación** (−1: `[SI10]` retirada); 16 invariantes (`[I08]` reformulada, no eliminada); 15 decisiones; 22 permisos — sin cambios. Acompaña al alcance v1.4 y a OXP v4.1 (retira el aviso de imputación) / Contabilidad v1.10. |
