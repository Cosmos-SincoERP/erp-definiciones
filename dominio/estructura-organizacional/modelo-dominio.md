...
|-----------|-----|-------------|
| `definicion-alcance.md` v1.0 | QUÉ hace el sistema | Fuente de verdad para glosario (23 términos), actores, flujos (15) y reglas (`[R##]`, 30 reglas). No se duplica aquí. |
...
| `anexo-orquestacion-creacion.md` v1.0 | Patrón de creación desde consumidores | Capa BFF + estado `Borrador` como vehículo de la consistencia eventual. Tres pendientes originales (PD1, PD2, PD3) que se cierran o se mantienen en este documento. |
...
| `origenSolicitud` | Datos opcionales **almacenados** | Cuando la unidad nace por solicitud desde un consumidor (F2): `{ subDominioOrigen, referenciaOperacionOrigen, correlationId }`. **Se almacena como atributo del agregado** tras `UnidadCreada` y permanece visible durante todo el ciclo de vida — permite que `UnidadDescartada` lo lleve en su payload sin re-leer el evento de creación, habilitando la cancelación en cascada de operaciones dependientes (`[R30]`). | null en F1 directa |
...
**Atributos de baja son write-once:** `motivoBaja`, `causalidadBaja`, `fechaEfectiva` y `origenSolicitud` se asignan una sola vez (al evento que origina la baja o, en el caso de `origenSolicitud`, al evento de creación) y no se modifican posteriormente. Esto se garantiza con el método interno `validarCoherenciaBaja()` (ver comportamientos calculados).
...
| `OrigenSolicitud` | `UnidadOrganizacional` (composición opcional) | Estructura `{ subDominioOrigen, referenciaOperacionOrigen }` que aparece solo cuando una unidad nace vía F2 (solicitud desde consumidor). Permite la cancelación en cascada de operaciones dependientes al descartar (`[R30]`). |
...
| `CrearUnidad` (F1 directa) | `[SI01]`, `[SI03]`, `[SI04]`, `[SI06]` |
| `SolicitarCreacionDeUnidad` (F2 desde consumidor) | `[SI01]`, `[SI03]`, `[SI04]`, `[SI06]`, `[SI07]` |
...
| Endpoint `verificarDisponibilidadCodigo` (BFF) | `[SI01]` (consulta del índice) |
...
1. El comando (`CrearGrupo`, `CrearUnidad`, `SolicitarCreacionDeUnidad`) consulta el índice antes de aceptar la creación.
...
Proyección que lista todas las unidades en `Borrador` con su `fechaUltimaActividadBorrador`, origen de solicitud (si aplica) y antigüedad. Es la fuente para la UI del administrador (F3, F8) y para el proceso programado de descarte automático (`[SI05]`).
...
Materializa `[D09]`. Cierra `[PD2]` del anexo de orquestación.
...
4. Emitir `UnidadDescartada` con `usuarioId: "sistema:descarte-automatico"`, `motivoBaja: "abandono_por_inactividad"`, `correlationId: jobExecutionId`. El agregado valida `puedeDescartarseAutomáticamente(umbral)` y rechaza si la condición ya no se cumple (race condition aceptable: el evento simplemente no se emite).
5. Registrar `JobDescarteUnidadProcesada` con `resultadoEmision: Emitido`.
6. Si la unidad tiene `origenSolicitud`, notificar al consumidor origen (ver tabla de compensación del servicio `DescarteAutomaticoBorradores` en Sección 3.6).
7. Al terminar el recorrido: marcar `JobDescarteEjecucion` como `Completed` (o `Failed` si hubo errores no recuperables).
...
- **No aplica** en comandos de creación (`CrearGrupo`, `CrearUnidad`, `SolicitarCreacionDeUnidad`): no existe versión previa.
...
#### `[SI07]` Idempotencia por `correlationId` en comandos desde consumidores

Materializa `[R29]`.

Todo comando recibido desde un sub-dominio consumidor (vía BFF) lleva un `correlationId` único que el sub-dominio persiste. Si el mismo `correlationId` llega dos veces (retry de red, replay del consumidor), el segundo se ignora y se retorna el resultado del primero.

**Comandos cubiertos por este patrón:**

- `SolicitarCreacionDeUnidad` (F2, desde OXP, Contabilidad, futuros consumidores).
- Cualquier comando futuro originado por un consumidor a través de la capa BFF.

**Mecanismo de persistencia:**

- Tabla `ComandoRecibido(comandoId, correlationId, tenantId, resultadoEmitido, fechaRecepcion)` con TTL de 7 días (configurable). Tras el TTL, los registros se purgan — un retry posterior se trataría como comando nuevo.
- La tabla es por tenant para evitar colisiones entre clientes distintos.

**Comandos NO cubiertos por este patrón:**

- Comandos originados por el administrador desde la UI propia del módulo (F1, F3, F4, F5, F6, F7, F8, F9, F10, F11, F14, F15): la idempotencia se gestiona en la UI (deshabilitar botón de submit tras click, validación de formulario, confirmación explícita en operaciones críticas).
...
**Endpoint complementario para BFF — `verificarDisponibilidadCodigo(codigo)`:**

Para optimizar la experiencia de usuario en F2, el servicio expone un endpoint de solo lectura `verificarDisponibilidadCodigo(codigo) → { disponible: boolean, sugerencia?: codigo }`. La capa BFF lo consume desde la UI antes del submit del comando `SolicitarCreacionDeUnidad`. Características:

- **Best-effort:** la verificación NO bloquea la unicidad. El servicio sigue validando al recibir el comando vía `[SI01]`.
- **Sin garantía transaccional:** si dos clientes verifican simultáneamente y obtienen "disponible", solo uno logra crear; el otro recibe `codigo-no-disponible` al hacer submit. El BFF maneja el error con la misma lógica de sugerencia.
- **El servicio sigue siendo la autoridad** de unicidad — el BFF no mantiene proyección local del dominio.
- **Detalle del flujo:** ver `anexo-orquestacion-creacion.md` v1.1, sección "Verificación de disponibilidad de código en BFF".
...
#### `[SI10]` Proyección de última imputación por unidad

Materializa `[I08]` y `[R25]`.

Mantener una fuente consultable de la última imputación registrada por unidad organizacional para validar la `fechaEfectiva` en procesos de reestructuración y traslado (`FusionarUnidades` F12, `DividirUnidad` F13, `TrasladarUnidad` F14).

Esta fuente puede implementarse como:

- Una proyección transversal alimentada por eventos de los consumidores transaccionales (OXP, Contabilidad).
- Una consulta federada a los consumidores dueños de las transacciones (cada consumidor expone un endpoint de lectura).
- Una proyección materializada por el motor contable si este consolida las imputaciones de todos los consumidores en un único lugar.

**Contrato mínimo esperado:**

```
obtenerUltimaImputacion(unidadId, tenantId) → fechaUltimaImputacion | null
```

**Comportamiento:**

- El servicio `ReestructuracionUnidad` consulta este contrato antes de aceptar `FusionarUnidades`, `DividirUnidad` o `TrasladarUnidad`. Si la fecha efectiva propuesta es anterior a la última imputación de cualquier unidad involucrada, el comando se rechaza con error `fecha-efectiva-anterior-a-historial`.
- Si el contrato no está disponible (proyección stale, consumidor inaccesible), la política por defecto es rechazar el comando para preservar la integridad histórica. El administrador es notificado de la dependencia faltante.
- La implementación específica (proyección propia vs. federada vs. delegada al motor contable) se decide al momento de construir la plataforma de integración del ERP y se documenta en EventCatalog o en el contrato de integración correspondiente.

**Nota sobre la dependencia funcional:** este contrato establece una segunda relación de Estructura Organizacional con los sub-dominios consumidores transaccionales, adicional a la principal de emisión/recepción de eventos del ciclo de vida. Ver `definicion-alcance.md` Sección 7 — Dependencias externas.
...
4. Si la unidad tenía `origenSolicitud`, notificar al consumidor para que cancele la operación dependiente (ver tabla de compensación abajo).
5. Cerrar la ejecución con estado final.
...
| 4 | Notificación al consumidor origen | Reintento con backoff exponencial (3 intentos: 1 s / 5 s / 30 s). Si persiste, marca `resultadoEmision: NotificacionPendiente` y registra en log de notificaciones fallidas para revisión humana. **La unidad sí queda descartada** independientemente del éxito de la notificación — el consumidor puede sincronizar posteriormente consultando el estado de la unidad. |

**Idempotencia:** garantizada por `(unidadId, jobExecutionId)`. Si el job crashea y se reinicia con el mismo `jobExecutionId`, los registros ya marcados en `JobDescarteUnidadProcesada` se saltan.
...
| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una unidad organizacional fue registrada en el sistema. Puede nacer directamente desde la UI del administrador (F1) o como solicitud desde un sub-dominio consumidor (F2, vía BFF). |
| **Causalidad** | Directa (comando `CrearUnidad` o `SolicitarCreacionDeUnidad`). |
| **Agregado** | `UnidadOrganizacional` |
| **Estado previo** | — (creación) |
| **Estado resultante** | `Borrador` (default) o `Activa` (solo en F1 cuando el administrador elige activar directamente — en ese caso se emite `UnidadActivada` en el mismo append). |
...
| **Información capturada** | `unidadId`, `codigo`, `nombre`, `tipoUnidad`, `descripcion`, `grupoPadreId`, `estadoInicial` (`Borrador` o `Activa` — **es un parámetro del comando que se captura para auditoría; el estado actual de la unidad se reconstruye reproduciendo la secuencia de eventos, no se lee de este campo**), `origenSolicitud` (opcional, solo en F2: `{ subDominioOrigen, referenciaOperacionOrigen, correlationId }`), `usuarioId`, `timestamp`. |
| **Efectos** | Se crea el agregado en el estado inicial elegido. Se inicializa `fechaUltimaActividadBorrador` si nace en `Borrador`. Si nace en `Activa`, se emite también `UnidadActivada`. Los consumidores con interés reciben la notificación. |

> **Nota sobre verificación previa en F2 (solicitud desde consumidor):** la capa BFF consume un endpoint del servicio del tipo `verificarDisponibilidadCodigo(codigo)` antes de enviar `SolicitarCreacionDeUnidad`. Si la verificación falla (código ya en uso), el sistema inteligente sugiere alternativa al usuario sin enviar el comando. La verificación es **best-effort** — el servicio sigue validando al recibir el comando y puede rechazar con `codigo-no-disponible` si hay race concurrente entre la verificación y el submit. El BFF maneja ese caso con la misma lógica de sugerencia. Ver `anexo-orquestacion-creacion.md` (Flujo F2) y `[SI07]` para detalle.

#### `UnidadActivada`

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una unidad pasa a estado `Activa`. Aplica al activar una unidad en `Borrador` (F3) o como evento colateral cuando una unidad nace directamente en `Activa` en F1 con `estadoInicial: Activa`. F5 (reactivación desde `Suspendida`) emite `UnidadReactivada`; F6 (reapertura desde `Inactiva`) emite `UnidadReabierta`. |
...
| **Efectos** | La unidad acepta imputaciones (`[R13]`). Los consumidores que tenían la unidad referenciada como `Borrador` destraban las operaciones pendientes. Si vino de F2, el consumidor origen recibe la confirmación. El payload self-contained permite a los consumidores actualizar referencias locales sin queries adicionales. |
...
| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una unidad que nunca operó se descarta (F8). Aplica al rechazo manual del administrador, al abandono por inactividad (proceso automático `[SI05]`) o a la cascada cuando el grupo padre se inactiva con borradores colgando (F10). Cierra el `[PD3]` del anexo de orquestación. |
...
| **Información capturada** | `unidadId`, `codigo`, `tipoUnidad`, `grupoPadreId`, `motivoBaja` (`operativa` cuando es rechazo manual del admin; `abandono_por_inactividad` cuando es automático o por cascada F10 sobre Borrador), `origenSolicitud` (objeto opcional: `{ subDominioOrigen, referenciaOperacionOrigen, correlationId }` — presente cuando la unidad nació vía F2), `esCascada` (boolean; `true` cuando es derivado de F10 o `[SI05]`), `correlationId` (presente cuando `esCascada == true`: `jobExecutionId` en `[SI05]`; `correlationId` de la saga en F10), `motivo` (texto libre opcional), `usuarioId` (`sistema:descarte-automatico` cuando proviene de `[SI05]`; identificador del administrador en F8), `timestamp`. |
| **Efectos** | El `codigo` de la unidad queda libre para una nueva solicitud (`[R11]`). Si la unidad tenía `origenSolicitud`, el sub-dominio publica el evento en un canal destinado al consumidor origen (ver Sección 3.6, `DescarteAutomaticoBorradores`) para que cancele en cascada las operaciones dependientes (`[R30]`). La unidad se filtra de reportes históricos. |
...
| `I07` | **Datos mínimos para activación.** Una unidad solo transiciona a `Activa` si `codigo`, `nombre`, `tipoUnidad` y `grupoPadreId` están definidos y el tipo está vigente en el catálogo del padre o ancestros. | Local | `UnidadOrganizacional` | `[R29]` |
| `I08` | **Fecha efectiva no anterior al historial.** En F12, F13 y F14, la `fechaEfectiva` no puede ser anterior a la última transacción registrada en las unidades involucradas ni a la última versión vigente de jerarquía. La validación requiere una fuente consultable de última imputación por unidad, alimentada por los consumidores transaccionales o por una proyección transversal acordada. | Eventual / Cross-domain | `UnidadOrganizacional` + consumidores | `[R25]`, `[SI10]` |
...
| `I10` | **Padre activo al crear/trasladar/reabrir.** Al crear (F1, F2, F9), trasladar (F14) o reabrir (F6) un nodo, el grupo padre destino debe estar en `Activo`. Enforcement por proyección (`[SI03]`) porque cruza dos agregados. | Eventual | Ambos | `[R07]`, `[R16]` |
...
| Capa BFF (Backend for Frontend) | Es infraestructura de aplicación que orquesta la experiencia de usuario. No es parte del dominio. | `anexo-orquestacion-creacion.md`. | — (infraestructura; sin eventos de dominio) |
...
| `D08` | **Un solo evento `UnidadDescartada` cubre rechazo del admin y abandono por inactividad** (cierra `[PD3]` del anexo de orquestación). El distinguidor del motivo va en el atributo `motivoBaja` (`operativa` vs `abandono_por_inactividad`). **Alcance específico de esta decisión:** `[PD3]` del anexo preguntaba si existía un evento explícito separado de rechazo (`UnidadRechazada`). La respuesta es no — el evento `UnidadDescartada` se unifica con diferenciación por atributo. La política operativa de descarte automático en sí (cuándo y cómo se ejecuta, umbrales, proceso programado) se resuelve por separado en `[D09]` + `[SI05]`. Los dos cierres son complementarios y cubren aspectos distintos de la misma área. | Evita inflar el catálogo de eventos. La auditoría diferencia los casos vía el atributo proyectado, no vía evento separado. | Plan, decisión cerrada con el usuario; cierra `[PD3]` del anexo orquestación |
| `D09` | **Descarte automático de Borradores por inactividad** mediante proceso programado del propio sub-dominio (`[SI05]`). Política: borrador con `fechaUltimaActividadBorrador` > umbral configurado por tenant (default sugerido 30 días) → emite `UnidadDescartada` con `motivoBaja: "abandono_por_inactividad"`. | Cierra `[PD2]` del anexo de orquestación. Reduce ruido operativo de Borradores abandonados. El umbral es parametrizable para que cada tenant ajuste según su realidad operativa. | Plan, decisión cerrada con el usuario; cierra `[PD2]` del anexo orquestación |
...
| `P05` | **Las solicitudes de creación desde consumidores son habituales en operación**, no excepcionales. OXP y Contabilidad emiten solicitudes con frecuencia cuando llegan documentos asociados a unidades no registradas. | Justifica que el flujo F2 sea tan robusto como F1 (idempotencia por `correlationId` en `[SI07]`, bandeja explícita en `[SI04]`, proceso de descarte automático en `[SI05]`). |

---

## 11. Pendientes por definir

**Mapping de pendientes cerrados desde el anexo de orquestación:**

| Pendiente original (anexo orquestación) | Resolución en este modelo |
|------------------------------------------|---------------------------|
| `[PD02]` (Política de expiración de Borradores) | **Cerrado** por `[D09]` + `[SI05]` (proceso programado de descarte automático con umbral configurable por tenant). |
| `[PD03]` (Comando explícito de rechazo) | **Cerrado** por `[D08]` (un solo evento `UnidadDescartada` con diferenciación por `motivoBaja`). |
| `[PD01]` (Política de activación automática por sistema inteligente) | **Sigue abierto** como `[PD01]` de este modelo (ver tabla a continuación). |

Naming convention: en ambos documentos (modelo y anexo) los pendientes se nombran `[PD01]`, `[PD02]`, `[PD03]` (no `[PD1]/[PD2]/[PD3]`). El anexo de orquestación fue actualizado en su v1.1 para esta convención.

| # | Pendiente | Tipo | Contexto | Trigger de activación |
|---|-----------|------|----------|----------------------|
| `PD01` | **Política de activación automática por sistema inteligente.** En F1 la activación es siempre humana. La activación automática (cuándo el sistema inteligente puede asumir el gesto sin intervención del administrador, p.ej. proyectos aprobados en un sistema de presupuesto con todos los datos) se evaluará en fases posteriores. | Futuro (F2+) | El alcance documentó que en F1 la activación es siempre humana. El sub-dominio queda preparado: el comando `ActivarUnidad` ya acepta `usuarioId: "sistema"`, pero las reglas de cuándo el sistema actúa no se definen aquí. | Cuando se integre el primer sub-dominio de presupuesto/aprobación que pueda servir como fuente inequívoca de contexto. Owner sugerido: equipo de producto + sub-dominio de Presupuesto. Continúa la conversación abierta de `[PD01]` del `anexo-orquestacion-creacion.md` v1.1. |
...
**Restricción de contexto:** todos los permisos se aplican dentro del scope `tenant` (cada tenant tiene su propia estructura organizacional independiente). Los permisos de operaciones específicas pueden restringirse adicionalmente por `subdominio_origen` (para diferenciar quién puede `solicitar_creacion_unidad` desde cada consumidor).
...
| Unidad organizacional | Crear directamente (F1) | `crear_unidad` |
| Unidad organizacional | Solicitar creación desde consumidor (F2) | `solicitar_creacion_unidad` |
