...
```
   OXP (consumidor)              EO (dueño)              Administrador EO
        │                            │                          │
  1 ─ Imputa contra su COPIA         │                          │
      LOCAL de unidades.             │                          │
      ¿Existe la unidad? NO.         │                          │
        │                            │                          │
  2 ─ DIFIERE solo la parte que      │                          │
      requiere la unidad; registra   │                          │
      todo lo demás. La operación    │                          │
      NO se detiene. ── corrección ─┐│                          │
        │                           ││                          │
  3 ─ Emite SEÑAL de demanda ────────►  proyecta en la          │
      (informativa, fire-and-forget; │   BANDEJA DE SUGERENCIAS │
       dedup por [SI07]) ─ visibilidad│   ([SI11]) ─────────────►  ve la sugerencia
        │                            │                          │
        │                            │  4 ─ decide crear ◄──────┘
        │                            │      (acto deliberado, F1:
        │                            │       CrearUnidad → ActivarUnidad)
        │                            │          │
        │   5 ─ UnidadActivada ◄─────┘──────────┘
        │       (evento de ciclo de vida, hacia todos los consumidores)
        │
  6 ─ La copia local de OXP se actualiza; el DIFERIDO del paso 2
      se resuelve SOLO (consistencia eventual) — sin consultar a EO
      en caliente, sin intervención humana en OXP.
        │
        ▼
  Causación completa contra la unidad real (coincide exacto con Contabilidad).
```

**Lo que el diagrama hace explícito:**

- **Dos caminos independientes desde el paso 1.** El de la *corrección* (2 → 5 → 6: diferir y esperar el evento) y el de la *visibilidad* (3: la señal). El sistema queda correcto por el primero; el segundo solo acelera que el administrador se entere.
- **La señal (paso 3) no es condición de nada.** Si se perdiera, el paso 2 sigue diferido y la unidad puede crearse igual por la planeación del administrador; cuando exista, el paso 6 ocurre de todos modos. Por eso la señal puede ser fire-and-forget.
