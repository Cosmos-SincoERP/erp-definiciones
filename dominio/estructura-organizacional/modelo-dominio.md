...
| [`../../guias-de-modelado/datos-entre-dominios.md`](../../guias-de-modelado/datos-entre-dominios.md) | **Fundamento del trato de la unidad como dato entre dominios** | Por qué el consumidor mantiene **copia local** y no consulta al dueño en caliente, las dos capas de sincronización, las tres estrategias para un dato que aún no existe (solicitar / valor de respaldo / **diferir**) y los anti-patrones. `[D13]` aplica esta guía a la unidad organizacional; aquí vive el razonamiento de *por qué* es el mejor patrón. |
...
| `codigo` | VO `Codigo` | Identificador plano de texto libre, único por tenant entre grupos y unidades. Inmutable. | longitud según el rango configurado por el tenant (`[R08]`, por defecto 2-12) |
| `nombre` | VO `Nombre` | Denominación descriptiva del grupo. | texto |
| `padreId` | Referencia | Identificador del grupo padre. Null cuando el grupo es un **grupo de primer nivel** (sin padre) — un tenant puede tener varios, cada uno con su propio árbol independiente (`[D14]`). Ver el comportamiento calculado `esDePrimerNivel()`. | uuid o null |
...
| `versionAgregado` | Entero (atributo interno de plataforma) | Stamp de concurrencia optimista materializado por `[SI05]`. No es atributo de negocio — se usa solo en validación de `expectedVersion` al hacer append. No aparece en los payloads de los eventos. | |
...
| `descendientesAfectablesPorCascada()` | Recorrido recursivo: sub-grupos `Activo` + unidades `Activa`/`Suspendida` + unidades `Borrador`. Devuelve lista clasificada por tipo. | F10, para mostrar al administrador el impacto previsto (`[R19]`) y para que la saga itere todos los nodos afectados. |
| `esDePrimerNivel()` | `padreId == null` | Nombre de negocio de la condición "grupo de primer nivel" (glosario del alcance, `[R28]`, `[D14]`). **Derivada, no almacenada:** un atributo almacenado permitiría el estado imposible "primer nivel con padre" — la derivación lo hace incontradecible por construcción. Las reglas y el código preguntan por este método, nunca por el null de `padreId` directamente; la proyección `[SI02]` lo expone para la UI y los reportes. |
| `puedeInactivarse()` | `estado == Activo` | Validación previa a F10. Si retorna false, el comando `InactivarGrupo` se rechaza. (La protección especial del raíz se retiró con `[D14]`: inactivar un grupo de primer nivel es un F10 normal — cascada con confirmación del impacto, `[R19]`.) |
...
| `codigo` | VO `Codigo` | Identificador plano de texto libre, único por tenant, inmutable. | longitud según el rango configurado por el tenant (`[R08]`) |
...
| `puedeReestructurarse()` | `estado in {Activa, Suspendida}` | Validación previa a F12/F13/F14 (`[I13]`). |
...
| `puedeModificarse()` | `estado in {Borrador, Activa, Suspendida}` | Validación previa a F15 (`[I14]`). |
...
**Descripción:** Clasificación de una unidad organizacional según su naturaleza (centro de costo, proyecto, sucursal, inmueble, departamento, y los que cada empresa agregue). Es un agregado raíz pequeño con **ámbito del tenant**: no pertenece a ningún nodo de la jerarquía (ver `[D09]`). El "catálogo de tipos" no es un agregado contenedor — es la **proyección de todos los `TipoUnidad` del tenant**. Las unidades lo referencian por identidad (`tipoUnidadId`), lo que permite renombrar un tipo sin romper referencias.
...
| `nombre` | VO `Nombre` | Denominación del tipo. **Única por tenant** (`[SI10]`) y **modificable** — las unidades referencian por id, no por nombre. | texto |
...
| `versionAgregado` | Entero (atributo interno de plataforma) | Concurrencia optimista materializada por `[SI05]`, igual que en los demás agregados. | |

**Proyección de catálogo vigente — `tiposVigentes(tenant)`:** consulta de los `TipoUnidad` del tenant con `estado == Activo`. La usan las validaciones de creación, activación y cambio de tipo de unidades (`[I07]`) y la UI del administrador. Reemplaza la antigua herencia dinámica desde el grupo raíz (retirada en el issue #86): ya no se recorre la jerarquía — el catálogo es plano, del tenant.
...
|----|-----------|-------------|
| `Codigo` | `GrupoOrganizacional`, `UnidadOrganizacional` | Cadena de **texto libre con salvaguardas de dominio**: no vacía, sin espacios al inicio/fin **ni espacios internos**, solo caracteres imprimibles (sin saltos de línea ni caracteres de control). Separadores como guion, punto o barra están permitidos pero son **cosméticos**: **codificación plana, sin estructura jerárquica embebida** — la jerarquía se modela como agregado separado y se proyecta vía `[SI02]` (ver `[DA1]`). La longitud se valida contra el rango `min`/`max` configurado por el tenant, dentro de la envolvente del dominio (`1 ≤ min ≤ max ≤ 50`; por defecto **2–12**; longitud fija = `min == max`) (`[R08]`, `[I03]`). Inmutable una vez asignado (`[R07]`). Único por tenant cruzando grupos y unidades, **sin distinguir mayúsculas de minúsculas** (`[R06]`, `[I09]`, `[SI01]`). |
...
| `FechaEfectiva` | Eventos de reestructuración y de transición de jerarquía | Momento a partir del cual una versión de la jerarquía o una reestructuración rige. No puede ser anterior a la última versión vigente de jerarquía de las unidades involucradas (`[I08]`, validable localmente); su coherencia con la actividad transaccional la define y responde el administrador (`[R23]`). |
...
|-------------------|---------------------------------------|
| `CrearGrupo` (F9) | `[SI01]`, `[SI02]`, `[SI03]`, `[SI05]` |
| `CrearUnidad` (F1 — admin) | `[SI01]`, `[SI03]`, `[SI04]`, `[SI05]` |
| `ActivarUnidad` (F3) | `[SI03]`, `[SI05]` |
| `SuspenderUnidad` (F4) / `ReactivarUnidad` (F5) | `[SI03]`, `[SI05]` |
| `ReabrirUnidad` (F6) | `[SI03]`, `[SI05]` |
| `InactivarUnidad` (F7) | `[SI05]` |
| `DescartarUnidad` (F8 manual) | `[SI04]`, `[SI05]` |
| `InactivarGrupo` (F10) | `[SI05]`, `[SI07]`, `[SI08]` |
| `ReactivarGrupo` (F11) | `[SI05]`, `[SI07]` (consulta correlación) |
| `FusionarUnidades` (F12) | `[SI02]`, `[SI05]`, `[SI08]` |
| `DividirUnidad` (F13) | `[SI02]`, `[SI05]`, `[SI08]` |
| `TrasladarUnidad` (F14) | `[SI02]`, `[SI03]`, `[SI05]` |
| `ModificarUnidad` / `ModificarGrupo` (F15) | `[SI05]` |
| `AgregarTipoUnidad` / `ModificarTipoUnidad` / `InactivarTipoUnidad` | `[SI05]`, `[SI10]` |

---

#### `[SI01]` Índice único de `Codigo` por tenant

Materializa `[I09]`, `[R06]`, `[R09]`.

Mantener un índice único por `(tenantId, codigo)` que cubre simultáneamente grupos y unidades organizacionales (espacio de nombres único — un grupo y una unidad no pueden compartir código en el mismo tenant). El índice **excluye unidades en `Descartada`** para liberar la identificación según `[R09]`.

El índice se construye sobre la **forma normalizada** del código: mayúsculas y minúsculas son equivalentes (`cc-001` y `CC-001` son el mismo código) y la forma Unicode se estabiliza para que un mismo carácter acentuado escrito de dos maneras no produzca "códigos gemelos"; los acentos **sí** distinguen (`CAJÁ` ≠ `CAJA`). El código se almacena y se muestra tal como el usuario lo escribió — la normalización es solo para comparar unicidad (`[R06]`, `[I03]`).
...
3. Si dos comandos concurrentes superan la validación pero solo uno logra el append (el otro recibe `version-conflict` por `[SI05]`), el segundo se rechaza con error específico `version-conflict` y la UI re-lee el estado y solicita confirmación al usuario.
...
#### `[SI05]` Concurrencia optimista por agregado

Soporta `[R10]`, `[R16]` y todas las transiciones FSM.
...
#### `[SI06]` Idempotencia de los comandos del administrador
...
- **Concurrencia optimista (`[SI05]`):** el comando lleva `expectedVersion`; un reintento sobre una versión ya aplicada se detecta como `version-conflict` y no duplica el efecto.
...
> **Nota — reorientación (issue #72).** Entre el #46 y el #72, `[SI06]` cubrió la idempotencia de la **señal de demanda** entrante. Al retirarse esa señal, `[SI06]` vuelve a su objeto original: la idempotencia de los comandos del administrador, que no necesita tabla propia porque la resuelven `[SI05]` y `[SI01]`.

#### `[SI07]` Saga `CascadaInactivacionGrupo` — política de completud

Materializa `[I12]`, `[I15]`, `[R17]`, `[R18]`.
...
**Sin compensación inversa:** no se implementa rollback automático. Si la saga queda irrecuperablemente incompleta, el operador debe ejecutar `ReactivarGrupo` (F11) sobre el grupo origen y reabrir los hijos manualmente. La invariante `[I15]` garantiza que toda saga converge en estado coherente.
...
#### `[SI08]` Tabla de deduplicación de eventos de saga

Materializa el principio at-least-once de `[SI07]` y `[SI05]`.
...
#### `[SI09]` Punto de resincronización de respaldo

Soporta `[R11]`, `[R27]` y la copia local de los consumidores.
...
- **Fuera del camino crítico:** el consumidor lo usa de fondo, no al imputar. Imputar siempre va contra su copia local (`[R11]`). Este punto solo repara la copia, no participa en cada operación.
...
#### `[SI10]` Índice único de nombre de `TipoUnidad` por tenant

Análoga a `[SI01]` (índice único del código — se referencia explícitamente como patrón fuente). Mantener un índice único por `(tenantId, nombre normalizado)` sobre los agregados `TipoUnidad`. Los comandos `AgregarTipoUnidad` y `ModificarTipoUnidad` (cuando el delta incluye `nombre`) validan contra el índice antes de aceptar; la concurrencia se resuelve con la misma política de `version-conflict` de `[SI01]`/`[SI05]`.
...
4. **Espera el write-ack del broker** para el `GrupoInactivado` raíz — solo la confirmación de persistencia en el event store, no de procesamiento downstream. Este es el "punto de no retorno" de la saga: una vez confirmada la persistencia del raíz, la saga está comprometida a propagar a todos los descendientes (`[I15]`). Si el broker no confirma (timeout, error), la saga aborta limpiamente sin emitir derivados.
5. Tras el write-ack, emite los eventos derivados en paralelo (no espera write-ack individual de cada uno; cada uno se valida vía `[SI08]` para evitar duplicados):
...
**Alerta operacional:** si la saga lleva más de 15 min (configurable por tenant) sin completar, se emite alerta interna para revisión humana. La saga sigue intentando converger en segundo plano. La invariante `[I15]` garantiza que toda saga converge a estado coherente (completa o `dead-letter`).
...
**Protocolo de proceso:** `correlationId` único; estado de la saga persistido en `SagaCascadaEstado` para retomar en caso de caída del proceso; cada evento derivado lleva el `correlationId` y `esCascada: true` para auditoría y para soportar la reactivación correlacionada de F11 (ver `[SI07]`).
...
1. Validar precondiciones cruzadas (`[I13]`, `[R20]`, `[R21]`, `[R22]`, `[R23]`). Las precondiciones por unidad las valida cada agregado vía `puedeFusionarse()`; las cruzadas (separación origen/destino, fecha efectiva coherente, destino en `Activa`) las valida el servicio.
...
5. Tras el write-ack, por cada unidad origen, emitir `UnidadInactivada` con `motivoBaja: "fusion"`, `causalidadBaja: destinoId`, `fechaEfectiva`, `esCascada: true` y `correlationId`. Los eventos a las unidades origen se emiten en paralelo, deduplicados por `[SI08]`. Los consumidores reconcilian el orden lógico vía `correlationId`.
...
| 4 | `UnidadInactivada` por origen | Stream de cada unidad origen | Max 5 reintentos por unidad. **Idempotencia por `(unidadId, correlationId)`** vía `[SI08]`. Si tras 5 reintentos no converge, entra en `dead-letter`; alerta operacional tras 15 min sin completar. |
...
- Mezcla libre de hijos en grupos (`[R03]`).
- Grupos de primer nivel (`esDePrimerNivel()`): 1..N por tenant — cada uno encabeza su **propio árbol independiente** y la frontera de consolidación es el tenant (`[D14]`).
...
> **Nota — qué cambió respecto al replanteamiento #46.** El #46 introdujo, además, una **señal de demanda** del consumidor hacia Estructura Organizacional y una **bandeja de sugerencias de creación**, pensadas para el caso "el consumidor necesita una unidad que el administrador aún no ha creado". Una vez implementada la asignación/distribución de la unidad en los consumidores (la UI elige de la fuente de verdad; las reglas resuelven sobre unidades existentes), ese caso deja de ocurrir en el camino operativo y ese aparato quedó **sin disparador** → se retiró (la bandeja, la regla de demanda del alcance y la parte 4 de `[D13]`). Lo que permanece es la copia local para validar, el diferir por consistencia eventual y la resincronización de respaldo.
...
- **`Inactiva`** — Estado de baja post-operación. Se conserva el historial. No admite imputaciones (`[R11]`) ni modificaciones (`[R15]`, `[I14]`). Admite reapertura mediante `UnidadReabierta` (F6), que la lleva a `Activa`. Lleva atributo `motivoBaja` proyectado (`operativa`, `fusion` o `division` — `cascada_grupo` no aplica aquí: un `Borrador` arrastrado por la cascada termina en `Descartada`, nunca en `Inactiva`).
...
- **`Descartada`** ■ — Terminal estricto (`[R12]`). La unidad nunca operó (siempre vino desde `Borrador`). Su `codigo` queda libre para una nueva solicitud (`[R09]`).
...
| **Precondiciones** | `[R06]` (código único), `[R05]` (grupo padre Activo), tipo vigente en el catálogo de tipos del tenant (`[I07]`, Sección 3.4), formato del código válido (`[R08]`), `[I09]` (unicidad cruzada en tenant). |
...
| **Precondiciones** | Datos mínimos completos (`[I07]`), grupo padre en estado `Activo` (`[I10]`, `[R05]`), no existe otra unidad `Activa` con el mismo `codigo` (`[I09]`). |
...
| **Efectos** | La unidad acepta imputaciones (`[R11]`). Los consumidores actualizan su copia local; las operaciones que estaban diferidas a la espera de esta unidad se resuelven solas (`[R27]`, `[D13]`). El payload self-contained permite a los consumidores actualizar referencias locales sin consultas adicionales. |
...
| **Estado resultante** | `Activa` |
| **Precondiciones** | Unidad en `Inactiva` (`[R13]`); grupo padre en `Activo` (`[R14]`, `[I06]`). |
...
| **Precondiciones** | Unidad en `Activa` o `Suspendida` (`[R15]`); en F12/F13 además se aplican `[R20]`-`[R23]`. |
...
| **Estado resultante** | `Descartada` ■ (terminal estricto, `[R12]`) |
...
| **Efectos** | El `codigo` de la unidad queda libre para una nueva creación (`[R09]`). La unidad se filtra de reportes históricos. Descartar un borrador del administrador no afecta a ningún consumidor — un borrador nunca fue referenciado por la operación de otro sub-dominio (ningún consumidor opera ni difiere contra borradores; difiere a la espera de una unidad `Activa`, `[D13]`). |

#### `UnidadModificada`

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Cambian uno o más campos descriptivos o clasificatorios de una unidad: nombre, tipo, descripción. El código y el grupo padre no se modifican por este evento (el código es inmutable, `[R07]`; el cambio de padre se hace por F14 con `UnidadTrasladada`). |
...
| **Precondiciones** | Unidad en estado modificable (`[R15]`, `[I14]`); si cambia el tipo, el nuevo tipo está vigente en el catálogo (`[I07]`). |
...
| **Precondiciones** | Código único en el tenant (`[I09]`); si se indica `padreId`, el grupo padre existe y está en `Activo` (`[R05]`) y la posición no introduce ciclos (`[I11]`); sin `padreId`, el grupo nace como **grupo de primer nivel** (`[D14]`); formato del código (`[R08]`). |
...
| **Precondiciones** | Grupo en `Activo`; el administrador confirmó el impacto previsto (`[R19]`, validación previa al comando). |
...
| **Precondiciones** | Todas las unidades origen en `Activa` o `Suspendida` (`[R21]`, `[I13]`); destino en `Activa` y distinto del conjunto origen (`[R20]`, `[R22]`); fecha efectiva no anterior a la última versión vigente de jerarquía (`[I08]`, validable localmente). La coherencia con la actividad transaccional la responde el administrador (`[R23]`). |
...
| **Precondiciones** | Unidad origen en `Activa` o `Suspendida` (`[R21]`); al menos dos destinos, todos en `Activa`, distintos del origen (`[R20]`, `[R22]`); fecha efectiva no anterior a la última versión vigente de jerarquía (`[I08]`, validable localmente). La coherencia con la actividad transaccional la responde el administrador (`[R23]`). |
...
| **Efectos** | Marca el inicio del proceso. Le sigue 1 `UnidadInactivada` para la origen (con `motivoBaja: "division"` y el mismo `correlationId`). Los consumidores reasignan sus referencias futuras según corresponda; el historial previo a la fecha efectiva queda referenciado al origen (`[R25]`). El payload self-contained (incluye códigos) permite a los consumidores reconstruir la división histórica sin queries adicionales. |
...
| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una unidad cambia de grupo padre en la jerarquía (F14). Conserva identidad, código, estado e historial transaccional (`[R26]`). Solo cambia su posición en el árbol y la versión vigente de jerarquía. |
...
| **Precondiciones** | Unidad en `Activa` o `Suspendida` (`[R21]`); nuevo grupo padre existente, en `Activo` y distinto del padre actual (`[R05]`, `[I10]`); fecha efectiva coherente con la versión vigente (`[R23]`, `[I08]`). |
...
| **Estado previo** | — (creación) |
| **Estado resultante** | `Activo` |
| **Precondiciones** | El `nombre` no duplica el de otro tipo del tenant (`[SI10]`, cubre activos e inactivos). |
...
| **Estado previo** | `Activo` |
| **Estado resultante** | Sin cambio de estado. |
| **Precondiciones** | Tipo existente y `Activo`. Si el delta incluye `nombre`, el nuevo nombre no duplica el de otro tipo del tenant (`[SI10]`). |
...
| **Agregado** | `TipoUnidad` |
| **Estado previo** | `Activo` |
| **Estado resultante** | `Inactivo` ■ (terminal en F1, `[D12]`) |
| **Precondiciones** | Tipo existente y `Activo`. |
| **Información capturada** | `tipoUnidadId`, `motivo` (opcional), `usuarioId`, `timestamp`. |
| **Efectos** | El tipo deja de aparecer en las opciones de creación de unidades. Las unidades existentes que lo usan no requieren acción. Su `nombre` no se libera (`[SI10]`). |
...
15 invariantes. Clasificación: **local** (un solo agregado, transaccional) o **eventual** (cruza fronteras, enforcement por proyección).

| # | Invariante | Tipo | Agregado | Referencia |
|---|-----------|------|----------|------------|
| `I01` | **Transiciones FSM válidas de unidad.** Solo se permiten las 7 transiciones documentadas en Sección 4.1 (`Borrador → Activa`, `Borrador → Descartada`, `Activa → Suspendida`, `Suspendida → Activa`, `Activa → Inactiva`, `Suspendida → Inactiva`, `Inactiva → Activa`). Cualquier otra es rechazada por el agregado antes del append. | Local | `UnidadOrganizacional` | `[R10]` |
| `I02` | **Transiciones FSM válidas de grupo.** Solo se permiten `Activo → Inactivo` y `Inactivo → Activo`. | Local | `GrupoOrganizacional` | `[R16]` |
| `I03` | **Formato del código.** El código es texto libre con salvaguardas: no vacío, sin espacios al inicio/fin ni internos, solo caracteres imprimibles. Su longitud cumple el rango `min`/`max` configurado por el tenant, dentro de la envolvente del dominio (`1 ≤ min ≤ max ≤ 50`; por defecto 2–12; longitud fija = `min == max`). El VO `Codigo` valida el rango efectivo del tenant. | Local | Ambos | `[R08]` |
| `I04` | **Inmutabilidad del código.** Una vez asignado, el código de una unidad o grupo no se modifica en ningún comando posterior. | Local | Ambos | `[R07]` |
| `I05` | **Coherencia `motivoBaja` ↔ flujo.** Cuando `estado in {Inactiva, Descartada}`, el atributo `motivoBaja` está definido y su valor corresponde al flujo que disparó la baja (`operativa` desde F7 o F8 manual; `fusion` desde F12; `division` desde F13; `cascada_grupo` desde F10 cascada para Borradores). | Local | `UnidadOrganizacional` | `[R24]` |
| `I06` | **Reapertura requiere padre activo.** Un comando `ReabrirUnidad` solo se acepta si el `grupoPadreId` está en `Activo` al momento del comando. | Eventual | `UnidadOrganizacional` | `[R14]` |
...
| `I08` | **Fecha efectiva de la reestructuración.** En F12, F13 y F14 la `fechaEfectiva` se gobierna en dos planos: **(a)** no puede ser anterior a la última versión vigente de jerarquía de las unidades involucradas — Estructura Organizacional lo valida **localmente** (es dueña de la jerarquía); **(b)** su coherencia con la actividad transaccional (no fijarla sobre periodos que ya tienen movimientos) la **define y responde el administrador** que ejecuta la reestructuración, como acto deliberado de gestión — el sistema no la valida contra el historial transaccional porque las transacciones/asientos son inmutables y los reportes ofrecen vista actual e histórica. | Local | `UnidadOrganizacional` | `[R23]` |
| `I09` | **Unicidad de código por tenant.** El `codigo` es único dentro del tenant cruzando grupos y unidades. Las unidades en `Descartada` se excluyen del índice para liberar la identificación (`[R09]`). Enforcement por proyección (`[SI01]`) porque cruza dos agregados. | Eventual | Ambos | `[R06]`, `[R09]` |
| `I10` | **Padre activo al crear/trasladar/reabrir.** Al crear (F1, F9), trasladar (F14) o reabrir (F6) un nodo, el grupo padre destino debe estar en `Activo`. Enforcement por proyección (`[SI03]`) porque cruza dos agregados. | Eventual | Ambos | `[R05]`, `[R14]` |
| `I11` | **No ciclos en la jerarquía.** Un grupo no puede ser su propio ancestro directo ni indirecto. La validación se hace al recibir comandos que cambian la jerarquía (`CrearGrupo`, `TrasladarUnidad`) consultando la proyección (`[SI02]`). | Eventual | `GrupoOrganizacional` | `[R02]` |
| `I12` | **Integridad de la cascada de inactivación.** Cuando `GrupoInactivado` se emite por la saga `CascadaInactivacionGrupo`, todos los descendientes vivos al momento de la captura reciben su propio evento (`GrupoInactivado` para sub-grupos, `UnidadInactivada` o `UnidadDescartada` para unidades) correlacionado por `correlationId`. No quedan nodos descendientes en estado operativo bajo un grupo inactivado. | Eventual | Ambos (vía saga) | `[R17]` |
| `I13` | **Reestructuración: estados y separación origen/destino.** En F12 y F13, las unidades origen están en `Activa` o `Suspendida`, todas las destino están en `Activa`, y no hay intersección entre el conjunto origen y el conjunto destino. | Local | `UnidadOrganizacional` (validado en saga `ReestructuracionUnidad`) | `[R20]`, `[R21]`, `[R22]` |
| `I14` | **Modificación bloqueada en estados terminales.** Las unidades en `Inactiva` o `Descartada` no aceptan `UnidadModificada`. La unidad debe ser reabierta primero (F6 → F15 → F7) si el cambio es necesario. | Local | `UnidadOrganizacional` | `[R15]` |
| `I15` | **Convergencia de la cascada de inactivación.** Toda saga `CascadaInactivacionGrupo` que emite el evento `GrupoInactivado` raíz **converge en estado coherente**: todos los descendientes capturados al inicio terminan en estado no operativo (`Inactivo`, `Inactiva` o `Descartada`) o registrados como `dead-letter` para revisión humana. No quedan descendientes en limbo indefinido. Enforcement por la persistencia de la saga (`[SI07]`) + alerta operacional tras N minutos sin completar. | Eventual (Saga) | Ambos (vía `CascadaInactivacionGrupo`) | `[R17]`, `[R18]` |
...
| Capa BFF (Backend for Frontend) | Es infraestructura de aplicación que orquesta la experiencia de usuario del propio módulo. No es parte del dominio. La orquestación BFF de creación desde consumidores quedó superada por `[D13]` (la demanda es una señal informativa, no un comando vía BFF). | Infraestructura de aplicación del módulo. | — (infraestructura; sin eventos de dominio) |
...
| **2.3** | **2026-07-08** | **Formato y longitud del código — texto libre con salvaguardas + rango configurable por tenant (issue #89).** Resuelve la contradicción "alfanumérico" vs los ejemplos con guion del anexo, y el choque del tope 4–12 con la migración de estructuras legadas (el código es inmutable y ancla de trazabilidad — remapear códigos largos la destruiría). **VO `Codigo` reescrito:** texto libre con salvaguardas de dominio (no vacío, sin espacios al inicio/fin **ni internos** — un espacio interno es invisible y genera duplicados fantasma —, solo caracteres imprimibles); separadores permitidos pero cosméticos (`[DA1]` intacta). **`[I03]` reescrita:** la longitud cumple el rango `min`/`max` configurado por el tenant dentro de la envolvente del dominio (`1 ≤ min ≤ max ≤ 50`; por defecto **2–12**, decidido con el usuario; longitud fija = `min == max`). **`[SI01]` con normalización de unicidad:** mayúsculas y minúsculas equivalentes (`cc-001` = `CC-001`) + forma Unicode estable (sin "códigos gemelos"); los acentos sí distinguen; el código se almacena y muestra como el usuario lo escribió — la normalización es solo para comparar. Filas de composición de ambos agregados ajustadas. Acompaña al alcance v1.10 (R10 y término 5 reconciliados — desaparece el "sugerida") y al anexo v1.5. |
| **2.4** | **2026-07-08** | **Ajuste de lenguaje: se retira el término "bosque"** (tecnicismo de teoría de grafos, señalado por el usuario) del texto vivo — `[D16]`, composición (`padreId`) y relaciones (3.8) pasan a decir "la estructura admite **varios grupos de primer nivel**, cada uno con su propio árbol independiente". "Árbol" se conserva (lenguaje natural del dominio). Acompaña al alcance v1.11 y al anexo v1.6. |
| **2.5** | **2026-07-08** | **Renumeración — se cierran los saltos de numeración en decisiones, invariantes y sugerencias de implementación** (decisión del usuario: precisión en los documentos por encima de la convención anterior de conservar numeración con huecos). **Mapeo — Decisiones:** D10→`D09`, D11→`D10`, D12→`D11`, D13→`D12`, D15→`D13`, D16→`D14` (quedan `D01`-`D14` continuas). **Invariantes:** I14→`I13`, I15→`I14`, I16→`I15` (quedan `I01`-`I15` continuas). **Sugerencias:** SI06→`[SI05]`, SI07→`[SI06]`, SI08→`[SI07]`, SI09→`[SI08]`, SI12→`[SI09]`, SI13→`[SI10]` (quedan `[SI01]`-`[SI10]` continuas; la sección del índice de nombres de tipo se reubicó tras la de resincronización para mantener el orden). Todas las referencias del cuerpo ajustadas (184 reemplazos), incluidas las referencias a reglas del alcance renumeradas (R4-R29→R2-R27, R31→R28). Las **menciones retrospectivas** a piezas retiradas quedaron sin número (la vieja numeración ya no es direccionable) y se eliminaron las dos notas de "SI retirada — se conserva la numeración", que existían solo para explicar los huecos. **Las entradas históricas de este control de versiones conservan la numeración de su momento** — leerlas con esta tabla de mapeo. **Referencias cruzadas actualizadas en el mismo cambio:** OXP modelo v4.5 (`[D15]`→`[D13]`, `[SI12]`→`[SI09]` de EO) y `documento-consolidado-erp.md` (5 referencias). Acompaña al alcance v1.12. |
