...
| 27 | **Retención a beneficiarios del exterior** | Retención en la fuente que la empresa practica sobre pagos o abonos en cuenta a una contraparte **sin residencia ni domicilio fiscal en el país**, por rentas de fuente nacional (Colombia: arts. 406 a 408 del Estatuto Tributario, 20 % general por concepto de pago). Se modela como **tributo propio**, distinto de la retención doméstica: cuando aplica, la retención doméstica no se practica — nunca concurren. Es el impuesto **definitivo** del beneficiario (no declara en el país). Su tarifa se reduce cuando el país de residencia fiscal del beneficiario tiene un convenio para evitar la doble imposición vigente (término 28). Impuestos la liquida como cualquier retención; si la empresa la **asume** (el proveedor recibe el total) o la descuenta lo decide el sub-dominio consumidor, que conoce el acuerdo con el proveedor. |
| 28 | **Convenio para evitar la doble imposición** | Tratado bilateral entre el país y otro Estado que reparte la potestad de gravar las rentas transfronterizas y, para las rentas que el país conserva, fija una tarifa de retención **reducida** por concepto (o la exime). Se gestiona como **catálogo propio** del sub-dominio, por país, con vigencia propia (entrada en vigor, protocolos, denuncia) y como contenido fiscal del producto (R42). El motor lo consulta a partir del **país de residencia fiscal** declarado en el perfil tributario de la contraparte — no del país de su identificación. |
...
3. El administrador fiscal mantiene los **perfiles tributarios** de las entidades fiscales (emisoras y contrapartes) como datos propios del sub-dominio de Impuestos (régimen tributario, condición de autorretenedor, gran contribuyente, agente de retención, entre otros según la jurisdicción). Este es el principal dato que el cliente configura al iniciar operación. El sistema ofrece **carga asistida del perfil tributario**: a partir del número de identificación de la entidad fiscal, consulta fuentes oficiales de la autoridad fiscal de la jurisdicción correspondiente y extrae automáticamente los atributos tributarios disponibles (responsabilidades fiscales, régimen, estado, actividad económica). El administrador fiscal valida y confirma los datos extraídos. **El alta del perfil ocurre siempre por una vía única que crea o reutiliza** (si no existe perfil para la identificación y el país, lo crea; si existe, lo entrega — repetir la operación nunca duplica ni falla), invocable desde tres lugares: el flujo de captura de los dominios (al registrar al tercero — ej: el Proveedor en OXP — el formulario ofrece capturar su perfil tributario en el mismo paso), la administración directa, o la carga asistida. La identidad de la entidad fiscal se valida **localmente con la validación empaquetada del producto** — sin consultar servicios. Los sub-dominios consumidores referencian a la entidad fiscal por su identificación (tipo y número de identificación); el sistema de impuestos resuelve el perfil tributario internamente.
...
| **Bodega de Terceros** | Eventos del perfil tributario (creado y cambios), por identificación legal | Enriquecer la ficha consolidada del tercero — Impuestos informa, la bodega muestra |
...
| **R42** | **Catálogo propio de convenios para evitar la doble imposición:** Los convenios vigentes de cada país con otros Estados se gestionan como catálogo propio del sub-dominio de Impuestos, por país, con la tarifa de retención convenida por concepto de pago (o la exención) y su vigencia. Se diferencian de los regímenes especiales empresariales (R41) porque no hay empresa inscrita — son tratados entre Estados — y de las tarifas del tributo porque la tarifa depende del **país de residencia fiscal de la contraparte**, no de un factor del tributo. La precarga estándar viene como parte del contenido fiscal del producto (R38) y el administrador fiscal puede extenderla. El país de residencia fiscal es un dato **declarado** del perfil tributario de la contraparte, obligatorio cuando no tiene domicilio fiscal en el país. | No |
...
| **R10** | **Resolución por perfil tributario:** El perfil tributario de la entidad fiscal contraparte afecta la determinación: régimen tributario, condición de autorretenedor, gran contribuyente, agente de retención y demás atributos según la jurisdicción. Si la contraparte es autorretenedora, la entidad fiscal emisora no le practica retención (la retención la asume la contraparte sobre sus propios ingresos). Si la contraparte **no tiene domicilio fiscal en el país**, la retención doméstica no aplica y en su lugar se practica la retención a beneficiarios del exterior (término 27), con la tarifa del convenio para evitar la doble imposición cuando el país de residencia fiscal de la contraparte lo tiene vigente (término 28). | No |
...
| **R17** | **Naturaleza aditiva, sustractiva o de provisión:** Los impuestos (IVA, INC, ITBIS) son aditivos — incrementan el valor total de la transacción. Las retenciones (RETEFUENTE, RETEFUENTE_EXTERIOR, RIVA, RICA) son sustractivas — se descuentan del valor a pagar al tercero. Las autorretenciones y los tributos autoliquidados son de **provisión** — se reconocen sobre la transacción sin afectar el valor a pagar ni a cobrar. La naturaleza del tributo determina su efecto sobre el valor neto. | No |
| **R18** | **Retención como recaudo anticipado o impuesto definitivo:** Según la jurisdicción, una retención puede operar como recaudo anticipado de un impuesto que el tercero declarará y descontará en su declaración (Colombia), o como impuesto definitivo que constituye el pago total de la obligación tributaria (algunas retenciones de República Dominicana; en Colombia, la retención a beneficiarios del exterior — término 27). El sistema registra esta distinción en el registro tributario porque afecta el tratamiento del certificado tributario y la información reportada al tercero. | No |
...
| **Retención a beneficiarios del exterior** | El motor determina la retención propia sobre pagos a contrapartes sin domicilio fiscal en el país (término 27), excluyendo la retención doméstica en ese caso y aplicando la tarifa del convenio para evitar la doble imposición cuando el país de residencia fiscal de la contraparte lo tiene vigente. | F1 |
| **Gestión de convenios para evitar la doble imposición** | El sistema mantiene como dato propio el catálogo de convenios vigentes por país, con tarifas convenidas por concepto de pago y vigencia (R42). Precarga estándar para CO y PA. | F1 |
...
|-------------|-------------|----------------------------------------|
| **Validaciones empaquetadas** | Paquete del producto con las reglas de identificación legal (tipo por país, formato, dígito de verificación), custodiado por Datos de Referencia. Viaja incluido en el despliegue — sin servicio en ejecución. | La identidad de la entidad fiscal se valida **localmente** al capturar el perfil — con las mismas reglas de todo el ERP y en la misma clave natural con la que consolida la bodega de Terceros. Impuestos no consulta ningún servicio de terceros (replanteamiento jun-2026): es **fuente** de la bodega, no su consumidor. |
...
| Gestión de convenios para evitar la doble imposición | Núcleo | Catálogo propio de convenios vigentes por país (CO 15, PA 17 en la precarga), con tarifas convenidas por concepto de pago. Lo consulta el motor a partir del país de residencia fiscal declarado en el perfil de la contraparte. |
...
| 1.5 | Junio 2026 | **El perfil tributario sin registro centralizado de terceros (replanteamiento #31, issue #39).** El alta del perfil queda definida por una vía única que crea o reutiliza (sin duplicados ni rechazos por repetición) con tres invocaciones (flujo de captura con composición en BFF, administración directa, carga asistida — Flujo 1); la identidad de la entidad fiscal se valida localmente con la validación empaquetada del producto, en la misma clave natural de la bodega de Terceros. La dependencia "Gestión de Terceros" (consulta de identidad) se reemplaza por "Validaciones empaquetadas"; nueva integración de salida: eventos del perfil hacia la bodega de Terceros para enriquecer la ficha consolidada. Acompaña al modelo v2.0.5 (`[D16]`). |
| 1.6 | Septiembre 2026 | **Retención a beneficiarios del exterior y convenios para evitar la doble imposición (issue #143).** Revisión con el contador de la empresa y las consultoras fiscales: la retención a proveedores sin domicilio fiscal en el país (20 % general, tarifa reducida con convenio) es una figura propia que no dependía de calidades del perfil ni del medio de pago y el alcance no la nombraba. **Glosario:** términos 27 (Retención a beneficiarios del exterior) y 28 (Convenio para evitar la doble imposición). **Reglas:** R10 ampliada (sin domicilio fiscal → retención del exterior en lugar de la doméstica); R17 incorpora la naturaleza de provisión (pendiente desde el modelo v2.0.8); R18 reconoce el carácter definitivo en Colombia; **R42** nueva (catálogo propio de convenios, molde de R40/R41). **Sección 7:** capacidades "Retención a beneficiarios del exterior" y "Gestión de convenios para evitar la doble imposición" (F1). **Sección 8 F1:** fila del catálogo de convenios. Acompaña al modelo v2.1.0 (`[D17]`, agregado `ConvenioDeDobleImposicion`). |
