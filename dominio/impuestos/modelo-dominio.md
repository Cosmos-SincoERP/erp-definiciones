...
- **Creación (`[D16]`):** comando idempotente **`AsegurarPerfilTributario`** — crea el perfil si no existe para (identificación × país) o retorna el existente. Tres vías de invocación: el **flujo de captura con composición en el BFF** (al registrar al tercero en su dominio — ej: el Proveedor en OXP — el formulario ofrece capturar el perfil tributario en el mismo paso; el BFF compone y el comando viaja a Impuestos), la **administración directa** del administrador fiscal, y la **carga asistida** desde fuentes oficiales. **Nunca por eventos de Terceros**: el registro centralizado desapareció con el replanteamiento (#31) — no existe `TerceroActivado` ni ningún detonante externo.
...
**Decisión:** (1) El `PerfilTributario` se crea mediante el comando idempotente **`AsegurarPerfilTributario`** (crear o reutilizar por identificación × país), invocado desde el flujo de captura con composición en el BFF, por la administración directa o por la carga asistida — ver Sección 3.6. (2) La `IdentificacionFiscal` se construye y valida con la pieza del paquete **`IdentificacionLegal`** — validación local empaquetada, misma clave natural de la bodega. (3) Los eventos del perfil se publican como **integración hacia la bodega de Terceros** para enriquecer la ficha consolidada — Impuestos es fuente de la bodega, nunca su consumidor.

**Justificación:** La transversalidad se resuelve con distribución (validación empaquetada + información por eventos), no con dependencia en ejecución — el mismo principio de todo el replanteamiento. La idempotencia del comando reproduce el patrón `AsegurarProveedor` de OXP (`[D30]` de su modelo): la captura nunca se bloquea ni duplica perfiles. La clave compartida con la bodega garantiza que el perfil del tercero y sus roles consoliden bajo la misma identidad sin traducción.
