...
|---|---|---|
| `Vigencia` | Rango temporal de validez de una entrada: `fechaDesde`, `fechaHasta` (opcional — abierto si no se cierra). Se usa para acotar temporalmente cualquier dato fiscal que pueda cambiar con vigencia futura (`[P5]`). | TarifaTributaria, CondicionDeAplicacion, CatalogoDeAtributosFiscales, PerfilTributario, JurisdiccionFiscal, CatalogoDeRegimenesEspeciales, ConvenioDeDobleImposicion, HomologacionFiscal, FormatoFiscal |
| `Origen` | Procedencia del contenido de configuración: `estándar` (provisto con el producto) o `personalizado` (configurado por el cliente). La precedencia es personalizado > estándar (`[P3]`). | CatalogoTributario, TarifaTributaria, CondicionDeAplicacion, CatalogoDeAtributosFiscales, JurisdiccionFiscal, CatalogoDeRegimenesEspeciales, ConvenioDeDobleImposicion, HomologacionFiscal, FormatoFiscal |
...
|---|---|---|---|
| **Núcleo** | Configuración tributaria, determinación/cálculo, perfil tributario, registro tributario, jurisdicciones fiscales, regímenes especiales empresariales y convenios para evitar la doble imposición, confirmación de transacciones | CatalogoTributario, TarifaTributaria, CondicionDeAplicacion, CatalogoDeAtributosFiscales, PerfilTributario, JurisdiccionFiscal, CatalogoDeRegimenesEspeciales, ConvenioDeDobleImposicion, RegistroTributario, MotorDeCalculo, ConfirmacionTributaria | `[F1]` |
...
**Naturaleza del tributo (`naturaleza`):**

Declara el efecto de la línea calculada sobre el valor de la transacción:

- **`aditivo`**: suma al valor a pagar/cobrar (IVA, INC, ICA — la contraparte lo factura o la empresa lo cobra).
- **`sustractivo`**: resta del valor a pagar/cobrar (retenciones — una parte retiene a la otra). Incluye la retención a beneficiarios del exterior (`RETEFUENTE_EXTERIOR` en CO): Impuestos la liquida como cualquier retención; si la empresa la **asume** en lugar de descontarla al proveedor es una decisión del sub-dominio consumidor (OXP, `[D37]` de su modelo), no de este catálogo.
- **`provision`**: se reconoce sobre la transacción pero **no afecta el valor a pagar/cobrar ni los saldos** — genera únicamente el reconocimiento contable (un débito y un crédito). Es la naturaleza de las autorretenciones (la empresa se retiene sobre sus propios ingresos: el cliente paga completo) y de los autoliquidados (la empresa asume un impuesto que la contraparte no factura: el proveedor recibe el total).
...
- **`gasto`**: el tributo solo existe cuando la emisora es adquiriente. Aplica a tributos autoliquidados (IVA_IMPORTACION_SERVICIOS: el adquiriente asume el IVA al contratar servicios con proveedores sin domicilio fiscal en el país — tributo autónomo de naturaleza `provision`) y a la retención a beneficiarios del exterior (RETEFUENTE_EXTERIOR: la empresa residente retiene al proveedor sin domicilio fiscal en el país — tributo autónomo sustractivo, `[D17]`).
...
- **porcentajeDePadre:** La tarifa se calcula como un porcentaje del valor del tributo padre. El motor primero calcula el tributo padre, luego aplica la tarifa de este tributo sobre el resultado. Requiere `tributoPadre`. Si el padre no se causa en la transacción, el hijo se descarta con motivo `dependencia_padre` (`[R14]`) — por eso los tributos **autoliquidados** (donde la contraparte no factura el impuesto de referencia) se modelan como tributos **autónomos con tarifa propia sobre la base**, nunca como hijos del tributo que reemplazan: un padre que no se causa mataría al hijo justo en el escenario que lo justifica.
...
| `Efecto` | Resultado de la evaluación cuando el criterio se cumple: **`aplicar`** (activa un tributo que la matriz de tratamientos declara candidato pero que solo aplica con una calidad o situación — autorretenciones, autoliquidados, retención a beneficiarios del exterior); **`aplicar-default`** (condición sin criterios que declara que el tributo aplica si ninguna exclusión se cumplió); **`aplicar-sin-calidades`** (aplica aun cuando ninguna calidad del perfil lo active); **`noAplicar`** (el tributo se excluye del desglose con motivo `perfil_no_aplica`); **`cambiarTarifa`** (la tarifa resuelta en `TarifaTributaria` se reemplaza por la que indica `tarifaAlternativa`); **`reverseCharge`** (el tributo original `T` se reemplaza por el tributo alternativo `T'` declarado en `tributoSubstituto`, **siempre que `T'` sea aplicable en la dirección fiscal actual** — verificado contra `T'.direccionFiscalAplicable`; si `T'` no es aplicable, `T` continúa su evaluación normal). **Precedencia** cuando varias condiciones del mismo tributo se cumplen a la vez: `noAplicar` prevalece sobre `cambiarTarifa`, y `cambiarTarifa` sobre `aplicar` / `aplicar-default` / `aplicar-sin-calidades`. `[D17]` |
| `TarifaAlternativa` | Tarifa que reemplaza a la del stream cuando el efecto es `cambiarTarifa`. Dos formas: **fija** — un porcentaje escrito en la condición; **convenio** — la tarifa no está escrita en la condición: el motor la toma del convenio para evitar la doble imposición vigente con el país de residencia fiscal de la contraparte (`ConvenioDeDobleImposicion`, Sección 3.9): primero la tarifa convenida para el concepto de pago del concepto evaluado; si no existe, la tarifa general del convenio; si el convenio no define ninguna, la condición **no modifica la tarifa** y rige la de `TarifaTributaria`. Una tarifa convenida de **cero** excluye el tributo del desglose con motivo `convenio_exime`, porque el convenio reserva la potestad de gravar al otro país (coherente con `[R12]`). `[D17]` |
| `Origen` | Procedencia del contenido: `estándar` o `personalizado` (precedencia personalizado > estándar). |

**Operadores de criterio:**

Cada criterio de una `Condicion` compara el atributo evaluado con un operador. Son los mismos para todos los países; el contenido fiscal de cada país usa los que necesita.

| Operador | Cómo se cumple el criterio | `valorEsperado` |
|---|---|---|
| `igual` | El valor vigente del atributo evaluado coincide exactamente con el valor esperado. | Obligatorio. |
| `alguno-de` | Al menos uno de los atributos listados tiene el valor esperado (calidades alternativas, por ejemplo las calidades ICA de Colombia). | Obligatorio; el criterio lista varios atributos. |
| `no-nulo` | El atributo evaluado tiene un valor vigente a la fecha de la transacción, cualquiera que sea (inscripciones en regímenes: basta estar inscrito). | No lleva. |
| `con-convenio-vigente` | El país indicado por el atributo evaluado tiene, a la fecha de la transacción, un convenio para evitar la doble imposición vigente en el `ConvenioDeDobleImposicion` del país del catálogo (`convenioVigenteA`). Solo se admite sobre atributos cuyo dominio es el catálogo de países (`[I15]`). | No lleva. |
...
│  │   la realiza el MotorDeCalculo — ver Sección 3.15)_     │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                              │
│  ○ = Value Object (sin ID)                                   │
└──────────────────────────────────────────────────────────────┘
...
- **Creación (`[D16]`):** comando **`AsegurarPerfilTributario`** — **crea o reutiliza**: si no existe perfil para (identificación × país) lo crea; si existe, lo retorna. Es seguro repetirlo — invocarlo varias veces deja un solo perfil. Tres vías de invocación: el **flujo de captura con composición en el BFF** (al registrar al tercero en su dominio — ej: el Proveedor en OXP — el formulario ofrece capturar el perfil tributario en el mismo paso; el BFF compone y el comando viaja a Impuestos), la **administración directa** del administrador fiscal, y la **carga asistida** desde fuentes oficiales. **Nunca por eventos de Terceros**: el registro centralizado desapareció con el replanteamiento (#31) — no existe `TerceroActivado` ni ningún detonante externo.
...
| `IdentificacionFiscal` | NIT/RNC/EIN de la entidad, tipo de documento, país de emisión. Inmutable — identifica a la entidad ante la autoridad fiscal. **Se construye y valida con la pieza del paquete `IdentificacionLegal`** (validación local empaquetada: tipo para el país, formato, DV) — queda en la **misma clave natural** con la que la bodega de Terceros consolida (`[D16]`). |
...
**País de residencia fiscal de la contraparte (`[D17]`):** el atributo `paisDeResidenciaFiscal` (definido en el `CatalogoDeAtributosFiscales` del país con referencia al catálogo de países de Datos de Referencia) es un dato **declarado** del perfil, no derivado del país de la identificación — una sucursal extranjera con identificación local reside en el país, y una sociedad con identificación local puede residir fuera. Es obligatorio cuando la entidad no tiene domicilio fiscal en el país (`[I29]`): junto con el tipo de persona y `tieneDomicilioFiscalEnElPais = false` conforma el **perfil mínimo del proveedor del exterior**. El certificado de residencia fiscal que respalda el dato se registra en la `FuenteDeAutoridad` del atributo. Lo consumen las condiciones con operador `con-convenio-vigente` (Sección 3.4) para aplicar la tarifa de convenio de doble imposición.
...
### 3.9. Agregado: ConvenioDeDobleImposicion `[F1]`

- **Raíz:** Convenios para evitar la doble imposición que un país tiene vigentes con otros países. Es contenido fiscal del producto (`estándar`) ampliable por el cliente (`personalizado`, precedencia `[P3]`). El motor lo consulta cuando una condición evalúa el país de residencia fiscal de la contraparte con el operador `con-convenio-vigente`, o cuando un efecto `cambiarTarifa` toma su tarifa del convenio (`TarifaAlternativa` de forma **convenio**, Sección 3.4). `[D17]`
- **Ciclo de vida:** Configuración — sin FSM transaccional.
- **Stream de eventos:** `convenio-doble-imposicion-{pais}`
...
| Entidad | Descripción | Atributos |
|---|---|---|
| `Convenio` | Convenio para evitar la doble imposición suscrito por el país del catálogo con otro país. Declara con qué país aplica, el instrumento que lo aprobó, desde cuándo rige y las tarifas de retención que sustituyen a las generales por concepto de pago. Una tarifa convenida de **cero** significa que el convenio reserva la potestad de gravar al país del beneficiario: el tributo se excluye del desglose con motivo `convenio_exime` (coherente con `[R12]`). Si el convenio no trae tarifa para el concepto ni tarifa general, la condición no modifica la tarifa y rige la de `TarifaTributaria`. **Identidad:** combinación inmutable `(paisContraparte + origen + vigencia.fechaDesde)`. Para cambiar el país o la fecha de entrada en vigencia se cierra el convenio (`ConvenioCerrado`) y se agrega uno nuevo. Campos modificables vía `ConvenioModificado`: `nombre`, `instrumentoNormativo`, `tarifasConvenidas`, `vigencia.fechaHasta`. `[D17]` `[I28]` `[I30]` | paisContraparte (código ISO 3166-1 alfa-2 del catálogo de países de Datos de Referencia), nombre, instrumentoNormativo (ley aprobatoria o decreto de promulgación), tarifasConvenidas (lista de `TarifaConvenida`), vigencia (VO), origen (VO). |

**Value Objects:**

| Value Object | Contenido |
|---|---|
| `TarifaConvenida` | Tarifa de retención pactada en el convenio para un concepto de pago: conceptoPago (código del catálogo de conceptos de pago del país, o `*` para la tarifa general del convenio), tarifa (porcentaje; cero = el convenio exime). Dentro de un `Convenio` hay a lo sumo una tarifa por concepto y a lo sumo una general (`[I28]`). |
| `Vigencia` | Rango temporal de validez: fechaDesde, fechaHasta. |
| `Origen` | Procedencia del contenido: `estándar` o `personalizado` (precedencia personalizado > estándar). |
...
| Comportamiento | Descripción |
|---|---|
| `convenioVigenteA(paisContraparte, fecha)` | Retorna el convenio con ese país vigente a la fecha; si coexisten uno estándar y uno personalizado, retorna el personalizado. Es lo que resuelve el criterio `con-convenio-vigente`. |
| `tarifaConvenidaPara(paisContraparte, conceptoPago, fecha)` | Retorna la tarifa del convenio vigente para el concepto; si no existe, la tarifa general (`*`); si tampoco existe, no retorna tarifa (la condición no modifica la tarifa). Es lo que resuelve la `TarifaAlternativa` de forma convenio. |

**Diagrama de composición:**

```
┌──────────────────────────────────────────────────────────────┐
│  ConvenioDeDobleImposicion (raíz: país)                      │
│                                                              │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ Convenio                                               │  │
│  │  paisContraparte: ES · nombre: Convenio Colombia–España│  │
│  │  instrumentoNormativo: Ley 1082 de 2006                │  │
│  │  ○ TarifaConvenida { conceptoPago: *, tarifa: 10 }     │  │
│  │  ○ TarifaConvenida { conceptoPago: REGALIAS, tarifa: 10}│  │
│  │  ○ Vigencia { 2008-10-23 → ∞ } · origen: estándar      │  │
│  └────────────────────────────────────────────────────────┘  │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ Convenio                                               │  │
│  │  paisContraparte: GB · nombre: Convenio Colombia–R.U.  │  │
│  │  ○ TarifaConvenida { conceptoPago: *, tarifa: 0 }      │  │
│  │    → el convenio exime: descarte `convenio_exime`      │  │
│  │  ○ Vigencia { 2019-12-13 → ∞ } · origen: estándar      │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                              │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ Comportamiento calculado                               │  │
│  │  convenioVigenteA(pais, fecha)                         │  │
│  │  tarifaConvenidaPara(pais, conceptoPago, fecha)        │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                              │
│  ○ = Value Object (sin ID)                                   │
└──────────────────────────────────────────────────────────────┘
...
**Relación con `CondicionDeAplicacion` (Sección 3.4):** el catálogo no se referencia desde el perfil ni desde las tarifas del tributo: lo consulta el motor al evaluar una condición cuyo criterio usa el operador `con-convenio-vigente` sobre el atributo `paisDeResidenciaFiscal` de la contraparte, y al resolver una `TarifaAlternativa` de forma convenio. En Colombia lo usa la retención a beneficiarios del exterior (`RETEFUENTE_EXTERIOR`, condición `RTF-EXT-02`); en Panamá, la retención del ISR sobre pagos al exterior (`ISR-03-cdi`).

**Frontera con `CatalogoDeRegimenesEspeciales` (Sección 3.8) y `TarifaTributaria` (Sección 3.3):** un convenio no es un régimen especial — `[D13]` registra inscripciones de **empresas** ante una autoridad y aquí no hay empresa inscrita, sino un tratado entre Estados. Tampoco es una tarifa del tributo — `TarifaTributaria` indexa por un factor propio del tributo (concepto, clasificación, actividad) y aquí la tarifa depende del **país de la contraparte** y de su vigencia propia, común a cualquier tributo que la norma someta al convenio. Por eso es un agregado propio, con la misma forma de catálogo por país que los demás agregados de configuración.

Decisiones de diseño aplicadas: `[D1]` Raíz por país. `[D17]` Convenios de doble imposición como catálogo propio.

### 3.10. Agregado: RegistroTributario (ES) `[F1]`
...
│  │  totalProvisiones()                                    │  │
│  │    → sum(líneas provision) — informativo/contable     │  │
│  │  valorNeto(baseTransaccion: $1.400k)                   │  │
│  │    → $1.400k + $266k - $81k = $1.585k                 │  │
│  │      (las provisiones no entran al neto)               │  │
...
| `totalProvisiones()` | Suma del valor de todas las `LineaDeDesglose` con naturaleza `provision`. Solo informativo/contable — **nunca participa del valor neto**. |
| `valorNeto(baseTransaccion)` | `baseTransaccion` + `totalImpuestos()` - `totalRetenciones()`. La base la provee el contexto transaccional. Las líneas de naturaleza `provision` no suman ni restan: se reconocen sobre la transacción sin afectar el valor a pagar/cobrar. |
...
### 3.11. Agregado: HomologacionFiscal `[F2]`
...
- **Eventos propios:** 4 — ver Sección 5.2.9.
...
### 3.12. Agregado: FormatoFiscal `[F2]`
...
- **Eventos propios:** 5 — ver Sección 5.2.10.
...
### 3.13. Agregado: EntregableFiscal (ES) `[F2]`

- **Raíz:** Instancia concreta de un reporte fiscal generado para un período, autoridad y tipo específicos. Representa la ejecución de un `FormatoFiscal` sobre los `RegistroTributario` confirmados del período, traducidos mediante `HomologacionFiscal`. Es un documento compuesto — un solo archivo que contiene datos de múltiples terceros y registros. Cada generación crea un nuevo stream. No incluye certificados (propio agregado `CertificadoTributario`, 3.14) ni declaraciones tributarias (ver `[PD4]`). `[R26]` `[R27]`
...
### 3.14. Agregado: CertificadoTributario (ES) `[F2]`
...
### 3.15. Servicio de dominio: MotorDeCalculo `[F1]`
...
| `descartados[]` | Tributos evaluados pero excluidos. Misma estructura + `motivoExclusion` (cuantia_minima / perfil_no_aplica / clasificacion_excluida / jurisdiccion_no_aplica / dependencia_padre / actividad_no_registrada / tarifa_no_configurada / convenio_exime). Cada descartado también lleva su `jurisdiccionResuelta` específica. `[R19]` |
...
**Agregados que lee directamente:** CatalogoTributario, TarifaTributaria, CondicionDeAplicacion, PerfilTributario, JurisdiccionFiscal, CatalogoDeRegimenesEspeciales, ConvenioDeDobleImposicion.
...
```
  ...
  Tributario  Tributaria DeAplic.   Atributos   Tributario
                            ...
                            │ lee (Cambios 2 y 3; convenios)
                ┌───────────┼──────────────────┬──────────────────────┐
                ▼           │                  ▼                      ▼
        JurisdiccionFiscal  │        CatalogoDeRegimenes-    ConvenioDeDoble-
        [D12]               │        Especiales [D13]        Imposicion [D17]
```
...
### 3.16. Flujo orquestado: ConfirmacionTributaria `[F1]`
...
**Agregados involucrados:** MotorDeCalculo (lectura, solo gravámenes), RegistroTributario (lectura del origen en desgravámenes + escritura). **Lecturas transitivas vía MotorDeCalculo en gravámenes:** CatalogoTributario, TarifaTributaria, CondicionDeAplicacion, JurisdiccionFiscal, CatalogoDeRegimenesEspeciales, PerfilTributario (ver Sección 3.15 — "Agregados que lee directamente" del MotorDeCalculo).
...
### 3.17. Servicio de dominio: CargaAsistida `[F1]`
...
### 3.18. Read Model: CatalogoJurisdiccional `[F1]`
...
### 3.19. Resumen de contenido fiscal por país
...
| País | Impuestos | Retenciones | Provisiones (autorretenciones y autoliquidados) | Total |
|------|:---------:|:-----------:|:---------------:|:-----:|
| Colombia | 3 (IVA, INC, ICA) | 5 (RETEFUENTE, RETEFUENTE_EXTERIOR, RIVA, RICA, SOBRETASA_BOMBERIL) | 4 (AUTO_RENTA, AUTO_RETEFUENTE, AUTO_RICA + IVA_IMPORTACION_SERVICIOS autoliquidado) | **12** |
...
|------|:------------------------------------:|:-------------------:|:---------------------------:|:--------------------:|
| Colombia | 16 (RETEFUENTE) + 2 (RETEFUENTE_EXTERIOR, una por convenio) + 4 (RIVA) + 5 (RICA) + 3 (IVA) + 4 (autos y autoliquidado) — 34 en total | 3 (IVA, INC, ICA) | 4 (RETEFUENTE, ICA, RICA, AUTO_RETEFUENTE) | 4 (ICA, RICA, SOBRETASA_BOMBERIL, AUTO_RICA) |
...
| TarifaTributaria | 23 (8 nacionales + 15 municipales) | 5 | 4 |
| CondicionDeAplicacion | 1 | 1 | 1 |
| CatalogoDeAtributosFiscales | 1 | 1 | 1 |
| ConvenioDeDobleImposicion | 1 | — | 1 |
...
| **Total streams config** | **~38** | **~13** | **~9** |

---

## 4. Máquinas de estado

Dos agregados del bounded context tienen FSM transaccional: `EntregableFiscal` (reportes) y `CertificadoTributario` (certificados individuales). Los **10 agregados de configuración** tienen ciclo de vida sin FSM (crear, agregar/modificar/cerrar o desactivar entidades internas) — no usan transiciones de estado del agregado raíz. `RegistroTributario` nace como hecho confirmado sin transiciones `[D4]`.
...
| ConvenioDeDobleImposicion | Configuración | ConvenioDeDobleImposicionCreado, ConvenioAgregado, ConvenioModificado, ConvenioCerrado | 4 |
...
| 2 | `TributoAgregado` | Se registró un nuevo tributo en el catálogo. | Código, nombre, naturaleza (aditivo/sustractivo/provision), caracterRetención, nivelJurisdiccional, factorDeTarifa, direccionFiscalAplicable, tributoPadre (si aplica), origen. | `[R03]` |
...
> **Integración hacia la bodega de Terceros (`[D16]`):** los eventos del `PerfilTributario` se publican además como integración, identificados por la identificación legal — **enriquecen la ficha consolidada** del tercero (la bodega los lleva directamente a su proyección de ficha, `[D9]` de Terceros; no tocan sus agregados). Impuestos es **fuente** de la bodega, no su consumidor: no depende de ella para nada.
...
#### 5.2.8. ConvenioDeDobleImposicion — 4 eventos

| # | Evento | Descripción | Información capturada | Reglas |
|:---:|---|---|---|---|
| 1 | `ConvenioDeDobleImposicionCreado` | Se creó el catálogo de convenios para evitar la doble imposición de un país. | País, origen. | `[D17]` |
| 2 | `ConvenioAgregado` | Se registró un convenio con otro país. Valida que el país contraparte exista en el catálogo de países y que no se solape con otro convenio vigente del mismo país y origen. | **Atributos que identifican el convenio (inmutables):** paisContraparte, origen, vigencia.fechaDesde. **Atributos adicionales:** nombre, instrumentoNormativo, tarifasConvenidas (lista de conceptoPago o `*` + tarifa), vigencia.fechaHasta. | `[D17]` `[I28]` `[I30]` |
| 3 | `ConvenioModificado` | Se actualizaron atributos modificables de un convenio existente (por ejemplo, un protocolo que cambia una tarifa convenida). | **Identifican:** paisContraparte + origen + vigencia.fechaDesde. **Campos modificables:** nombre, instrumentoNormativo, tarifasConvenidas, vigencia.fechaHasta. Para cambiar el país o la fecha de entrada en vigencia se cierra el convenio y se agrega uno nuevo. | `[D17]` `[I28]` |
| 4 | `ConvenioCerrado` | Se cerró la vigencia de un convenio (denuncia o terminación). Sigue siendo válido para cálculos con fecha de transacción dentro de su rango. | **Identifican:** paisContraparte + origen + vigencia.fechaDesde. vigenciaHasta (fecha de cierre). | `[I28]` |

#### 5.2.9. HomologacionFiscal — 4 eventos
...
#### 5.2.10. FormatoFiscal — 5 eventos
...
| **Precondiciones** | Confirmación recibida del sub-dominio consumidor autorizado (OXP, CXC). Contexto transaccional completo: entidades fiscales, ubicaciones, conceptos con clasificación, desglose confirmado. Cálculo de referencia obtenido por `ConfirmacionTributaria` (3.16): motor (gravámenes) o prorrateo del registro origen (desgravámenes). `[R22]` |
...
El flujo orquestado `ConfirmacionTributaria` (Sección 3.16) emite este evento cuando la confirmación de una transacción no llega a crear el `RegistroTributario`. El evento de éxito del flujo es `RegistroTributarioCreado` (Sección 5.3.1), emitido por el agregado `RegistroTributario`.
...
| **Precondiciones** | Comando de confirmación recibido pero alguna validación falla en pasos 1-4 del flujo `ConfirmacionTributaria` (Sección 3.16). |
...
| I28 | **Unicidad de `Convenio` por país contraparte y de tarifa por concepto (local):** Dentro de un `ConvenioDeDobleImposicion`, no pueden coexistir dos `Convenio` con el mismo `paisContraparte` y `origen` cuyas vigencias se solapen; si coexisten uno estándar y uno personalizado para el mismo país, `convenioVigenteA()` retorna el personalizado (precedencia `[P3]`). Dentro de un `Convenio`, `tarifasConvenidas` contiene a lo sumo una `TarifaConvenida` por `conceptoPago` y a lo sumo una general (`*`). Verificación: el agregado valida ambas reglas como precondición de `ConvenioAgregado` y `ConvenioModificado`; garantiza que `tarifaConvenidaPara(pais, concepto, fecha)` resuelva de forma única. | ConvenioDeDobleImposicion | `[D17]` `[P5]` |
| I29 | **País de residencia fiscal obligatorio sin domicilio en el país (local):** En un `PerfilTributario`, si el atributo `tieneDomicilioFiscalEnElPais` vigente es `false`, debe existir un `paisDeResidenciaFiscal` vigente en la misma fecha. Verificación: precondición de `AtributoFiscalAgregado`, `AtributoFiscalModificado` y `AtributoFiscalCerrado` sobre cualquiera de los dos atributos; el motor rechaza la solicitud con `[R31]` cuando el perfil de la contraparte no la cumple. Aplica en los países cuyo catálogo de atributos define `tieneDomicilioFiscalEnElPais` (CO); en los demás el país de residencia se declara cuando se quiera aplicar un convenio. | PerfilTributario | `[D17]` |
| I30 | **Integridad referencial `paisContraparte` y `paisDeResidenciaFiscal` → catálogo de países de Datos de Referencia (eventual, entre bounded contexts):** ambos deben referenciar un código ISO 3166-1 alfa-2 activo del catálogo `paises` de Datos de Referencia. Verificación al escribir (`ConvenioAgregado`, `AtributoFiscalAgregado`/`Modificado`) con la copia local del catálogo de países; si un país se desactiva después, el dato histórico se conserva y la señalización es tardía, con el mismo mecanismo de `[I14]`. | ConvenioDeDobleImposicion, PerfilTributario, Datos de Referencia | `[D17]` `[I14]` |
...
**Aplica a:** RegistroTributario (3.10), MotorDeCalculo, EntregableFiscal.
...
**Decisión:** El MotorDeCalculo siempre opera stateless: calcula y retorna la evaluación completa sin crear ningún registro. La respuesta incluye dos conjuntos: tributos aplicados (desglose propuesto) y tributos descartados con motivo estructurado de exclusión (`cuantia_minima`, `perfil_no_aplica`, `clasificacion_excluida`, `jurisdiccion_no_aplica`, `dependencia_padre`, `actividad_no_registrada`, `tarifa_no_configurada`, `convenio_exime`). El consumidor presenta ambos conjuntos al usuario, quien puede excluir tributos propuestos o incluir tributos descartados. La creación del `RegistroTributario` ocurre únicamente cuando el consumidor envía el comando de confirmación — en ese momento Impuestos re-ejecuta el motor internamente para obtener el cálculo original y lo contrasta con el desglose confirmado.
...
**Aplica a:** MotorDeCalculo, RegistroTributario (3.10).
...
**Aplica a:** HomologacionFiscal (3.11), FormatoFiscal (3.12), EntregableFiscal.
...
**Aplica a:** CatalogoTributario (3.2), JurisdiccionFiscal (3.7), MotorDeCalculo (3.15), contrato semántico del consumidor (`[D9]`).
...
**Aplica a:** MotorDeCalculo, RegistroTributario (3.10), todos los sub-dominios consumidores.
...
**Aplica a:** JurisdiccionFiscal (3.7), TarifaTributaria (3.3), CondicionDeAplicacion (3.4), RegistroTributario (3.10), MotorDeCalculo (3.15), contrato semántico (`[D9]`).
...
**Aplica a:** `PerfilTributario` (3.6 — entidad `ActividadEconomicaRegistrada` y comportamiento `actividadEconomicaPara()`), `CatalogoDeAtributosFiscales` (3.5 — retiro de la cláusula obsoleta de la precarga estándar), `MotorDeCalculo` (3.15 — paso 2.d consulta `actividadEconomicaPara()` exclusivamente), `[PD9]` (plan de migración de perfiles).
...
### [D16] Detonante del PerfilTributario: captura idempotente, clave por identificación legal, fuente de la bodega

**Contexto:** El replanteamiento arquitectónico (#31) eliminó el registro centralizado de terceros: no existe `TerceroActivado` ni ningún evento de Terceros que pueda detonar la creación del perfil. La dependencia de la era anterior ("Gestión de Terceros" como servicio que se consulta para referenciar la identidad) desapareció con ella.

**Decisión:** (1) El `PerfilTributario` se crea mediante el comando **`AsegurarPerfilTributario`**, que **crea o reutiliza** por (identificación × país), invocado desde el flujo de captura con composición en el BFF, por la administración directa o por la carga asistida — ver Sección 3.6. (2) La `IdentificacionFiscal` se construye y valida con la pieza del paquete **`IdentificacionLegal`** — validación local empaquetada, misma clave natural de la bodega. (3) Los eventos del perfil se publican como **integración hacia la bodega de Terceros** para enriquecer la ficha consolidada — Impuestos es fuente de la bodega, nunca su consumidor.

**Justificación:** La transversalidad se resuelve con distribución (validación empaquetada + información por eventos), no con dependencia en ejecución — el mismo principio de todo el replanteamiento. El comando crea o reutiliza, como el `AsegurarProveedor` de OXP (`[D30]` de su modelo): si el perfil ya existe, lo retorna en lugar de fallar o duplicarlo — la captura continúa y solo existe un perfil por identificación × país. Es además seguro de repetir ante reintentos técnicos (la propiedad que los modelos del proyecto llaman *idempotencia*). La clave compartida con la bodega garantiza que el perfil del tercero y sus roles consoliden bajo la misma identidad sin traducción.

### [D17] Convenios para evitar la doble imposición como catálogo propio y retención a beneficiarios del exterior como tributo autónomo

**Contexto:** La retención en la fuente sobre pagos a beneficiarios sin residencia ni domicilio fiscal en el país no sigue las reglas de la retención doméstica: tiene tarifa propia por concepto (Colombia: 20 % general, arts. 406 a 408 ET), es el impuesto definitivo del beneficiario y se reduce o desaparece cuando el país de residencia fiscal del beneficiario tiene un convenio para evitar la doble imposición vigente con el país del catálogo. Hasta ahora el contenido fiscal de Colombia traía conceptos `EXTERIOR_*` dentro de la retención doméstica que ninguna condición disparaba, y Panamá declaraba una condición (`ISR-03-cdi`) con un criterio y un efecto que el modelo no definía. La lista de convenios y sus tarifas por concepto es un dato que cambia con vigencia propia, distinto de las tarifas del tributo y de los regímenes empresariales. La revisión con el contador de la empresa y las consultoras fiscales (sep-2026) confirmó la regla de negocio: 20 % general, tarifa reducida con convenio, y en la práctica la empresa asume la retención porque el proveedor del exterior recibe el total.

**Decisión:** **(1)** La retención a beneficiarios del exterior es un **tributo autónomo** del catálogo del país (`RETEFUENTE_EXTERIOR` en Colombia): naturaleza `sustractivo`, carácter `definitivo`, dirección `gasto`, factor `conceptoPago`, sin padre, con su propio stream de tarifas indexado por **los mismos códigos de concepto de pago** que la retención doméstica — el consumidor no cambia la asignación de sus conceptos según el proveedor. La retención doméstica queda excluida para la contraparte sin domicilio fiscal mediante una condición propia con el mismo criterio que activa la del exterior (exclusión mutua explícita, no `reverseCharge`). Los conceptos que no constituyen renta de fuente nacional (compras de bienes) no tienen entrada en el stream y el motor los descarta con motivo `tarifa_no_configurada`. **(2)** Los convenios viven en el agregado **`ConvenioDeDobleImposicion`** (raíz por país, `[D1]`), con una entidad `Convenio` por país contraparte y sus tarifas convenidas por concepto de pago. **(3)** `Condicion` incorpora el operador **`con-convenio-vigente`** sobre el atributo del perfil `paisDeResidenciaFiscal`, y `TarifaAlternativa` admite la forma **convenio**, que resuelve la tarifa desde el agregado (por concepto, luego general; cero exime con motivo `convenio_exime`). El VO `Efecto` formaliza además los efectos que el contenido fiscal ya usaba (`aplicar`, `aplicar-default`, `aplicar-sin-calidades`, `tributoSubstituto`) y la precedencia entre efectos. **(4)** El país de residencia fiscal es un atributo **declarado** del `PerfilTributario` (catálogo de países de Datos de Referencia), obligatorio cuando la entidad no tiene domicilio fiscal en el país (`[I29]`); su soporte documental se registra en `FuenteDeAutoridad`. **(5)** Impuestos **liquida** la retención; si la empresa la **asume** (el proveedor recibe el total) o la descuenta al beneficiario es una decisión del sub-dominio consumidor, que conoce el acuerdo con el proveedor — en OXP, la segunda causa de asunción de su decisión `[D37]` (asumida por defecto para beneficiarios del exterior, editable por documento). El motor no conoce la asunción y liquida sobre la base facturada.

**Justificación:** El tributo autónomo evita mezclar las reglas del exterior con las quince condiciones de la retención doméstica y sigue el precedente de `IVA_IMPORTACION_SERVICIOS` (v2.0.8). El catálogo propio sigue el patrón de `[D13]`: un dato con vigencia y autoridad propias no se embebe en tarifas ni en condiciones, se referencia. Se descartó modelar los convenios como un tipo de `CatalogoDeRegimenesEspeciales` porque ese catálogo registra inscripciones de empresas y no tiene forma para tarifas por concepto, y dentro de `TarifaTributaria` porque exigiría un factor bidimensional (concepto × país) en un agregado con un factor por tributo. Reutilizar `cambiarTarifa` en lugar de un efecto nuevo conserva el vocabulario del motor: lo que cambia es la fuente de la tarifa, no el efecto. Se descartó la naturaleza `provision` ("siempre asumida") porque la asunción no es obligación normativa sino práctica comercial, `[D37]` de OXP ya asignó esa decisión al consumidor, y el asiento resultante es el mismo que OXP produce hoy para la retención asumida por medio de pago. Se descartó un booleano declarado por proveedor ("amparado por convenio") porque replica a mano la lista oficial en cada perfil. Pendientes fiscales en `[PD13]`.

**Aplica a:** ConvenioDeDobleImposicion (3.9), CondicionDeAplicacion (3.4), CatalogoTributario (3.2), TarifaTributaria (3.3), CatalogoDeAtributosFiscales (3.5), PerfilTributario (3.6), MotorDeCalculo (3.15).
...
| PD13 | **Validación fiscal de la retención a beneficiarios del exterior y de los convenios de doble imposición (`[D17]`).** El contenido precargado de Colombia (`RETEFUENTE_EXTERIOR` al 20 %, convenios con tarifa general del 10 %) sigue la regla de negocio acordada con la consultoría; quedan por confirmar: (1) la lista oficial de convenios vigentes y sus fechas de aplicación; (2) la tarifa convenida **por concepto** de cada convenio (varios eximen los beneficios empresariales sin establecimiento permanente o limitan el 10 % a regalías y servicios técnicos) y el tratamiento de la Decisión 578 de la Comunidad Andina; (3) los conceptos con tarifa distinta del 20 % y la tarifa del 35 % para jurisdicciones no cooperantes (requeriría una segunda lista de países y una condición propia); (4) cuando la empresa asume la retención, si la base se reajusta como mayor valor del pago y si el gasto de la retención asumida es deducible — afectaría la aritmética del consumidor que asume, no la del motor; (5) si aplicar el convenio exige certificado de residencia fiscal (`FuenteDeAutoridad`); (6) las condiciones `RTF-04a/b` (proveedor no responsable de IVA) y `RICA-02` frente al proveedor extranjero. Para Panamá, las tarifas convenidas de los 17 convenios están vacías hasta la validación. | Contenido fiscal CO/PA; Secciones 3.2, 3.3, 3.4, 3.9. | Al recibir la validación de la consultoría fiscal; cada respuesta se aplica como refinamiento de los precargados sin cambio estructural. |
...
|---------|-------|-------------|
| 2.1.0 | Septiembre 2026 | **Retención a beneficiarios del exterior y convenios para evitar la doble imposición (issue #143; cierra el #138 del equipo de desarrollo).** Revisión con el contador de la empresa y las consultoras fiscales: la retención a proveedores sin domicilio fiscal en el país (20 % general, 10 % con convenio) no dependía de ninguna calidad del perfil y el diseño no la ejecutaba (conceptos `EXTERIOR_*` sin disparador dentro de RETEFUENTE; `RTF-04a` apagaba la retención doméstica por accidente; sin país de residencia ni lista de convenios). **Cambios estructurales:** **(1)** nuevo agregado **`ConvenioDeDobleImposicion`** (Sección 3.9 nueva; raíz por país; entidad `Convenio` con `TarifaConvenida` por concepto; 4 eventos en 5.2.8; `[I28]`, `[I30]`); **(2)** `CondicionDeAplicacion`: tabla de **operadores de criterio** formalizada (`igual`, `alguno-de`, `no-nulo` y el nuevo **`con-convenio-vigente`**), VO `Efecto` completado con los efectos que el contenido ya usaba y su **precedencia**, nuevo VO **`TarifaAlternativa`** de forma fija o **convenio**; `[I15]` ampliada; **(3)** `PerfilTributario`: atributo declarado `paisDeResidenciaFiscal`, obligatorio sin domicilio fiscal (`[I29]`); **(4)** motor (3.15): evaluación del convenio en 2.c, tarifa por convenio y descarte `tarifa_no_configurada` en 2.d, motivos `convenio_exime`/`tarifa_no_configurada`, lee el agregado nuevo; **(5)** nueva decisión **`[D17]`** (tributo autónomo sustractivo `RETEFUENTE_EXTERIOR`, exclusión mutua explícita, asunción decidida por el consumidor — `[D37]` de OXP; descartadas la naturaleza `provision`, el tipo de régimen especial y el booleano por proveedor); pendiente **`[PD13]`**. Conteos: 10 agregados de configuración, 61 eventos (13 agregados con eventos), 31 invariantes (I1-I10 + I11a/I11b + I12-I30), 17 decisiones, 13 pendientes; Colombia 12 tributos. **Renumeración de secciones** (sin saltos): 3.9-3.18 → 3.10-3.19 (RegistroTributario 3.10, HomologacionFiscal 3.11, FormatoFiscal 3.12, EntregableFiscal 3.13, CertificadoTributario 3.14, MotorDeCalculo 3.15, ConfirmacionTributaria 3.16, CargaAsistida 3.17, CatalogoJurisdiccional 3.18, Resumen por país 3.19) y 5.2.8-5.2.9 → 5.2.9-5.2.10; las filas anteriores de este historial conservan la numeración de su época. **Contenido fiscal:** catálogo tributario CO v1.5, tarifas CO v1.4, condiciones CO v1.3, atributos CO v1.3, homologación CO v1.2, condiciones PA v1.1, atributos PA v1.1, catálogos nuevos `co-/pa-convenio-de-doble-imposicion` v1.0; anexos CO y PA. Acompaña al alcance v1.6 (glosario 27-28, R10 ampliada, R17-R18 ajustadas, R42 nueva), a OXP v4.16 y a las plantillas de Contabilidad v1.12. |
...
| 2.0.8 | Julio 2026 | **Naturaleza `provision` y autoliquidados autónomos (issues #117 y #118, sobre la resolución del #110).** El equipo de desarrollo demostró que el autoliquidado modelado como hijo del IVA (`porcentajeDePadre` al 100%) se descartaba a sí mismo: el proveedor del exterior no factura IVA → el padre no entra al resultado → `[R14]` mata al hijo justo en el único escenario que lo justifica. Resolución (tres definiciones del usuario): **(1)** el IVA asumido no es el mismo IVA de la compra — código propio con reglas propias, sin arrastrar las reglas e hijos del IVA real (la lectura provisional de la implementación de "causar el IVA" queda superada); **(2)** el autoliquidado no tiene padre ni hijos — `IVA_IMPORTACION_SERVICIOS` pasa a tributo **autónomo** con `factorDeTarifa: clasificacion` y tarifas espejo del IVA sobre la base; **(3)** la acotación a servicios (el disparador alcanzaba bienes importados cuyo IVA recauda la aduana) se resuelve con los mecanismos existentes: clasificaciones nuevas `SERVICIOS_GRAV_19`/`SERVICIOS_GRAV_5` (el consumidor clasifica por lo que ES el concepto, no por quién lo vende) + matriz de tratamientos — sin ámbitos nuevos en `Condicion`. **Naturaleza `provision` formalizada** (tercera naturaleza en `Tributo`, `LineaDeDesglose`, `TributoAgregado`): se reconoce sobre la transacción sin afectar el valor a pagar/cobrar ni los saldos — aplica al autoliquidado y corrige las tres autorretenciones (`AUTO_RENTA`/`AUTO_RETEFUENTE`/`AUTO_RICA`, antes `sustractivo`); nueva operación `totalProvisiones()`; `valorNeto()` documenta que las provisiones no participan. Nota en `porcentajeDePadre` (por qué los autoliquidados nunca se modelan como hijos). La condición `RIVA-03` (sustitución `reverseCharge`) se retira de los precargados CO — cada tributo se activa por sus propias condiciones; el efecto `reverseCharge` permanece en el modelo como mecanismo disponible. El medio de pago NO entra al motor (ratifica la decisión del #94): el caso del emisor de tarjeta que retiene el IVA de servicios digitales (437-2 num. 8) se maneja en OXP con el patrón de concurrencia existente. Catálogo CO v1.4, condiciones v1.2, tarifas v1.2, atributos v1.2. |
| 2.0.7 | Julio 2026 | **Renombre `AUTO_RIVA` → `IVA_IMPORTACION_SERVICIOS` (issue #110, resolución con consultoría fiscal jul-2026).** La "autorretención de IVA en ventas" heredada del sistema de facturación no tiene sustento normativo (no existe autorretenedor de rete-IVA) y se remueve como concepto; el tributo queda definido como la **autoliquidación del IVA en importación de servicios** (art. 437-2 num. 3 ET): el adquiriente asume el 100% del IVA teórico cuando el proveedor de servicios no tiene residencia ni domicilio fiscal en el país — el proveedor recibe el total de la factura. Disparador corregido en los precargados: de `emisora.esAgenteRetenedorIVA` (residuo legado que chocaba con `RIVA-01b`) a `contraparte.tieneDomicilioFiscalEnElPais = false` (atributo nuevo, catálogo de atributos v1.1). Ventana pre-producción: sin registros históricos, no aplica el patrón de sustitución de código. Precargados: catálogo CO v1.2 (incluye tratamientos `AUTO_RETEFUENTE` × 4 del issue #111), condiciones v1.1, tarifas v1.1, anexos actualizados. Pendientes con consultores: matriz de tratamientos del autoliquidado, restricción a responsables de IVA, perfil mínimo del proveedor del exterior (preguntas 6-8 del catálogo). |
| 2.0.6 | Julio 2026 | **Residuo del issue #93 (ICA solo en dirección Ingreso).** La sección de direccionalidad inherente (§ `Tributo`) todavía listaba ICA entre los tributos bidireccionales cuyo "comportamiento específico se modela vía condiciones" — regla incumplible: no existe condición que responda "¿cuándo un comprador paga ICA?" porque la respuesta normativa es nunca, y eso es lo que codifica `direccionFiscalAplicable: ingreso`. ICA sale de la lista de `ambas` y se documenta en `ingreso` junto a las autorretenciones (en gasto el comprador solo practica RICA). Alinea el modelo con el catálogo CO v1.1 y el anexo de dirección fiscal, corregidos desde el PR #99. |
| 2.0.5 | Junio 2026 | **Detonante del PerfilTributario sin registro centralizado de terceros (replanteamiento #31, issue #39).** Nueva decisión `[D16]`: creación por el comando `AsegurarPerfilTributario`, que crea o reutiliza por identificación × país (captura con composición en BFF, administración directa o carga asistida — nunca por eventos de Terceros); `IdentificacionFiscal` construida y validada con la pieza del paquete `IdentificacionLegal` (misma clave natural de la bodega); los eventos del perfil se publican como integración hacia la bodega de Terceros (enriquecen la ficha consolidada — Impuestos es fuente, no consumidor). Sección 3.6 con la vía de creación; nota de integración en 5.2.5. Acompaña al alcance v1.5. |
