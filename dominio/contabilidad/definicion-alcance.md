...
| 2 | **Línea de traducción** | N1 | Unidad mínima de información que un sub-dominio transaccional emite para ser traducida a contabilidad. Contiene el tipo de componente, el contexto de negocio (clasificación, tercero, empresa, naturaleza fiscal) y el valor ya distribuido por unidad organizacional. Contrato estandarizado entre todos los sub-dominios y N1. La contrapartida también viaja como línea — es la única cuyo valor no envía el consumidor (lo calcula el motor para completar el balance). |
...
| 13 | **Contrapartida** | N1 | Partida del borrador que completa el balance para que la ecuación débito = crédito se cumpla. El sub-dominio consumidor la emite como una línea de traducción más — con su tercero y su clasificación, pero **sin valor ni unidad organizacional**: el valor lo calcula el motor (diferencia entre débitos y créditos) y la unidad organizacional se asigna según la preferencia de la empresa [R50]. Los consumidores siguen sin conocer cuentas contables ni naturalezas débito/crédito — eso lo determina la plantilla de asiento. |
...
| 23 | **Cadena de resolución** | N1 | Proceso de tres niveles que determina la cuenta auxiliar para cada partida del borrador: Nivel A (regla manual del analista contable), Nivel C (aprendizaje acumulado del sistema) y Nivel B (inferencia analizando el plan de cuentas). Se evalúan en orden de precedencia: A primero, luego C, luego B. Los tres niveles trabajan sobre el mismo dato recibido: la clasificación de la línea, que A y C emparejan por similitud contra sus textos registrados y B compara contra las descripciones de las cuentas. Los componentes que representan la contraparte de un hecho anterior no pasan por la cadena — se resuelven por resolución por espejo [R53]. |
| 24 | **Aprendizaje del sistema** | N1 | Registro acumulado de las resoluciones de cuentas que el contador ha tomado al completar borradores pendientes. Cada resolución asocia las dimensiones estables (tipo de transacción, tipo de componente, empresa) y el texto de clasificación de la línea resuelta con la cuenta auxiliar elegida. Al llegar una línea nueva, el sistema busca dentro de la misma combinación de dimensiones estables y empareja la clasificación **por similitud** contra los textos aprendidos. Corresponde al Nivel C de la cadena de resolución. |
...
| 31 | **Clasificación** | N1 | Texto semántico que viaja en cada línea de traducción y del que depende la resolución confiable de la cuenta. El consumidor lo compone **mecánicamente** por tipo de componente a partir de datos de sus catálogos (ej. en OXP, para el gasto: descripción del concepto + concepto de pago + clasificación tributaria) — no lo digita un usuario, no es un código ni una llave. Es obligatorio en toda línea [R52]. |
| 32 | **Resolución por espejo** | N1 | Mecanismo por el cual una partida que representa la contraparte contable de un hecho económico anterior (cruce de obligación, aclaración de partida, amortización o reversa de anticipo, nota crédito) toma la **misma cuenta** que usó ese hecho, ubicándola a través de la referencia al hecho relacionado que trae la línea. Garantiza que el cruce contable salde. Si el hecho relacionado no está (ej. saldos migrados), el borrador queda pendiente para el contador — nunca se adivina la cuenta. [R53] |
...
| **Sub-dominio de Terceros** | **Bodega consolidadora** de los terceros que los dominios operativos capturan con sus roles (v2.0, replanteamiento jun-2026). Publica la señal global (Activo/Inactivo) y el mapa canónico de fusiones. | N1 consume su señal global (**copia local** — sin consulta en caliente) para validar terceros activos al crear borradores, y su mapa canónico para presentar los reportes por tercero (R51). El tercero de cada transacción llega **embebido** en las líneas de traducción. |
| **Sub-dominio de Estructura Organizacional** | **Dueño único** de las unidades organizacionales: las gobierna y publica sus eventos de ciclo de vida (creada, activada, suspendida, reactivada, inactivada, reabierta, fusionada, dividida, trasladada). **Nunca es prerrequisito** — N1 no la consulta en caliente. | N1 mantiene **copia local** de las unidades por suscripción a esos eventos y valida la unidad activa contra ella (R07). La unidad de cada transacción llega **embebida** en las líneas de traducción. |
...
3. El **motor de traducción** recibe las líneas y ejecuta la traducción:
   ...
   b. Para cada rol de la plantilla, resuelve la **cuenta auxiliar** del plan de cuentas. Si la partida representa la contraparte de un hecho económico anterior, la resuelve por **resolución por espejo** — copia la cuenta que usó ese hecho [R53]. Para las demás, aplica la cadena de resolución: primero busca una regla de derivación configurada por el analista contable; si no la encuentra, busca en el aprendizaje acumulado de resoluciones previas; si tampoco, infiere la cuenta más probable analizando el plan de cuentas. En los tres niveles, la clasificación que trae la línea es el insumo del emparejamiento [R52].
   c. Completa la **contrapartida** con el valor que balancea el asiento (suma de débitos = suma de créditos) — el tercero y la clasificación de la contrapartida los envía el consumidor en su propia línea [R54].
...
3. Cada decisión del contador **alimenta el aprendizaje** del sistema: cuando llegue una línea de la misma combinación de dimensiones estables con clasificación igual o suficientemente similar, el sistema resolverá automáticamente. El analista contable puede convertir un aprendizaje en una regla formal de derivación cuando quiere que sea explícita e inmutable. Las partidas resueltas por espejo no alimentan el aprendizaje [R53].
...
| 2. Traducción y resolución | Comportamiento de la traducción, resolución, re-emisión, gobernanza del aprendizaje, entrega y reportes por tercero (N1) | R11–R15, R35–R36, R50–R51 |
...
| R07 | **Datos maestros activos:** Las cuentas auxiliares, los terceros y las unidades organizacionales tienen un estado activo o inactivo. Un borrador solo puede usar datos maestros en estado activo. Para los terceros, la condición se evalúa contra la **copia local de la señal global** que publica la bodega de Terceros; para las unidades organizacionales, contra la **copia local de los eventos de ciclo de vida** que publica el sub-dominio de Estructura Organizacional (en ambos casos: suscripción — sin consulta en caliente; replanteamiento jun-2026). Los asientos contables ya generados con datos que posteriormente pasaron a inactivo se conservan intactos — la inactivación no afecta el histórico, solo impide su uso en nuevos registros. **Reestructuración de unidades:** cuando una unidad se fusiona, divide o traslada en Estructura Organizacional, los asientos ya generados permanecen inmutables con la unidad de su momento; presentar reportes por la estructura vigente (consolidando reestructuraciones) es función de la capa de reportería, que aplica el mapa de reestructuración de Estructura Organizacional al leer — sin reescribir asientos. | No |
...
| R12 | **Aprendizaje progresivo:** Cada vez que el contador resuelve una cuenta en un borrador pendiente, el sistema aprende esa decisión junto con la clasificación de la línea. En el futuro, cuando llegue una línea de la misma combinación de dimensiones estables (mismo tipo de transacción, mismo tipo de componente, misma empresa) con una clasificación **igual o suficientemente similar**, el sistema resolverá automáticamente sin intervención — la clasificación idéntica (compra repetida) siempre resuelve; la similitud insuficiente pasa al siguiente nivel de la cadena. El analista contable puede convertir un aprendizaje en una regla formal de derivación cuando quiere que sea explícita e inmutable. | No |
...
| R45 | **Validación contractual del motor de traducción:** El motor valida que cada línea de traducción tenga un rol disponible en la plantilla del tipo de transacción recibido. Si una o más líneas no encajan, el motor rechaza el hecho económico completo y no crea borrador. Estos rechazos también ocurren cuando el tipo de transacción no tiene plantilla configurada, cuando se recibe una referencia de origen duplicada que ya no es reemplazable, cuando alguna línea llega sin clasificación [R52] o cuando falta la línea de contrapartida que la plantilla exige [R54]. En todos estos casos, el motor notifica al consumidor con un motivo estructurado para que decida la acción correctiva (re-emitir corregido, escalar al equipo de producto si el motivo refleja un defecto de configuración del producto, o reconocer un duplicado en caso de idempotencia). Estos rechazos no son hechos contables, no aparecen en la consola de contabilización y no requieren intervención del contador. La durabilidad del hecho económico hasta que sea procesado correctamente es responsabilidad del sub-dominio consumidor emisor. | No |
...
| R50 | **Unidad organizacional de la cuenta por pagar (contrapartida):** La unidad organizacional de la partida de cuenta por pagar (contrapartida) se maneja según la preferencia de cada empresa: (a) distribuida igual que el gasto que origina la obligación, (b) consolidada en una unidad organizacional general, o (c) sin unidad organizacional. Aplica a toda contrapartida tipo cuenta por pagar (causación de gasto, nota crédito de gasto, anticipo a proveedor). Cuando el plan de cuentas exige unidad organizacional en la cuenta, la opción sin unidad no aplica. El producto ofrece la opción; no impone un único comportamiento. | Sí |
| R51 | **Reportes por tercero canónico:** Los auxiliares por tercero, la información exógena y los certificados de retención se presentan por el **tercero canónico**, aplicando el mapa de fusiones que publica la bodega de Terceros **al momento de leer** — los asientos contables permanecen inmutables con el tercero embebido de su momento. Cuando la bodega fusiona dos terceros duplicados (ej: el mismo proveedor registrado con CC y con NIT), los saldos y reportes se consolidan bajo la identificación canónica sin reescribir ningún registro. | No |
| R52 | **Clasificación obligatoria en toda línea:** Toda línea de traducción debe traer su texto de clasificación, compuesto mecánicamente por el consumidor a partir de datos de sus catálogos según el tipo de componente. Es el insumo del emparejamiento en los tres niveles de la cadena de resolución: las reglas y el aprendizaje lo comparan por similitud contra sus textos registrados (dentro de la misma combinación de dimensiones estables) y la inferencia lo compara contra las descripciones de las cuentas del plan. Sin clasificación no hay resolución confiable — un grupo del plan de cuentas admite muchas cuentas válidas. Una línea sin clasificación rechaza el hecho económico completo [R45]. | No |
| R53 | **Resolución por espejo del hecho relacionado:** Las partidas que representan la contraparte contable de un hecho económico anterior (cruce de obligación del extracto, aclaración de una partida en disputa, amortización o reversa de anticipo, y todas las partidas de una nota crédito) toman la **misma cuenta** que usó ese hecho — el motor la copia del registro del hecho relacionado usando la referencia que trae la línea. La plantilla de asiento define qué componentes se resuelven por espejo y qué rol espejan; la naturaleza débito/crédito la da la plantilla del hecho nuevo (normalmente inversa a la original). El espejo tiene precedencia sobre la cadena de resolución, no alimenta el aprendizaje, y si el hecho relacionado no existe en Contabilidad (ej. saldos migrados de un sistema anterior) el borrador queda pendiente para que el contador asigne — nunca se resuelve por similitud, porque una cuenta distinta rompería el cruce. | No |
| R54 | **La contrapartida la emite el consumidor como línea:** El sub-dominio consumidor envía la contrapartida como una línea de traducción más, con su tercero (ej. el proveedor en la causación, el banco/emisor en el extracto) y su clasificación (ej. medio de pago + observación general de la compra), pero sin valor ni unidad organizacional: el valor lo calcula el motor para completar el balance y la unidad organizacional se asigna según la preferencia de la empresa [R50]. Así Contabilidad no conoce los campos internos de cada consumidor y cada consumidor incorpora los atributos propios con los que compone su clasificación. Si la plantilla del tipo de transacción tiene rol de contrapartida y la línea no llega, el hecho económico se rechaza completo [R45]. | No |
...
| Gestión de terceros | Creación, actualización e inactivación de terceros (proveedores, clientes, empleados). | La captura vive en los dominios operativos con sus roles (el Proveedor en OXP, etc.); la bodega de Terceros consolida (replanteamiento jun-2026). N1 recibe el tercero **embebido** en las líneas de traducción — como siempre lo hizo. |
...
|-------------|-------------|------------------------|
| **Sub-dominio de Terceros** | Bodega consolidadora (v2.0): publica la señal global del tercero y el mapa canónico de fusiones. **Nunca es prerrequisito** — N1 no la consulta en caliente. | N1 mantiene copia local de la señal para R07 y aplica el mapa canónico en los reportes por tercero (R51). |
| **Sub-dominio de Estructura Organizacional** | **Dueño único** de las unidades organizacionales, su jerarquía, tipos y estado activo/inactivo: las gobierna y publica sus eventos de ciclo de vida. **Nunca es prerrequisito** — N1 no la consulta en caliente. | N1 mantiene copia local de las unidades por suscripción a esos eventos para validar la unidad activa (R07); la unidad llega embebida en las líneas de traducción. Los asientos quedan inmutables ante reestructuración de unidades (R07). |
...
| **Plataforma: Datos base** | Países, monedas y tipos de documento — **piezas del paquete del producto** (custodiadas por Datos de Referencia; replanteamiento jun-2026): viajan incluidas en el despliegue, sin servicio en ejecución. | N1 valida moneda (para el borrador) y tipos de documento (identificación de terceros en las partidas) **localmente**, con las mismas reglas de todo el ERP. |
...
| 1.7 | Junio 2026 | Política configurable por empresa para la unidad organizacional de la cuenta por pagar (contrapartida) — issue #17. Surgió al modelar el cruce de la OXP del extracto (issue #18). Cambios: (1) Nueva regla R50 en el frente "Traducción y resolución (N1)" — la unidad organizacional de la contrapartida tipo cuenta por pagar se maneja según la preferencia de cada empresa (distribuida como el gasto / consolidada en una unidad general / sin unidad organizacional), respetando la obligatoriedad del plan de cuentas; el producto ofrece la opción, no impone un comportamiento. Total ahora 50 reglas. Acompaña actualización de `modelo-dominio.md` v1.6 (nueva invariante I33 configurable por empresa, ajuste del paso 4 del ServicioDeTraduccion, nota en el asiento de ejemplo). El mecanismo de almacenamiento/resolución de la preferencia queda a criterio de implementación. |
| 1.8 | Junio 2026 | **Ajuste cruzado por Terceros v2.0 (replanteamiento #31, issue #37).** Terceros dejó de ser "fuente de verdad que valida": es la **bodega consolidadora** de los terceros que los dominios capturan con sus roles. Cambios: actor y dependencia de Terceros redefinidos (señal global por suscripción con copia local + mapa canónico de fusiones — nunca consulta en caliente; el tercero llega embebido en las líneas, como el modelo as-built ya lo hacía); R07 precisa que los terceros se validan contra la copia local de la señal; nueva **R51** — los reportes por tercero (auxiliares, exógena, certificados) se presentan por el tercero canónico aplicando el mapa de fusiones al leer, con asientos inmutables; fila "Plataforma: Datos base" actualizada al modelo de piezas del paquete (validación local, sin servicio). Total ahora 51 reglas. Cierra el [PD4] del modelo de Terceros. |
| 1.9 | Junio 2026 | **Estructura Organizacional: de "fuente de verdad que valida" a copia local por suscripción (replanteamiento #45, issue #47 — gemelo del #37).** EO deja de tratarse como un maestro que se consulta: es el **dueño único** de las unidades y publica sus eventos de ciclo de vida; N1 mantiene **copia local** por suscripción y valida la unidad activa contra ella (R07) — sin consulta en caliente; la unidad llega **embebida** en las líneas de traducción (como el modelo as-built ya lo hacía). Cambios: actor y dependencia de Estructura Organizacional redefinidos; **R07** ampliada (las unidades se validan contra la copia local de los eventos de EO, junto a los terceros) y con **nota de reestructuración** (los asientos quedan inmutables con la unidad de su momento; presentar reportes por la estructura vigente es función de la capa de reportería que aplica el mapa de reestructuración al leer — **no** se modela como regla nueva de Contabilidad, a diferencia del tercero canónico R51, porque la reestructuración de unidades es un hecho de negocio, no una corrección de duplicados). Sin reglas nuevas (sigue en 51). Acompaña el ajuste del modelo (`I7b` precisada a copia local — cierra de paso el cabo que el #37 dejó al no tocar esa invariante). |
| 1.10 | Junio 2026 | **Consistencia del modelo de comunicación con EO (issue #56).** Se añade `reactivada` (transición `Suspendida`→`Activa`) a la enumeración de eventos de ciclo de vida de unidades que Contabilidad consume por copia local (actor y dependencia de Estructura Organizacional): faltaba, y sin ella una unidad reactivada desde suspensión no se reflejaría en la copia local para `R07`. El replanteamiento de `R25`/`I08` y el retiro de la proyección de imputación (`[SI10]`) vive en EO; Contabilidad nunca tuvo proyección de imputación, así que no se ve afectada por ese frente. Sin cambios en el conteo de reglas (51). |
| 1.11 | Julio 2026 | **Clasificación semántica, contrapartida como línea y resolución por espejo (issue #104).** Cierra la zona gris sobre el campo de clasificación de la línea de traducción, que tenía confundido al equipo de desarrollo (¿texto abierto o valor normalizado?). **Definición:** la clasificación es **texto semántico** que cada consumidor compone mecánicamente por tipo de componente desde sus catálogos (no la digita un usuario, no es código ni llave) y es **obligatoria en toda línea** — es el insumo del emparejamiento en las tres capas de la cadena de resolución: reglas y aprendizaje la comparan **por similitud** dentro de la combinación de dimensiones estables (tipo de transacción, tipo de componente, empresa); la inferencia la compara contra las descripciones de las cuentas. **Reglas nuevas:** [R52] (clasificación obligatoria), [R53] (resolución por espejo del hecho relacionado — cruces, aclaraciones, amortizaciones/reversas de anticipo y notas crédito toman la misma cuenta del hecho original; precede a la cadena y no alimenta el aprendizaje) y [R54] (la contrapartida la emite el consumidor como línea con tercero y clasificación, sin valor ni unidad organizacional — el motor balancea y aplica [R50]). **Ajustadas:** [R12] (aprendizaje por similitud suficiente; la clasificación idéntica siempre resuelve), [R45] (dos rechazos nuevos: línea sin clasificación, contrapartida faltante). **Glosario:** términos 2, 13, 23 y 24 reescritos; términos nuevos 31 "Clasificación" y 32 "Resolución por espejo". Flujos 2 y 4 de la Sección 4 ajustados. Corregido de paso un defecto de formato que separaba a R51 de su tabla. Conteo de reglas: 51 → **54**. Acompaña al modelo v1.12 ([D15], [I34]/[I35], [SI8]) y a los cambios cruzados en el catálogo de plantillas v1.9, el anexo de ejemplos v1.5 y el modelo de OXP v4.8. |
