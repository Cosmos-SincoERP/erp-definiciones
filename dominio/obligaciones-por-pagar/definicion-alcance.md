...
| **OXP de Comercio** | Obligación por pagar originada por un hecho económico de egreso (compra, gasto o costo). Independiente del medio de pago: puede originarse en compra con tarjeta (crédito o débito), factura de proveedor a crédito, pago de contado u otro medio. La forma de pago **casi nunca puede deducirse del soporte** — se resuelve al radicar como el **tipo** de medio de pago (con tarjeta o no) por la cadena de resolución `[R44]`, sin identificar la tarjeta concreta: esa vinculación queda establecida después, en la conciliación, por la partida del extracto donde aparece el consumo. El tipo capturado determina el flujo de resolución financiera dentro del sistema (vía extracto bancario o pago directo desde Tesorería) y es una de las dos causas por las que una retención queda asumida (R45). |
...
| **Extracto Bancario** | Documento emitido por la entidad financiera que detalla las transacciones y cargos de una **tarjeta (producto financiero)** en un período determinado. Algunas entidades ofrecen además un **extracto consolidado** de una cuenta que agrupa varias tarjetas; en esta fase la conciliación se maneja **por tarjeta** — un archivo consolidado no se radica como deuda `[R43]`. Puede contener transacciones en múltiples monedas (ej: tarjetas con facturación segmentada por moneda). Cuando un extracto contiene partidas en monedas mixtas, las partidas en moneda extranjera se convierten a moneda funcional al momento de la radicación `[R05d]`. Si todas las partidas están en una sola moneda (sea COP, USD u otra), el extracto opera en esa moneda directamente. |
| **Producto Financiero** | Registro maestro, definido por el usuario, de cada **tarjeta** de la empresa sobre la que se reciben extractos (crédito o débito prepago). Es la fuente del emisor (tercero entidad financiera) y de la configuración por tarjeta: cargos adicionales que maneja y plazo de alerta de conciliación. *(El período de facturación y la fecha de pago son datos de cada extracto, no del producto; las cuentas del ajuste por tolerancia las resuelve la traducción contable `[R10]`.)* Puede llevar la **identificación de la cuenta consolidadora** a la que la tarjeta está asociada/amparada — un solo dato opcional cuyo mismo valor **agrupa** las tarjetas de una misma cuenta: alimenta el diagnóstico de la salvaguarda de consolidado `[R43]` y la advertencia al registrar un producto cuyo número coincida con una cuenta anotada `[R39]`. La conciliación y la deuda se manejan siempre **por tarjeta**. La empresa registra sus productos antes de radicar extractos — el sistema no adivina el comportamiento de cada entidad financiera: verifica contra lo que el usuario definió `[R39]`. *(Evolución anotada en el issue #106: soporte del extracto consolidado de una cuenta como fuente de deuda.)* |
| **Medio de Pago** | Forma en que la empresa paga una obligación. Tipos canónicos: **tarjeta de crédito, tarjeta débito, tarjeta débito prepago, transferencia, efectivo, cheque, otro** — acompañados del **número que identifica el instrumento**: en los tipos tarjeta es la identidad (el número del producto financiero registrado); en los demás es opcional (el número del cheque, la referencia de la transferencia). La **entidad bancaria no hace parte del medio de pago**: es el emisor del producto financiero registrado. En la OXP de Comercio el medio de pago se resuelve por la **cadena de resolución** `[R44]`, con su origen rastreado — cuando la tarjeta se resuelve contra el registro, la compra la **referencia** como producto financiero. La OXP de Extracto **no lleva medio de pago**: es la deuda de la tarjeta y se paga por tesorería — se identifica por su producto financiero y copia de él el tipo de tarjeta `[R39]`. Determina el flujo de resolución financiera (vía extracto bancario o pago directo desde Tesorería) y el tratamiento de las **retenciones asumidas**. |
...
| **Alerta de plazo** | El sistema alerta cuando la conciliación no está completada dentro del plazo configurado previo a la fecha de pago (por defecto 3 días). Este plazo es configurable en las preferencias de cada producto financiero `[R39]`. |
...
| ID | Regla | Configurable |
|----|-------|--------------|
| R06 | El extracto debe estar 100% conciliado antes de pasar a la etapa de confirmación. *Nota: Las partidas del extracto que no cuenten con OXP de comercio pueden resolverse mediante: (a) anticipos, o (b) marca de **Partida en Disputa** (para errores bancarios, fraudes potenciales o transacciones no reconocidas). Los cargos adicionales del extracto (4x1000, cuota de manejo, intereses) se consideran conciliados como parte de la OXP de Extracto, no requieren OXP de Comercio y no generan anticipos. Se debe configurar por cada producto financiero `[R39]` cuáles cargos adicionales maneja, para detectarlos automáticamente en el extracto.* | No |
| R06b | Las **Partidas en Disputa** permiten alcanzar el 100% de conciliación sin generar anticipos. Su resolución posterior puede ser: (1) **Descartada:** la partida específica se marca como descartada cuando el banco reversa la transacción, compensándola contra la línea de "Reverso Bancario" en un extracto futuro; (2) **Reclasificación Contable:** cuando se identifica el gasto real y se radica la OXP de Comercio correspondiente, el sistema vincula la partida en disputa del extracto original con la nueva OXP de Comercio mediante una reclasificación contable, sin generar un nuevo documento de cobro ni duplicar la deuda. *Nota contable (issue #90):* al causar el extracto, la partida en disputa viaja a una cuenta transitoria de **partidas por aclarar** — así la deuda con el banco refleja el total real que cobra —; cada resolución la cancela: el descarte, dentro de la causación del extracto donde llega el reverso; la reclasificación, mediante un traslado contable propio que además salda la cuenta por pagar del proveedor identificado (ver `[D36]` del modelo). | No |
| R07 | El sistema alerta cuando la conciliación no está completada dentro del plazo configurado previo a la fecha de pago. | Sí (por producto financiero, default 3 días) |
...
| R10 | La conciliación automática aplica una tolerancia en la comparación de valores. Si la diferencia está dentro de la tolerancia, se acepta el cruce automáticamente. Si la diferencia supera la tolerancia, el sistema funciona en modo asistido sugiriendo al usuario las partidas que considera correspondientes. **Importante:** La diferencia dentro de tolerancia no desaparece; el sistema genera automáticamente un movimiento de ajuste hacia "Gastos Bancarios" (si el extracto es mayor) o "Aprovechamientos Bancarios" (si el extracto es menor). El **sentido** del ajuste (pérdida si el extracto es mayor, ganancia si es menor) lo determina OXP y viaja en el componente correspondiente, registrado contra el **proveedor de la OXP de Comercio cruzada** — el tercero de la partida — y no contra la entidad bancaria; la **cuenta** exacta la resuelve la **traducción contable** del sub-dominio Contabilidad dentro del grupo del rol (`AJUSTE_TOLERANCIA_PERDIDA` / `AJUSTE_TOLERANCIA_GANANCIA` del catálogo de plantillas), no una configuración por tarjeta. Cuando la OXP de Comercio tiene **tarjeta referenciada o últimos 4 dígitos** capturados `[R44]`, la coincidencia con el producto del extracto **prioriza** esa candidata en las sugerencias del modo asistido — la referencia primero, los dígitos sueltos después; nunca excluye: la mayoría de compras no traen el dato, puede venir mal extraído, y una tarjeta reemplazada cambia de número. | Sí (por empresa, default +/- 1000 COP) |
| R10b | **Ajuste por Diferencia en Cambio:** Para cada OXP de Comercio en moneda extranjera, si existe diferencia entre el valor radicado (TRM del día de la transacción) y el valor reflejado en el extracto (TRM del día de corte), el sistema genera automáticamente un **concepto de ajuste por diferencia en cambio** sobre la OXP de Extracto: clasificado como gasto financiero si la TRM subió, o ingreso financiero si la TRM bajó. Se genera un concepto de ajuste por cada OXP de Comercio en moneda extranjera. Estos conceptos permiten el cruce exacto sin crear nuevas OXP. El ajuste se registra contra el **proveedor de la OXP de Comercio cruzada** (el tercero de la partida), no contra la entidad bancaria; el sentido (pérdida o ganancia) lo determina OXP y viaja en el componente correspondiente — el sistema contable no lo infiere. Nota: las partidas del extracto en moneda extranjera ya fueron convertidas a moneda funcional durante la radicación `[R05d]`; la diferencia de cambio se calcula comparando la TRM de radicación de la OxpComercio contra la TRM del extracto aplicada a la partida. | No |
...
| ID | Regla | Configurable |
|----|-------|--------------|
| R11 | Los documentos del dominio OXP pueden confirmarse manualmente por un usuario o automáticamente según configuración de la empresa. La confirmación aplica a tres tipos de documento con estados previos diferentes: (1) **OXP de Comercio:** la confirmación valida la operación de radicación y habilita la causación; los pagos (vía extracto, anticipo, devolución o pago directo) reducen progresivamente el saldo pendiente. (2) **OXP de Extracto:** la confirmación se habilita únicamente cuando la conciliación está completada (100%), y posteriormente se monitorea el pago. (3) **Anticipo:** la confirmación valida el registro y habilita la causación; los pagos externos del anticipo (extracto, pago directo) solo se aplican desde el estado Causada. Los Anticipos nacidos por excedente de devolución se confirman automáticamente (heredan la confirmación del flujo de devolución). La confirmación exige además que la **distribución por unidad organizacional esté asignada** — los pendientes de asignación se resuelven a más tardar aquí, porque la confirmación es la última puerta humana antes de la causación: en el canal manual el sistema exige resolverla; en el canal automático el documento queda retenido y la confirmación se re-dispara sola al resolverse (mismo esquema de la verificación del medio de pago de R44). El pendiente por **propagación** de la copia local de unidades (la unidad existe, su evento viene en camino) **no bloquea** la confirmación: difiere la causación de esa parte y se libera solo. | Sí (por empresa) |
...
| R39 | **Registro de productos financieros y emisor del extracto:** El emisor/banco de una OXP de Extracto **proviene siempre del producto financiero registrado** al que el extracto se atribuye `[R41]` — nunca se digita por extracto ni se infiere del histórico. La empresa define sus productos financieros — sus **tarjetas** — con su emisor (un **tercero (entidad financiera)**, no un proveedor de OXP), su configuración (cargos adicionales que maneja, plazo de alerta de conciliación) y, si aplica, la **identificación de la cuenta consolidadora** a la que está asociada — el mismo valor agrupa las tarjetas de una misma cuenta. Si el número de tarjeta de un producto nuevo coincide con una cuenta consolidadora ya anotada, el sistema **advierte** (no bloquea): probablemente se está registrando la cuenta como si fuera una tarjeta — el camino a duplicar la deuda (consolidado + individuales del mismo período). *El registro nace en OXP como dueño provisional del dato (componente Medios de pago); cuando exista el dueño definitivo (Tesorería o un contexto global), se extrae y OXP lo consume por copia local.* | No |
...
| R41 | **Atribución del extracto a un producto registrado:** Todo extracto, llegue por carga de un usuario o por el **proceso automático que lee el correo**, se atribuye a un producto financiero del registro usando los datos del archivo (número de tarjeta). Si el sistema no logra atribuirlo (número desconocido, archivo ambiguo), el extracto **no se pierde ni bloquea el flujo**: queda en una **bandeja de pendientes de atribución** donde un usuario lo resuelve — registrando el producto nuevo o corrigiendo la atribución. Al resolver la bandeja, el sistema **pre-llena** el registro del producto con los datos interpretados del archivo (número de tarjeta; emisor si el archivo lo trae) — **asistencia de captura**: el usuario confirma o corrige, el registro nunca se crea solo. | No |
| R42 | **Unicidad del extracto por producto y período:** Solo puede existir un extracto radicado por combinación (producto financiero, período). El reenvío del correo o la recarga del mismo archivo se reconocen y descartan como **reintento idempotente** — sin crear segunda deuda y sin generar alerta. | No |
| R43 | **Salvaguarda ante extractos consolidados:** La conciliación y la deuda se manejan **por tarjeta**. Si un archivo trae partidas de **varios números de tarjeta** (indicio de un extracto consolidado de una cuenta que agrupa tarjetas), el sistema **no lo radica automáticamente**: queda en la bandeja de pendientes de atribución `[R41]` con alerta — si las tarjetas del archivo están anotadas a la misma **cuenta consolidadora** `[R39]`, la alerta lo **diagnostica**: el archivo es probablemente el consolidado de esa cuenta —, y el usuario decide cómo gestionarlo (obtener los extractos individuales, o registrar el caso para evaluación). Una partida con un **número de tarjeta no registrado** genera alerta (tarjeta sin registrar o extracto mal atribuido); no bloquea. *(El sistema detecta y alerta, no exige — mismo criterio del refuerzo por canal de R38. El soporte del extracto consolidado como fuente de deuda queda anotado como evolución en el issue #106.)* | No |
| R44 | **Resolución del medio de pago de la OXP de Comercio:** Al radicar, el medio de pago se resuelve por una cadena de niveles — **señales fuertes deciden, señales débiles sugieren, la ausencia no inventa** (molde de R40): **(1) Capturado por el usuario** — gana siempre; incluye la **selección de la tarjeta del registro de productos financieros** (la compra queda con la referencia); editable en cualquier momento antes de causar. **(2) Deducido del soporte con señal fuerte:** si el soporte trae los **últimos 4 dígitos** de la tarjeta (ej. PDF de proveedor del exterior), se resuelve `tarjeta`; si los dígitos **coinciden con un único producto registrado**, la compra queda con la **referencia** a esa tarjeta; sin coincidencia, los dígitos se guardan como **evidencia suelta** (la conciliación establecerá la tarjeta en firme). **(3) Default del proveedor:** si el proveedor tiene configurado un **medio de pago habitual** (tipo y, opcionalmente, su **tarjeta habitual** del registro), resuelve automáticamente y desde el primer día; **si no lo tiene configurado, el sistema no asume nada por él** — el default se configura, no se infiere del histórico. **(4) Sugerencia del XML de facturación electrónica** — solo **pre-llena** (es lo que el emisor declaró, no cómo pagó la empresa): en canal manual el usuario confirma; en canal automático no decide sola — queda tentativa. El default del proveedor pesa más que lo declarado por el emisor. **(5) Sin resolución → default conservador:** tipo no-tarjeta (tratamiento estándar de retenciones), **marcado como asumido por defecto**. Toda resolución **registra su origen** (`capturado \| deducido_soporte \| default_proveedor \| sugerido_xml \| asumido_por_defecto`). **Verificación en la confirmación (cierre del flujo automático):** las resoluciones **fuertes** (capturado, deducido del soporte, default del proveedor) avanzan sin fricción — la confirmación automática procede; las **débiles** (`sugerido_xml`, `asumido_por_defecto`) **exigen decisión humana** según el modo configurado por empresa (bloqueo / decisión obligatoria — mismo esquema de R38): en el canal automático la OXP queda **retenida en Pendiente** con motivo "medio de pago por confirmar" (bandeja de pendientes de confirmación, pre-llenada para resolver con un clic). Mientras no se confirme no hay causación, y el pago directo exige estado Causada `[R35]` — el doble pago del caso "tarjeta asumida como crédito" queda **estructuralmente cerrado**. | Sí (default por proveedor; modo de verificación por empresa) |
| R45 | **Retención asumida:** Una retención del desglose fiscal queda **asumida** por la empresa — se declara y certifica igual, pero no se descuenta del valor a pagar y se reconoce como gasto propio — por una de dos causas: **(a)** el pago del documento ya salió completo (medio de pago tarjeta): aplica a todas las retenciones del documento; **(b)** la retención es la **retención a beneficiarios del exterior** que Impuestos liquida al proveedor sin domicilio fiscal en el país: nace asumida por defecto, porque en la práctica el proveedor del exterior recibe el total, y el usuario puede desmarcarla en el documento antes de causar cuando el contrato permite descontarla. La decisión de asumir es de OXP, que conoce el medio de pago y el acuerdo con el proveedor; Impuestos solo liquida la retención. | No |
...
|------|-------------|:----:|
| **Registro de proveedores** | Captura y gobierno del registro propio del Proveedor: identificación legal validada localmente, razón social, tipo de persona, direcciones, contactos, estado operativo y **medio de pago habitual** (opcional: tipo y, si aplica, tarjeta habitual del registro — nivel 3 de la cadena `[R44]`). Vía única idempotente desde la radicación (crear o reutilizar — la radicación nunca se bloquea), asistencia de captura no bloqueante de la bodega, emisión del evento estándar de rol y aplicación automática de las decisiones de la bodega (señal global, correcciones). | F1 |
| **Medios de pago** | **Registro de productos financieros** (las tarjetas de la empresa — crédito y débito prepago — con la identificación de la cuenta consolidadora a la que están asociadas, si aplica): emisor, configuración de cargos adicionales y plazos de alerta `[R39]`; y características de las demás formas de pago (crédito a proveedor — plazos, condiciones de pago — y contado). Modelado como componente independiente dentro del BC de OXP — en el futuro se extraerá a Tesorería o a un sub-dominio de configuraciones transversales cuando otros sub-dominios lo requieran. | F1 |
...
| **OXP de Extracto** | Ciclo de vida completo de obligaciones consolidadas por producto financiero (tarjeta `[R39]`). Ciclo: radicación → conciliación → confirmación → causación → pago. | F1 |
...
| **Tesorería - Medios de pago** | Componente que gestiona los datos maestros de los productos financieros (número, tipo, fecha de corte, fecha de pago, límites, entidad bancaria). | Por ahora OXP es el **dueño provisional** del registro de productos financieros (agregado `ProductoFinanciero` del modelo) `[R39]`. Cuando se construya el dueño definitivo (Tesorería o un contexto global), el registro se extrae y OXP pasa a consumirlo por **copia local por eventos** — mismo patrón ya usado con las unidades organizacionales. |
...
| OXP de Extracto | Obligaciones consolidadas por producto financiero (tarjeta). Ciclo: radicación → conciliación → confirmación → causación → pago. |
...
| Medios de pago | Registro de productos financieros (las tarjetas de la empresa) y características de las demás formas de pago (crédito, contado). Componente independiente dentro del BC. |
...
| 1.17 | Julio 2026 | **Nota contable en `R06b` — la partida en disputa cierra su ciclo en la contabilidad (issue #90).** Al causar el extracto, la partida en disputa viaja a una cuenta transitoria de **partidas por aclarar** (la deuda con el banco refleja el total real que cobra); cada resolución la cancela — el descarte dentro de la causación del extracto donde llega el reverso, y la reclasificación mediante un traslado contable propio que salda además la CxP del proveedor identificado. Detalle en `[D37]` del modelo (v4.6) y en el catálogo de plantillas de Contabilidad v1.7. |
| 1.18 | Julio 2026 | **Registro de productos financieros — replanteamiento de la resolución del emisor del extracto (issue #106; reemplaza el planteamiento del #57).** Casos de uso del equipo de desarrollo mostraron que: casi ninguna compra permite deducir la forma de pago; en algunos extractos no es posible determinar el emisor; y existen entidades que agrupan varias tarjetas bajo una **cuenta consolidadora** con extracto consolidado (caso real Bancolombia). La evidencia operativa (fechas de corte y pago distintas por tarjeta, contabilización y gestión independientes) muestra que **la unidad real de conciliación es la tarjeta** — se decidió **no cubrir el extracto consolidado en esta fase** (anotado como evolución en el #106). Se replantea: la empresa **define sus productos financieros** — sus tarjetas — y el sistema verifica contra lo definido, sin adivinar el comportamiento de cada entidad financiera. **Nuevo término del glosario:** Producto Financiero (tarjeta con emisor, configuración y marca informativa opcional de asociación a cuenta consolidadora). **R39 reescrita** (deja de ser provisional: el emisor proviene siempre del producto registrado; el registro nace en OXP como dueño provisional, extraíble a Tesorería o a un contexto global con copia local). **Reglas nuevas:** R41 (atribución del extracto a un producto registrado — canal usuario o proceso automático de correo; bandeja de pendientes de atribución con asistencia de captura: pre-llenado desde el archivo, el registro nunca se crea solo), R42 (unicidad por producto y período — reintentos idempotentes, sin alerta), R43 (salvaguarda ante extractos consolidados: archivo con varios números de tarjeta → bandeja con alerta, no se radica automáticamente; partida con tarjeta no registrada → alerta; detecta y alerta, no exige — criterio de R38). Glosario "OXP de Comercio" corregido (la forma de pago casi nunca se deduce del soporte: se captura el **tipo** al radicar; la tarjeta concreta queda establecida en la conciliación) y "Extracto Bancario" ajustado (conciliación por tarjeta; el consolidado no se radica como deuda en esta fase). Componente **Medios de pago** de la tabla de alcance reescrito como registro de productos financieros; dependencia "Tesorería - Medios de pago" actualizada (dueño provisional + copia local futura). Configuraciones "por tarjeta" formalizadas contra el registro (R06, R07 y la alerta de plazo de la Sección 4 — el registro guarda solo lo que una regla usa: cargos y plazo de alerta; el período y la fecha de pago son de cada extracto). **R10 corregida de fondo:** la cuenta del ajuste por tolerancia la resuelve la **traducción contable** (rol `AJUSTE_TOLERANCIA` del catálogo de plantillas de Contabilidad), no una configuración por tarjeta — inconsistencia preexistente: el modelo lo definía así desde el issue #10 y el alcance seguía diciendo lo contrario. Acompaña al modelo v4.9 (agregado `ProductoFinanciero`; retiro de [D35]/[SI9] con renumeración). |
| 1.19 | Julio 2026 | **Medio de pago canónico y cadena de resolución (issue #96; completa lo diferido por la decisión de retenciones asumidas).** Análisis de canales con el equipo de desarrollo: las compras entran por carga manual de PDF o carga automática (XML de facturación electrónica, o PDF para proveedores del exterior sin FE); el **XML FE a veces declara tarjeta pero no certifica cómo pagó la empresa**; los PDF del exterior **a veces traen los últimos 4 dígitos** de la tarjeta. **Nuevo término del glosario:** Medio de Pago (tipos canónicos: tarjeta de crédito, tarjeta débito, tarjeta débito prepago, transferencia, efectivo, cheque, otro; número y entidad solo para tarjetas). **Nueva regla R44** — cadena de resolución al radicar (molde de R40): capturado (incluye selección de la tarjeta del registro — la compra queda con la **referencia** al producto financiero) → deducido del soporte (últimos 4 = señal fuerte; con coincidencia única en el registro → referencia; sin coincidencia → evidencia suelta) → **default del proveedor** (medio de pago habitual configurable, con tarjeta habitual opcional; sin default, el sistema no asume) → sugerencia del XML (solo pre-llena, no decide sola en canal automático) → conservador no-tarjeta **marcado como asumido**; origen rastreado por resolución. **Verificación en la confirmación:** resoluciones fuertes avanzan solas; las débiles (`sugerido_xml`/`asumido_por_defecto`) quedan **retenidas en Pendiente** (bandeja de pendientes de confirmación) hasta decisión humana, según modo configurable por empresa (bloqueo/decisión obligatoria, esquema R38) — sin confirmación no hay causación y el pago directo exige Causada (R35): el doble pago "tarjeta asumida como crédito" queda estructuralmente cerrado. **Coherencia con el extracto sin regla ni evento propios** (se evaluó y retiró una regla de alerta): la garantizan la verificación de la confirmación de R44, el **control de saldos existente** (los pagos por extracto y por tesorería compiten por el mismo saldo, que solo se consume una vez — el doble pago no puede consumarse) y la **diferencia neto-vs-total** que el propio cruce hace visible (corrección operativa). **R10 ampliada:** los últimos 4 capturados **priorizan** las sugerencias del modo asistido, nunca excluyen. Tabla de alcance ("Registro de proveedores" + medio de pago habitual). Definición completa en el comentario del #96 (jul-27). Acompaña al modelo v4.10. |
| 1.20 | Agosto 2026 | **El extracto no lleva medio de pago; la cuenta consolidadora como dato agrupable con diagnóstico y advertencia (issues #126 y #127, planteados por el equipo de desarrollo).** Glosario **Medio de Pago**: la OXP de Extracto deja de llevar el dato — el extracto **es** la deuda de la tarjeta y se paga por tesorería; se identifica por su producto financiero y copia de él el tipo de tarjeta (el origen de la deuda no es una forma de pago). Glosario **Producto Financiero**: la marca informativa de asociación pasa a ser la **identificación de la cuenta consolidadora** — un solo dato opcional cuyo mismo valor **agrupa** las tarjetas de una misma cuenta. **R39 ampliada:** el registro guarda esa identificación y, si el número de tarjeta de un producto nuevo coincide con una cuenta anotada, el sistema **advierte** (no bloquea) — cierra el registro accidental de la cuenta consolidadora como si fuera una tarjeta, el camino a duplicar la deuda (consolidado + individuales del mismo período) que la salvaguarda de R43 no cubría. **R43 ampliada:** cuando las tarjetas del archivo están anotadas a la misma cuenta consolidadora, la alerta **diagnostica** por qué sospecha un consolidado. Componente **Medios de pago** de la tabla de alcance ajustado. **Alineación por validación contra la implementación (ago-12):** en el glosario Medio de Pago, la **entidad bancaria deja de hacer parte del medio de pago** (es el emisor del producto financiero registrado) y el número identifica el instrumento — identidad en los tipos tarjeta, opcional en los demás (número del cheque, referencia de la transferencia). Acompaña al modelo v4.12. |
| 1.21 | Agosto 2026 | **La confirmación exige la distribución asignada; el pendiente de propagación no la bloquea (issue #123).** R11 precisada con la distinción entre los **dos pendientes de distribución**: el pendiente de **asignación** (ninguna regla resolvió y el usuario no ha asignado) se resuelve a más tardar en la **confirmación** — la última puerta humana antes de la causación: en canal manual el sistema exige resolverla, en canal automático el documento queda retenido y la confirmación se re-dispara sola al resolverse (mismo esquema de la verificación del medio de pago de R44); el pendiente de **propagación** de la copia local de unidades (la unidad existe, su evento viene en camino) no bloquea la confirmación — difiere la causación de esa parte y se libera solo. Acompaña al modelo v4.14 (nueva invariante I29). |
| 1.22 | Septiembre 2026 | **El sentido de los ajustes de conciliación lo determina OXP y se registran contra el proveedor de la compra cruzada (issue #132, planteado por el equipo de desarrollo).** R10 y R10b precisadas: la diferencia en cambio y el ajuste por tolerancia nacen al cruzar la partida del extracto con la OXP de Comercio; OXP determina si son pérdida o ganancia y lo comunica en el componente que envía al sistema contable (antes R10 decía que la cuenta la resolvía la traducción contable "según el sentido de la diferencia", lo que dejaba esa decisión al sistema contable). Ambos ajustes quedan registrados contra el **proveedor de la OXP de Comercio cruzada** — el tercero de la partida — y no contra la entidad bancaria emisora del extracto; la cuenta exacta sigue siendo de la traducción contable, dentro del grupo del rol correspondiente. Sin cambios en el glosario ni en el alcance funcional. Acompaña al modelo v4.15 (nueva decisión D42) y al catálogo de plantillas de Contabilidad v1.11. |
| 1.23 | Septiembre 2026 | **Retención asumida con dos causas — nueva regla R45 (issue #143; espejo del `[D17]` de Impuestos v2.1.0).** La retención a beneficiarios del exterior (proveedor sin domicilio fiscal en el país, tributo propio de Impuestos) nace asumida por defecto sin depender del medio de pago, desmarcable por documento; la causa por tarjeta se mantiene. Glosario "OXP de Comercio" ajustado (remite a R45). Acompaña al modelo v4.16 (`[D37]` ampliada). |
