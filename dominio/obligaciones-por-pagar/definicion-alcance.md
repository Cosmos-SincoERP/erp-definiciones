...
| R10 | La conciliación automática aplica una tolerancia en la comparación de valores. Si la diferencia está dentro de la tolerancia, se acepta el cruce automáticamente. Si la diferencia supera la tolerancia, el sistema funciona en modo asistido sugiriendo al usuario las partidas que considera correspondientes. **Importante:** La diferencia dentro de tolerancia no desaparece; el sistema genera automáticamente un movimiento de ajuste hacia "Gastos Bancarios" (si el extracto es mayor) o "Aprovechamientos Bancarios" (si el extracto es menor) — la cuenta exacta la resuelve la **traducción contable** del sub-dominio Contabilidad según el sentido de la diferencia (rol `AJUSTE_TOLERANCIA` del catálogo de plantillas), no una configuración por tarjeta. Cuando la OXP de Comercio tiene **tarjeta referenciada o últimos 4 dígitos** capturados `[R44]`, la coincidencia con el producto del extracto **prioriza** esa candidata en las sugerencias del modo asistido — la referencia primero, los dígitos sueltos después; nunca excluye: la mayoría de compras no traen el dato, puede venir mal extraído, y una tarjeta reemplazada cambia de número. | Sí (por empresa, default +/- 1000 COP) |
