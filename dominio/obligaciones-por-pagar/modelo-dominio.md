...
- **Amortización del anticipo:** cuando `OxpComercioCausada` regulariza un anticipo, la información de la amortización viaja como **tipo de componente** dentro de las líneas de traducción del mismo asiento, no como `tipoTransaccion` independiente. El motor del sub-dominio Contabilidad resuelve internamente la cuenta de anticipos a proveedores mediante su asociación inteligente (ver `[D26]`). OXP siempre emite `tipoTransaccion = causacion_gasto` para `OxpComercioCausada`, regularice anticipo o no.
...
| `amortizacion_anticipo` | regularización de anticipo `[D26]` | `causacion_gasto` | OxpComercio |
...
> **Nota de coordinación cruzada:** `amortizacion_anticipo` y `ajuste_tolerancia` aún no están registrados como componentes en el catálogo de plantillas de Contabilidad (`causacion_gasto`). Su incorporación al catálogo es un punto de seguimiento del lado de Contabilidad para preservar la coincidencia 1:1.
...
| **Efectos** | Integración saliente: causación de OxpExtracto enviada al sistema contable (JSON) con `tipoTransaccion = causacion_gasto` `[D27]`. Registra el total del extracto contra la entidad bancaria, incluyendo cargos adicionales y ajustes (cargos financieros, diferencia en cambio y ajustes por tolerancia viajan como tipos de componente dentro de las mismas líneas). Espera `EntregaAceptada` del sub-dominio Contabilidad para registrar la `referenciaDestino`. Si `saldoPorPagar()` > 0: extracto disponible para recibir pagos (notificados por el sistema contable y/o devolución). Si `saldoPorPagar()` = 0 (devolución cubrió 100% en Confirmada): emite `ExtractoPagado` como derivado por transición. |
...
| D26 | **La amortización del anticipo se entrega al sistema contable junto con la causación de la OXP de Comercio que lo regulariza.** Cuando una OXP de Comercio que regulariza un anticipo se causa, la información de la amortización viaja dentro de esa misma causación. El sistema contable produce un único registro que reconoce el gasto y reclasifica el saldo del anticipo a las cuentas definitivas. No se emite un mensaje separado para la amortización. El Anticipo recibe la confirmación vía `AnticipoAmortizado` para cerrar su ciclo. | El asiento contable de la amortización (Db CxP proveedor · Cr Anticipos a proveedores) ocurre en el mismo momento que la causación de la OXP de Comercio (Db Gasto · Cr CxP proveedor). Separarlos implicaría dos registros descoordinados sobre el mismo hecho económico. Mantenerlos juntos es la práctica contable estándar (alineado con SAP Down Payment Clearing F-54 y Oracle Prepayment Application). | DDD: el evento de causación de la OXP de Comercio es el único punto donde se notifica al sistema contable sobre ese hecho económico, incluyendo todos sus efectos (gasto, retenciones, amortización si aplica). El Anticipo recibe el eco vía `AnticipoAmortizado` para cerrar su propio ciclo contable. |
