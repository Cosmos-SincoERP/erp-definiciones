...
**Priorización por tarjeta (`[R10]` `[R44]`):** en el modo asistido, la candidata cuya **tarjeta referenciada** (`productoFinancieroId`) coincide con el producto del extracto se prioriza primero; la coincidencia de **últimos 4 sueltos**, después — nunca excluyen (la mayoría de compras no traen el dato; puede venir mal extraído; una tarjeta reemplazada cambia de número). **Coherencia del medio de pago (`[R45]`):** la vinculación de una OXP cuyo medio de pago resuelto **no es tarjeta**, o cuya tarjeta referenciada/dígitos **discuerdan** del producto del extracto, emite `IncoherenciaMedioPagoDetectada` — detecta y alerta, no bloquea el cruce.
...
#### IncoherenciaMedioPagoDetectada

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una partida del extracto se vinculó con una OXP de Comercio cuyo medio de pago resuelto no corresponde: no es tarjeta (sus retenciones se descontaron con tratamiento estándar) o su tarjeta referenciada/dígitos discuerdan del producto del extracto. |
| **Causalidad** | Derivada de `VinculacionRealizada` — evaluación de coherencia del `ServicioDeConciliacion` (`[R45]`). |
| **Agregado** | OxpComercio |
