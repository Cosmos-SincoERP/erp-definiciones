...
- **Partida en disputa (ciclo por cuenta transitoria, ver `[D37]`):** al causar el extracto, cada partida en disputa viaja como **tipo de componente** (`partida_por_aclarar`) dentro de `causacion_gasto` — así la contrapartida (CxP del banco/emisor) refleja el total real del extracto. La resolución por **descarte** también viaja como componente (`partida_aclarada`) dentro de la causación del extracto futuro donde llega el reverso bancario (`R10c`). La resolución por **reclasificación** sí es `tipoTransaccion` independiente (`reclasificacion_partida`, emitido por `PartidaEnDisputaReclasificada`): ocurre después de que ambas causaciones salieron y no hay dónde embeberla — mismo criterio del Caso B de `[D26]`.
- **Retenciones asumidas (`[D38]`):** cuando el pago del documento ya se realizó completo (medio de pago tarjeta), las retenciones del desglose viajan **idénticas** al caso normal (misma línea Cr, misma cuenta, tercero = proveedor — se declaran y certifican igual) y cada una emite **además** una línea `retencion_asumida` (Db espejo, gasto propio de la empresa); el valor a pagar no las descuenta. La decisión es de OXP — el único que conoce el medio de pago.
...
| `retencion_asumida` | retención del desglose marcada como **asumida** (medio de pago tarjeta, `[D38]`) — una línea Db espejo por cada retención asumida | `causacion_gasto` | OxpComercio |
| `contrapartida` | línea de contrapartida del hecho económico (`[D39]`): tercero del documento (proveedor, o banco/emisor en el extracto) + clasificación compuesta (medio de pago + observación general); **sin valor ni unidad organizacional** — los rinde el motor de Contabilidad | `causacion_gasto`, `anticipo_a_proveedor`, `nota_credito_gasto`, `reversa_anticipo`, `amortizacion_anticipo` (todas las plantillas con rol CONTRAPARTIDA; `reclasificacion_partida` no la lleva) | OxpComercio, OxpExtracto, Anticipo, Devolucion |
...
- **`clasificacion`** (por línea, **obligatoria**): texto semántico que Contabilidad empareja por similitud para resolver la cuenta (`[R52]` de Contabilidad). OXP la compone **mecánicamente** según el componente — **recetas de composición** (`[D39]`):
...
| `retencion_asumida` | nombre y % del tributo asumido + medio de pago que origina la asunción (`[D38]`) |
...
| `lineasParaTraduccion()` | Pre-computa una lista plana de líneas, una por cada combinación (componente × destino de negocio), con el valor ya distribuido (valor × porcentaje). Cada línea incluye: `tipoComponente` canónico (`gasto`; los tributos con su código específico del desglose fiscal: `iva`, `inc`, `retefuente`, `reteiva`, `reteica`; `diferencia_en_cambio` y `amortizacion_anticipo` cuando apliquen — ver catálogo canónico de `tipoComponente`), identificador de unidad organizacional, valor distribuido y, en las líneas de concepto (`gasto`), la `descripcionConcepto` tomada de `ConceptoDeGasto.descripcion`. Cuando el medio de pago es de tipo tarjeta (`[D38]`), cada retención del desglose emite además su línea **`retencion_asumida`** (Db espejo del valor) y las retenciones no se descuentan del valor a pagar. Cada línea lleva su **`clasificacion`** compuesta según las recetas de `[D39]`, y el hecho emite además la **línea `contrapartida`** (tercero = proveedor; medio de pago + observación general; sin valor ni unidad). El servicio de Traducción Contable recibe estas líneas y resuelve la cuenta de cada una con su clasificación (y el balance de la contrapartida) — no necesita entender distribuciones, herencias ni los componentes internos de OXP. |
...
| **Efectos** | La partida cuenta como conciliada para alcanzar el 100% `[R06]`. Permite avanzar sin generar anticipos. Resolución posterior vía `PartidaEnDisputaDescartada` o `PartidaEnDisputaReclasificada`. Contablemente no emite nada en este momento: la línea `partida_por_aclarar` viaja cuando el extracto se causa (`ExtractoCausado`, `[D37]`). |
...
| **Efectos** | Cierra el ciclo de la partida en disputa. Contablemente, la cancelación de la transitoria viaja como línea `partida_aclarada` dentro de la causación del extracto donde llegó el reverso (`[D37]`) — este evento no emite integración propia. |
...
| **Información capturada** | Referencia a la nueva OxpComercio, líneas de traducción de la reclasificación (`[D37]`), fecha de resolución, usuario. |
| **Efectos** | Sistema vincula la partida del extracto original con la nueva OxpComercio. Sin generar documentos duplicados ni nueva deuda `[R06b]`. **Integración saliente (`[D37]`):** emite `tipoTransaccion = reclasificacion_partida` con dos líneas — Db CxP del proveedor de la nueva OxpComercio (`cruce_obligacion`, tercero = proveedor) y Cr transitoria de partidas por aclarar (`partida_aclarada`, tercero = banco/emisor); `terceroPrincipal` = banco/emisor; sin contrapartida del motor. Espera `EntregaAceptada` para registrar la `referenciaDestino`, como toda causación (`[D28]`/`[SI6]`). |
