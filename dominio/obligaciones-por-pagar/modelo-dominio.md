...
| `contrapartida` | medio de pago + observación general del documento (en el extracto: tipo de producto —tarjeta de crédito/cuenta— + banco/emisor y periodo) |
...
| `MedioDePago` | Tipo canónico, número, entidad bancaria — copiado del `ProductoFinanciero` atribuido (`[D39]`) |
| `ValorMonetario` | Monto, moneda, TRM (si aplica), monto en moneda funcional |
...
| `asociadaA` | Marca informativa opcional | { esAsociada, datosCuentaConsolidadora (texto libre, opcional) } — la tarjeta está asociada/amparada a una **cuenta consolidadora**. **Sin efecto en las reglas de esta fase**: documenta la realidad y es la semilla de la evolución (extracto consolidado como fuente de deuda — issue #106) |
...
| Comando | Descripción |
|---|---|
| `RegistrarProductoFinanciero` | **Única vía de creación** — registro deliberado, previo a radicar extractos. Valida la unicidad del número de tarjeta (I26). Invocable desde la resolución de la bandeja de pendientes de atribución (`[R41]`): registrar el producto que faltaba y continuar la radicación. |
| `ActualizarProductoFinanciero` | Actualización explícita de datos (emisor, tipo, configuración de cargos, plazo de alerta, marca `asociadaA`). El evento captura **delta**. Las radicaciones futuras copian el dato corregido; los extractos históricos no se reescriben. |
...
| **Estado previo** | (nuevo) — no existía previamente. |
| **Estado resultante** | Activo. |
| **Precondiciones** | Número de tarjeta único en el registro (I26). Emisor capturado como **tercero (entidad financiera)** — no es un `Proveedor` (`[D31]`). |
| **Información capturada** | Número de tarjeta (clave natural), emisor (`InformacionTercero`), tipo (`credito` \| `debito_prepago`), configuración de cargos adicionales `[R06]`, plazo de alerta de conciliación `[R07]`, marca informativa `asociadaA` (opcional). |
...
| **Información capturada** | **Delta:** `productoFinancieroId` + campos modificados (emisor, tipo, configuración de cargos, plazo de alerta, marca `asociadaA`). |
