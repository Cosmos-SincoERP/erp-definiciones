...
| `lineasParaTraduccion()` | Pre-computa una lista plana de líneas, una por cada combinación (componente × destino de negocio), con el valor ya distribuido (valor × porcentaje). Cada línea incluye: `tipoComponente` canónico (`gasto`; los tributos con su código específico del desglose fiscal: `iva`, `inc`, `retefuente`, `reteiva`, `reteica`; `diferencia_en_cambio` y `amortizacion_anticipo` cuando apliquen — ver catálogo canónico de `tipoComponente`), identificador de unidad organizacional, valor distribuido y, en las líneas de concepto (`gasto`), la `descripcionConcepto` tomada de `ConceptoDeGasto.descripcion`. Cuando el medio de pago es de tipo tarjeta (`[D37]`), cada retención del desglose emite además su línea **`retencion_asumida`** (Db espejo del valor) y las retenciones no se descuentan del valor a pagar. Cada línea lleva su **`clasificacion`** compuesta según las recetas de `[D38]`, y el hecho emite además la **línea `contrapartida`** (tercero = proveedor; medio de pago + observación general; sin valor ni unidad). El servicio de Traducción Contable recibe estas líneas y resuelve la cuenta de cada una con su clasificación (y el balance de la contrapartida) — no necesita entender distribuciones, herencias ni los componentes internos de OXP. |
...
| **Información capturada** | `proveedorId` (referencia al Proveedor `[D31]`); tercero (NIT, razón social — copiado del Proveedor), fecha de transacción, valor en moneda original, moneda, TRM del día si aplica `[R05b]`, valor en moneda funcional, número de soporte/factura, medio de pago resuelto por la cadena (`[R44]`/`[D40]`: tipo canónico + origen de la resolución + tarjeta — { `productoFinancieroId`, últimos 4 } si se resolvió contra el registro, últimos 4 sueltos como evidencia si no), conceptos (gasto/costo + impuestos + retenciones) con clasificacionTributaria y conceptoPago por cada concepto (resueltos desde el catálogo del sub-dominio origen o del catálogo de gasto directo de OXP), subDominioOrigen `[SI5]`, distribución de costos si aplica `[R05c]`, soportes documentales adjuntos. |
