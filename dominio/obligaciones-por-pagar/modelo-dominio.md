...
| `MedioDePago` | VO | Tipo canónico (`[D40]`); número y entidad bancaria solo para los tipos tarjeta |
...
| Componente | Tipo | Contenido |
|---|---|---|
| `numeroTarjeta` | Texto | **Clave natural** — única en el registro (I26). Llave de atribución de extractos `[R41]` |
...
| `ActualizarProductoFinanciero` | Actualización explícita de datos (emisor, tipo, configuración de cargos, plazo de alerta, `cuentaConsolidadora`). El evento captura **delta**. Las radicaciones futuras copian el dato corregido; los extractos históricos no se reescriben. |
...
| **Estado resultante** | (sin cambio de estado). |
| **Precondiciones** | Si cambia el número de tarjeta (corrección excepcional de la clave natural), la unicidad I26 se mantiene. |
