...
| **Registro de medios de pago** | Registro de productos financieros (las tarjetas de la empresa) con su emisor y configuración; atribución de extractos con bandeja de pendientes | ProductoFinanciero | `[F1]` |
...
| `PartidaEnDisputaReclasificada` | `reclasificacion_partida` | Reclasificación de partida por aclarar (plantilla nueva — catálogo precargado v1.7, issue #90) |
...
- **Diferencia en cambio y ajuste por tolerancia (`[D42]`):** viajan como **tipos de componente** dentro de las líneas de `ExtractoCausado` — el único documento que los produce: ambos nacen al **cruzar la partida del extracto con la compra** en la conciliación (`[R10]`, `[R10b]`). No son `tipoTransaccion` separados. Cada ajuste viaja en **uno de dos componentes según su signo, decidido por OXP** — pérdida (Db: `diferencia_en_cambio_perdida`, `ajuste_tolerancia_perdida`) o ganancia (Cr: `diferencia_en_cambio_ganancia`, `ajuste_tolerancia_ganancia`) — porque la naturaleza contable vive en el rol de la plantilla y una línea alimenta una sola partida; nunca ambos para un mismo ajuste. La línea lleva el **tercero del proveedor de la compra cruzada**, igual que `cruce_obligacion` — el ajuste es la diferencia sobre esa obligación, no un cargo del banco. Esto preserva la trazabilidad al documento origen, necesaria para ajustes o notas crédito posteriores.
...
- **Partida en disputa (ciclo por cuenta transitoria, ver `[D36]`):** al causar el extracto, cada partida en disputa viaja como **tipo de componente** (`partida_por_aclarar`) dentro de `causacion_gasto` — así la contrapartida (CxP del banco/emisor) refleja el total real del extracto. La resolución por **descarte** también viaja como componente (`partida_aclarada`) dentro de la causación del extracto futuro donde llega el reverso bancario (`R10c`). La resolución por **reclasificación** sí es `tipoTransaccion` independiente (`reclasificacion_partida`, emitido por `PartidaEnDisputaReclasificada`): ocurre después de que ambas causaciones salieron y no hay dónde embeberla — mismo criterio del Caso B de `[D26]`.
- **Retenciones asumidas (`[D37]`):** una retención del desglose queda **asumida** por dos causas: **(a)** el pago del documento ya se realizó completo (medio de pago tarjeta) — no hubo de dónde descontarla; **(b)** el tributo es la **retención a beneficiarios del exterior** (`RETEFUENTE_EXTERIOR` de Impuestos): el proveedor sin domicilio fiscal en el país recibe el total, así que nace asumida por defecto y el usuario puede desmarcarla en el documento cuando el contrato sí permite descontarla. En ambos casos la retención viaja **idéntica** al caso normal (misma línea Cr, misma cuenta, tercero = proveedor — se declara y certifica igual) y emite **además** una línea `retencion_asumida` (Db espejo, gasto propio de la empresa); el valor a pagar no la descuenta. La decisión es de OXP — el único que conoce el medio de pago y el acuerdo con el proveedor; Impuestos liquida sin conocerla.
- **Tributos autoliquidados — naturaleza `provision` (`[D41]`):** todo tributo de naturaleza `provision` del desglose fiscal (autoliquidados en dirección gasto: la empresa asume un impuesto que el proveedor no factura — el proveedor recibe el total) emite un **par de líneas explícito** dentro de `causacion_gasto`: la línea de **reconocimiento** (Db, `tipoComponente` = código del tributo) y la línea de **provisión por pagar** (Cr, `tipoComponente` = código con sufijo `_por_pagar`), ambas del mismo valor y con tercero = proveedor. Se netean entre sí — el valor a pagar no cambia y la contrapartida no se altera. Mismo molde del par explícito de `[D37]`.

**Catálogo canónico de `tipoComponente` emitidos por OXP:**

Cada línea de traducción que OXP emite lleva un `tipoComponente` cuyo nombre es **canónico** (snake_case) y coincide 1:1 con los componentes declarados en el catálogo de plantillas del sub-dominio Contabilidad (`dominio/contabilidad/datos-precargados/plantillas-de-asiento.*`). Para los tributos, el `tipoComponente` es el **código del tributo** tomado del desglose fiscal del sub-dominio Impuestos (no un genérico "impuesto"/"retención") — así Contabilidad puede acotar la cuenta por grupo del PUC `[D12 Contabilidad]`. La dirección contable (débito/crédito) no se codifica en el nombre: la determina la plantilla según el `tipoTransaccion`, por lo que los componentes de una nota crédito reutilizan el mismo nombre que en la causación (sin sufijo `_devuelto`). Los tributos de naturaleza `provision` son la excepción con nombre: emiten **dos componentes por tributo** — el código del tributo (reconocimiento) y el código con sufijo `_por_pagar` (la provisión) — porque sus dos partidas aterrizan en cuentas distintas y una línea solo alimenta una partida (`[D41]`). Los **ajustes de conciliación** son la segunda excepción: llevan el **signo en el nombre** (`diferencia_en_cambio_perdida`/`_ganancia`, `ajuste_tolerancia_perdida`/`_ganancia`) porque un mismo hecho puede producir cualquiera de los dos sentidos y la plantilla no puede saberlo — el emisor sí, y lo decide al elegir el componente (`[D42]`).
...
| `retefuente_exterior` | `Tributo` retención a beneficiarios del exterior del `DesgloseFiscal` (proveedor sin domicilio fiscal en el país; sustituye a `retefuente`, nunca concurren). Nace **asumida** por defecto (`[D37]`) | `causacion_gasto`, `nota_credito_gasto` | OxpComercio, Devolucion (Comercio) |
...
| `iva_importacion_servicios` | `Tributo` de naturaleza provision del `DesgloseFiscal` — línea de **reconocimiento** del par (`[D41]`) | `causacion_gasto`, `nota_credito_gasto` | OxpComercio, Devolucion (Comercio) |
| `iva_importacion_servicios_por_pagar` | `Tributo` de naturaleza provision del `DesgloseFiscal` — línea de **provisión por pagar** del par (`[D41]`) | `causacion_gasto`, `nota_credito_gasto` | OxpComercio, Devolucion (Comercio) |
...
| `diferencia_en_cambio_perdida` | `AjustePorDiferenciaCambio` con clasificación gasto financiero (la TRM subió) — el signo lo decide OXP (`[D42]`) | `causacion_gasto` | OxpExtracto |
| `diferencia_en_cambio_ganancia` | `AjustePorDiferenciaCambio` con clasificación ingreso financiero (la TRM bajó) — `[D42]` | `causacion_gasto` | OxpExtracto |
| `ajuste_tolerancia_perdida` | `AjustePorTolerancia` con dirección extracto mayor que la compra — `[D42]` | `causacion_gasto` | OxpExtracto |
| `ajuste_tolerancia_ganancia` | `AjustePorTolerancia` con dirección extracto menor que la compra — `[D42]` | `causacion_gasto` | OxpExtracto |
| `cruce_obligacion` | `Vinculacion` (valor de la obligación cruzada, a TRM de radicación) | `causacion_gasto`, `reclasificacion_partida` | OxpExtracto |
| `partida_por_aclarar` | `PartidaExtracto` en estado disputa al causar el extracto | `causacion_gasto` | OxpExtracto |
| `partida_aclarada` | partida de reverso bancario vinculada a una disputa (`R10c`) · reclasificación de la disputa | `causacion_gasto` (reverso) · `reclasificacion_partida` | OxpExtracto |
...
| `retencion_asumida` | retención del desglose marcada como **asumida** (medio de pago tarjeta, o retención a beneficiarios del exterior asumida por defecto — `[D37]`) — una línea Db espejo por cada retención asumida | `causacion_gasto`, `nota_credito_gasto` | OxpComercio, Devolucion (Comercio) |
| `contrapartida` | línea de contrapartida del hecho económico (`[D38]`): tercero del documento (proveedor, o banco/emisor en el extracto) + clasificación compuesta (medio de pago + observación general); **sin valor ni unidad organizacional** — los rinde el motor de Contabilidad | `causacion_gasto`, `anticipo_a_proveedor`, `nota_credito_gasto`, `reversa_anticipo`, `amortizacion_anticipo` (todas las plantillas con rol CONTRAPARTIDA; `reclasificacion_partida` no la lleva) | OxpComercio, OxpExtracto, Anticipo, Devolucion |

> **Nota de coordinación cruzada:** `amortizacion_anticipo` y `ajuste_tolerancia` se incorporaron como roles propios en el catálogo de plantillas de Contabilidad (`causacion_gasto`, v1.3) en el mismo cambio que esta canonización, preservando la coincidencia 1:1. Sus grupos del PUC quedan `porValidar` por consultor contable (ítems 9 y 10 de la revisión pendiente del catálogo). `cruce_obligacion` se incorporó como rol propio `CRUCE_OBLIGACION` (débito, grupo del PUC `["2205","2335"]`) en la misma plantilla `causacion_gasto` (issue #18), preservando la coincidencia 1:1. `partida_por_aclarar` y `partida_aclarada` se incorporaron como roles `PARTIDA_POR_ACLARAR`/`PARTIDA_ACLARADA` (transitoria de partidas por aclarar, grupo tentativo `["1360","1380"]` `porValidar`), y `reclasificacion_partida` como plantilla nueva, en el catálogo v1.7 (issue #90) — misma coincidencia 1:1. En el catálogo v1.11, `diferencia_en_cambio` y `ajuste_tolerancia` se **desdoblaron por signo** en los roles `DIFERENCIA_EN_CAMBIO_PERDIDA`/`_GANANCIA` y `AJUSTE_TOLERANCIA_PERDIDA`/`_GANANCIA`, cada uno con naturaleza fija y un solo grupo del PUC (`[D42]`) — misma coincidencia 1:1.

**Campos que OXP puebla en el contrato `LineaTraduccion`:**

El contrato `LineaTraduccion` se define en el sub-dominio Contabilidad. OXP, como consumidor, puebla — además del `tipoComponente` canónico y el valor — la **clasificación** de cada línea, la **línea de contrapartida**, la **referencia al hecho relacionado** de los componentes de espejo y dos campos de narración (ver `[D15]`/`[R52]`-`[R54]` de Contabilidad y `[R48]`/`[D13]`):

- **`clasificacion`** (por línea, **obligatoria**): texto semántico que Contabilidad empareja por similitud para resolver la cuenta (`[R52]` de Contabilidad). OXP la compone **mecánicamente** según el componente — **recetas de composición** (`[D38]`):

| `tipoComponente` | Receta de composición de `clasificacion` |
|---|---|
| `gasto` | descripción del concepto + concepto de pago + clasificación tributaria (`ConceptoDeGasto`) |
| `iva`, `inc`, `retefuente`, `retefuente_exterior`, `reteiva`, `reteica` | concepto de pago del concepto al que pertenece + nombre y % del tributo (desglose fiscal) |
| `retencion_asumida` | nombre y % del tributo asumido + causa de la asunción: medio de pago que la origina, o beneficiario del exterior (`[D37]`) |
| `iva_importacion_servicios` | concepto de pago del concepto al que pertenece + nombre y % del tributo — igual que los demás tributos (`[D41]`) |
| `iva_importacion_servicios_por_pagar` | nombre y % del tributo + naturaleza de la provisión (autoliquidado por pagar) (`[D41]`) |
| `concepto_devuelto` | igual que `gasto` — espejo del concepto original devuelto (`ConceptoDevuelto`) |
| `cargo_financiero` | subtipo del cargo (`CargoFinanciero.subtipo`: 4x1000, cuota de manejo, intereses) |
| `diferencia_en_cambio_perdida`, `diferencia_en_cambio_ganancia` | "diferencia en cambio" + sentido (pérdida/ganancia) + moneda original (`AjustePorDiferenciaCambio`). El sentido ya viaja en el componente (`[D42]`); en la clasificación es texto de apoyo a la similitud, no la vía por la que Contabilidad decide la naturaleza |
| `ajuste_tolerancia_perdida`, `ajuste_tolerancia_ganancia` | "ajuste por tolerancia" + sentido (extracto mayor/menor que la compra) (`AjustePorTolerancia`). Misma nota: el sentido viaja en el componente (`[D42]`) |
| `amortizacion_anticipo` | descripción del anticipo que se amortiza |
| `anticipo` | descripción del anticipo |
| `reversa_anticipo` | motivo de la reversa (`ReversaTotal.motivoReversa`) + descripción del anticipo original |
| `cruce_obligacion` | "cruce de obligación" + documento fuente y proveedor de la compra cruzada (`Vinculacion` → OxpComercio) |
| `partida_por_aclarar` | descripción de la partida del extracto + motivo de la disputa |
| `partida_aclarada` | descripción de la partida original en disputa + naturaleza de la resolución (reverso bancario / reclasificación) |
| `contrapartida` | medio de pago + observación general del documento (en el extracto: tipo de producto —el `tipoTarjeta` copiado: `credito` \| `debito_prepago`— + banco/emisor y periodo) |

- **Línea `contrapartida`** (una por hecho económico cuya plantilla tiene rol CONTRAPARTIDA — `[R54]` de Contabilidad): lleva el **tercero** del documento (el que antes solo viajaba en `terceroPrincipal`) y su clasificación; viaja **sin valor ni unidad organizacional** (el motor calcula el balance y rinde la unidad según `[I33]` de Contabilidad). `reclasificacion_partida` no la emite (sus dos líneas viajan con valor, `[D36]`).
- **`referenciaHechoRelacionado`** (por línea, en los componentes de **espejo** — `[R53]` de Contabilidad): `cruce_obligacion` → causación de la OxpComercio cruzada; `partida_aclarada` → causación del extracto que abrió la disputa; `amortizacion_anticipo` y `reversa_anticipo` → causación del anticipo original; todas las líneas de `nota_credito_gasto` (contrapartida incluida) → causación devuelta. Contabilidad copia la cuenta de la partida del rol espejado — garantiza que el cruce contable salde en la misma cuenta.

- **`descripcionConcepto`** (por línea): narración del movimiento. OXP la incluye **solo** en las líneas de concepto de negocio — `gasto` (de `ConceptoDeGasto.descripcion`), `concepto_devuelto` (de `ConceptoDevuelto.descripcion`) y `anticipo`. Las líneas de tributo (`iva`, `retefuente`, ...), `cargo_financiero`, los ajustes de conciliación (`diferencia_en_cambio_perdida`/`_ganancia`, `ajuste_tolerancia_perdida`/`_ganancia`), `amortizacion_anticipo`, `partida_por_aclarar`, `partida_aclarada` y `retencion_asumida` no la llevan (su cuenta es autodescriptiva — el catálogo de Contabilidad las marca `llevaDescripcionConcepto: false`).
...
Además, OXP puebla **`terceroPrincipal`** a nivel del hecho económico (no por línea): es el `InformacionTercero` de la raíz del agregado emisor — **el proveedor** en `OxpComercioCausada`, `AnticipoCausado` y `DevolucionCausada`; **el banco/emisor** en `ExtractoCausado` y en `PartidaEnDisputaReclasificada`. Desde `[D38]` es **informativo**: el tercero de la contrapartida ya no se toma de aquí sino de la **línea `contrapartida`**, que lleva el mismo tercero (esto resuelve el caso del extracto — issue #28 — cuyas líneas `cruce_obligacion` traen varios proveedores pero cuya contrapartida es la CxP del banco/emisor, `[D29]`). En el ciclo de la partida en disputa, el tercero por línea sigue la misma lógica: la transitoria de partidas por aclarar (`partida_por_aclarar`/`partida_aclarada`) lleva siempre el **banco/emisor** — la reclamación es contra él y abre/cierra contra el mismo tercero — y el cruce de la reclasificación lleva el **proveedor** de la nueva OxpComercio (`[D36]`).
...
- **Reparación de respaldo (Capa 2 de la guía):** de fondo y fuera del camino crítico, OXP reconcilia su copia contra el punto de resincronización de EO (`[SI09]` de EO) ante un evento perdido o un desfase tras estar caído.
...
| `MedioDePago` | Tipo canónico (`tarjeta_credito \| tarjeta_debito \| tarjeta_prepago \| transferencia \| efectivo \| cheque \| otro`) con el **origen de la resolución** (`capturado \| deducido_soporte \| default_proveedor \| sugerido_xml \| asumido_por_defecto`, `[R44]`/`[D40]`); tarjeta opcional: { `productoFinancieroId`, últimos 4 copiados } cuando se resolvió contra el registro (selección del usuario o dígitos con coincidencia única), o solo { últimos 4 } como **evidencia suelta** sin coincidencia |
...
| `DesgloseFiscal` | Agrupa los cálculos fiscales derivados de un `ConceptoDeGasto`. Inmutable — se reemplaza completo al recalcular. Contiene: `List<Tributo>` de impuestos, `List<Tributo>` de retenciones y `List<Tributo>` de provisiones — tributos autoliquidados de naturaleza `provision` que se reconocen contablemente sin afectar el valor a pagar (`[D41]`). |
| `Tributo` | Cálculo fiscal individual (impuesto, retención o provisión). Tipo, base, tarifa, valor. Inmutable — es el resultado de aplicar reglas fiscales al gasto. Las retenciones llevan además la marca **asumida** con su **origen** (`medio_de_pago` — resuelta por el medio de pago tarjeta; `beneficiario_exterior` — valor por defecto de la retención a beneficiarios del exterior; `manual` — decisión del usuario en el documento). La marca es del documento, no del cálculo fiscal: se puede cambiar por edición antes de causar sin recalcular el desglose (`[D37]`). |
...
Al consultar la distribución efectiva de cualquier componente, el agregado aplica la siguiente cadena (en orden de prioridad; ver `[D35]`):
...
3. **Reglas de preferencia de distribución (Nivel A — determinístico, `[D35]`)** → Conjunto de reglas configuradas en `CatalogoReglasDistribucion`, evaluadas por **especificidad** sobre los criterios `proveedor`, `tipo de gasto/clasificación` y `lugarEjecucion`: gana la regla que casa más criterios; si dos empatan en especificidad, desempata el orden de prioridad de criterios (proveedor > tipo de gasto > lugarEjecucion). La regla **sin criterios** (preferencia general de la empresa) es el default más general — preserva el comportamiento anterior. Resuelve **automáticamente** (no requiere confirmación: el usuario configuró la regla). Cubre el arranque en frío.
4. **Sugerencia por aprendizaje (Nivel B — no vinculante, `[D35]`/`[SI9]`)** → Si ninguna regla aplica, el sistema **pre-llena** con la unidad más frecuente del patrón histórico para esa combinación (`proveedor + tipo + lugarEjecucion`); el usuario **confirma o corrige** → pasa a instrucción explícita (nivel 1). No resuelve sola.
5. **Destino único pendiente** → Si ninguna regla resolvió y no se confirmó sugerencia, destino único al 100% pendiente **de asignación** por el usuario — se resuelve a más tardar en la **confirmación** (I29: sin asignación no hay confirmación; en canal automático el documento queda retenido y la confirmación se re-dispara al resolverse). Distinto es el pendiente **de propagación**: la unidad asignada por la cadena existe en la fuente de verdad pero aún no está Activa en la copia local de Estructura Organizacional (`I24`, `[D34]`) — no bloquea la confirmación; la causación de esa parte se difiere hasta que llegue el evento `UnidadActivada`, sin aproximar con una unidad provisional.

Los niveles 1–3 y 5 son **determinísticos y propios del agregado**; el nivel 4 es **asistencia de la capa de aplicación** (no vinculante, `[D23]`/`[D35]`). Cada distribución resuelta registra **con qué nivel** se asignó (trazabilidad). La sugerencia (nivel 4) y las reglas (nivel 3) operan siempre sobre la **copia local de unidades activas** (`[SI8]`); nunca proponen una unidad inexistente o inactiva.
...
| `valorNeto()` | `valorBruto()` + `totalImpuestos()` - `totalRetenciones()`. Las provisiones no suman ni restan: se reconocen contablemente sin afectar el valor a pagar (`[D41]`). |
...
| `lineasParaTraduccion()` | Pre-computa una lista plana de líneas, una por cada combinación (componente × destino de negocio), con el valor ya distribuido (valor × porcentaje). Cada línea incluye: `tipoComponente` canónico (`gasto`; los tributos con su código específico del desglose fiscal: `iva`, `inc`, `retefuente`, `retefuente_exterior`, `reteiva`, `reteica`; `amortizacion_anticipo` cuando aplique — ver catálogo canónico de `tipoComponente`), identificador de unidad organizacional, valor distribuido y, en las líneas de concepto (`gasto`), la `descripcionConcepto` tomada de `ConceptoDeGasto.descripcion`. Cada retención del desglose marcada como **asumida** (`[D37]`: por medio de pago tarjeta — todas las retenciones — o por ser la retención a beneficiarios del exterior, asumida por defecto) emite además su línea **`retencion_asumida`** (Db espejo del valor) y no se descuenta del valor a pagar. Cada tributo de naturaleza `provision` del desglose emite su **par** de líneas — reconocimiento (código del tributo) y provisión por pagar (código + `_por_pagar`), mismo valor, tercero = proveedor — sin afectar el valor a pagar (`[D41]`). Cada línea lleva su **`clasificacion`** compuesta según las recetas de `[D38]`, y el hecho emite además la **línea `contrapartida`** (tercero = proveedor; medio de pago + observación general; sin valor ni unidad). El servicio de Traducción Contable recibe estas líneas y resuelve la cuenta de cada una con su clasificación (y el balance de la contrapartida) — no necesita entender distribuciones, herencias ni los componentes internos de OXP. |

### Agregado: OxpExtracto [F1]

- **Raíz:** Una obligación consolidada del período, originada por extracto bancario.
- **Producto financiero:** el extracto se identifica por la referencia `productoFinancieroId` (`[R41]`) y lleva copiados al radicar el emisor (`InformacionTercero`), el número de tarjeta (llave de la atribución) y el `tipoTarjeta` (`credito` \| `debito_prepago`) — un dato simple, no un value object. El extracto **no lleva medio de pago**: es la deuda de la tarjeta y se paga por tesorería — su único cruce de pago externo es el confirmado por el sistema contable (`[D39]`).
...
| `AjustePorDiferenciaCambio` | Ajuste generado al vincular OxpComercio en moneda extranjera. La clasificación decide el componente que emite (`diferencia_en_cambio_perdida`/`_ganancia`); su línea contable lleva el tercero del proveedor de la OxpComercio de origen (`[D42]`). | OxpComercio origen, TRM radicación, TRM extracto, valor, clasificación (gasto/ingreso financiero). |
| `AjustePorTolerancia` | Ajuste generado al vincular con diferencia dentro de tolerancia. Inmutable una vez creado. Identidad: referencia trazable al par OxpComercio-partida que lo originó. Participa individualmente en `InstruccionDistribucion` y `lineasParaTraduccion()`. La dirección decide el componente que emite (`ajuste_tolerancia_perdida`/`_ganancia`); su línea contable lleva el tercero del proveedor de la OxpComercio de origen (`[D42]`). | OxpComercio origen, valor diferencia, dirección (extracto mayor/menor). |
...
| Value Object | Contenido |
|---|---|
| `InformacionTercero` | NIT, razón social — identifica al **emisor/banco** del producto financiero; no es un `Proveedor` de OXP (entidad financiera, `[D31]`). **Se copia del `ProductoFinanciero` al radicar** (referencia `productoFinancieroId`, `[D39]`, `[R39]`) — ya no se captura por extracto ni se infiere del histórico |
| `ValorMonetario` | Monto, moneda, TRM (si aplica), monto en moneda funcional |
...
| `lineasParaTraduccion()` | Pre-computa una lista plana de líneas, una por cada combinación (componente × destino de negocio), con el valor ya distribuido (valor × porcentaje). `tipoComponente` canónico por componente: `CargoFinanciero` → `cargo_financiero` (tercero = banco/emisor); `AjustePorDiferenciaCambio` → `diferencia_en_cambio_perdida` si su clasificación es gasto financiero (la TRM subió) o `diferencia_en_cambio_ganancia` si es ingreso financiero (la TRM bajó); `AjustePorTolerancia` → `ajuste_tolerancia_perdida` si su dirección es extracto mayor o `ajuste_tolerancia_ganancia` si es extracto menor (ver catálogo canónico de `tipoComponente`). **El signo lo decide OXP** con el dato que la entidad ya tiene — un ajuste emite una sola línea, en el componente de su sentido — y las líneas de ambos ajustes llevan el **tercero del proveedor de la `OxpComercio` de origen**, no el banco/emisor (`[D42]`). Estos componentes no llevan `descripcionConcepto` (su cuenta es autodescriptiva). Además, emite una línea **`cruce_obligacion`** por cada `Vinculacion`, con el **tercero** del proveedor de la `OxpComercio` cruzada, el **valor** de la obligación saldada (`Vinculacion.valorCruzado`, a TRM de radicación — la diferencia con el valor de la partida del extracto ya viaja como ajuste de conciliación por signo: `diferencia_en_cambio_perdida`/`_ganancia` o `ajuste_tolerancia_perdida`/`_ganancia`) y la **distribución de origen** de la compra (`Vinculacion.distribucionOrigen`). El cruce no lleva `descripcionConcepto`. También emite una línea **`partida_por_aclarar`** por cada partida en estado disputa (valor de la partida; tercero = banco/emisor) y una línea **`partida_aclarada`** por cada partida de reverso bancario vinculada a una disputa de un extracto anterior (`R10c`; tercero = banco/emisor) — ver `[D36]`; ninguna de las dos lleva `descripcionConcepto`. La unidad organizacional del cruce **la rinde Contabilidad según `[I33]`** (distribuida con la distribución de origen, consolidada en una unidad general, o sin unidad) — espeja cómo se registró la CxP de la causación original, garantizando el neteo. Las líneas `cruce_obligacion` y `partida_aclarada` viajan con **`referenciaHechoRelacionado`** (a la causación cruzada y a la causación del extracto que abrió la disputa, respectivamente) para que Contabilidad resuelva su **cuenta por espejo** (`[R53]` de Contabilidad, `[D38]`) — el neteo queda garantizado también por cuenta, no solo por unidad. Cada línea lleva su `clasificacion` según las recetas de `[D38]`, y el hecho emite además la **línea `contrapartida`** (tercero = banco/emisor; tipo de producto —el `tipoTarjeta` copiado: `credito` \| `debito_prepago`— + banco/emisor y periodo; sin valor ni unidad). El servicio de Traducción Contable recibe estas líneas y resuelve cada cuenta con espejo o clasificación según el componente. |
...
| `MedioDePago` | VO | Tipo canónico (`[D40]`) + número que identifica el instrumento — en los tipos tarjeta es la identidad (número del producto financiero registrado); opcional en los demás (número del cheque, referencia de la transferencia). La entidad bancaria no hace parte del medio de pago: es el emisor del producto financiero registrado |
...
| `DesgloseFiscal` | Agrupa los cálculos fiscales derivados de un `ConceptoDevuelto`. Inmutable — se reemplaza completo al recalcular. Contiene: `List<Tributo>` de impuestos, `List<Tributo>` de retenciones y `List<Tributo>` de provisiones (`[D41]`). | Comercio |
| `Tributo` | Cálculo fiscal individual (impuesto, retención o provisión). Tipo, base, tarifa, valor. Inmutable. | Comercio |
...
| `lineasParaTraduccion()` | Pre-cómputo de líneas planas (concepto × destino) con valor distribuido. `tipoComponente`: `concepto_devuelto` para cada `ConceptoDevuelto`, más los tributos devueltos con su código (`iva`, `retefuente`, `retefuente_exterior`, `reteiva`, ...) — incluidos los pares de provisión (`[D41]`) y la línea `retencion_asumida` espejo de cada retención que la causación devuelta marcó como asumida (`[D37]`). Las líneas de `concepto_devuelto` llevan `descripcionConcepto` (de `ConceptoDevuelto.descripcion`). Cada línea lleva su `clasificacion` espejo de la causación original y **todas** viajan con `referenciaHechoRelacionado` a la causación devuelta — en `nota_credito_gasto` toda la plantilla (contrapartida incluida) se resuelve **por espejo** (`[R53]` de Contabilidad, `[D38]`). Emite además la línea `contrapartida` (tercero = proveedor; sin valor ni unidad). `tipoTransaccion = nota_credito_gasto` `[D27]`. | Línea por cada `CargoFinancieroDevuelto`, `tipoComponente = cargo_financiero`. `tipoTransaccion = nota_credito_gasto`. | Línea única de la `ReversaTotal`, `tipoComponente = reversa_anticipo`, con `referenciaHechoRelacionado` al anticipo original (espejo del rol ANTICIPO) + línea `contrapartida` (espejo de la CONTRAPARTIDA del anticipo). `tipoTransaccion = reversa_anticipo`. |
...
- **Raíz:** Catálogo de reglas de preferencia de distribución por unidad organizacional, configuradas por la empresa. Materializa el **Nivel A** de la cadena de resolución de la unidad (`[D35]`) — determinístico: resuelve la distribución automáticamente cuando una regla casa, sin requerir confirmación del usuario.
- **Ciclo de vida:** Configuración — sin FSM transaccional.
...
> **Cómo se evalúa (Nivel A de `[D35]`):** ante una transacción, se eligen las reglas activas cuyos criterios definidos coinciden; gana la de **mayor especificidad**; si empatan, desempata el orden de prioridad de criterios (proveedor > tipo de gasto > lugarEjecucion). La unidad resultante se valida contra la copia local de unidades activas (`[SI8]`); si no existe/activa, el componente cae en destino pendiente (`I24`/`[D34]`). El **aprendizaje** (Nivel B, `[SI9]`) solo entra si **ninguna** regla casa.
...
| `medioPagoHabitual` | Opcional: { tipo canónico; `tarjetaHabitual`: referencia a `ProductoFinanciero` (opcional, solo tipos tarjeta) } | **Nivel 3 de la cadena de resolución del medio de pago** (`[R44]`/`[D40]`): configurado deliberadamente por el negocio — sin configuración, el sistema no asume nada por el proveedor; pesa más que lo declarado en el XML del emisor. La tarjeta habitual se valida contra el registro al configurarla; si está Inactiva al radicar, el nivel resuelve solo el tipo y deja advertencia. **No viaja a la bodega de Terceros** — configuración operativa local de OXP, no dato de identidad. |
...
| `ActualizarProveedor` | Actualización explícita de datos (razón social, tipo de persona, identificación, direcciones, contactos, medio de pago habitual). Si cambia un dato de identidad compartido, el cambio viaja a la bodega y puede abrir divergencia allá — comportamiento correcto y deseado. |
...
### Agregado: ProductoFinanciero [F1]

El registro maestro de cada **tarjeta** de la empresa sobre la que se reciben extractos — materializa el componente "Medios de pago" del alcance (`[R39]`, issue #106) y reemplaza la resolución del emisor por inferencia del histórico del planteamiento anterior (issue #57). **El usuario define sus productos financieros y el sistema verifica contra lo definido** — no adivina el comportamiento de cada entidad financiera. OXP es su **dueño provisional**: cuando exista el dueño definitivo (Tesorería o un contexto global), el registro se extrae y OXP lo consume por **copia local por eventos** (patrón `[D34]`/`[SI8]`).

**Identidad:** `productoFinancieroId`. La clave natural es el **número de tarjeta** (I26) — la llave con la que se atribuyen los extractos (`[R41]`).

**Estructura:**

| Componente | Tipo | Contenido |
|---|---|---|
| `numeroTarjeta` | Texto | **Clave natural** — única en el registro (I26) e **inmutable**: la corrección de un número mal digitado es inactivar el producto y registrar uno nuevo. Llave de atribución de extractos `[R41]` |
| `emisor` | `InformacionTercero` | El tercero **entidad financiera** (no un `Proveedor`, `[D31]`). Fuente única del emisor de los extractos (`[R39]`): se **copia al radicar** el OxpExtracto — patrón `[D31]` |
| `tipoTarjeta` | `credito` \| `debito_prepago` | Gobierna qué cargos aplican (intereses solo en crédito, `[R19]`) |
| `configuracionCargos` | Colección | Cargos adicionales que la tarjeta maneja (4x1000, cuota de manejo, intereses) — `[R06]` |
| `plazoAlertaConciliacion` | Días | Plazo de la alerta de conciliación previo a la fecha de pago (default 3) — `[R07]` |
| `cuentaConsolidadora` | Identificación opcional | Identificación de la **cuenta consolidadora** a la que la tarjeta está asociada/amparada — un solo campo: si está, la tarjeta es asociada; el **mismo valor agrupa** las tarjetas de una misma cuenta. La leen dos controles: el **diagnóstico de la salvaguarda de consolidado** (`[R43]`) y la **advertencia al registrar** un producto cuyo número coincida con una cuenta ya anotada (`[R39]` — advertencia, no bloqueo). Semilla real de la evolución (extracto consolidado como fuente de deuda) |
| `estado` | `Activo` \| `Inactivo` | Inactivo ⇒ **advertencia** (no bloqueo) al radicar extractos nuevos — el extracto rezagado de una tarjeta cancelada debe poder procesarse |

> **Omisiones deliberadas (F1):** el **sentido del ajuste por tolerancia** lo decide OXP al conciliar (componente `ajuste_tolerancia_perdida`/`_ganancia`, `[D42]`) y la **cuenta** la resuelve la traducción contable dentro del grupo de cada rol (`AJUSTE_TOLERANCIA_PERDIDA`/`_GANANCIA` del catálogo de plantillas de Contabilidad) — frontera definida en `AjustePorTolerancia` (Sección 6), no una configuración por tarjeta. El **período de facturación y la fecha de pago** son datos de cada extracto (viajan en `ExtractoRadicado`), no del producto. Los **límites y fechas de corte/pago del plástico** son datos maestros del dueño futuro (Tesorería) — ninguna regla de F1 los usa.

**Comandos:**

| Comando | Descripción |
|---|---|
| `RegistrarProductoFinanciero` | **Única vía de creación** — registro deliberado, previo a radicar extractos. Valida la unicidad del número de tarjeta (I26). Si el número coincide con una `cuentaConsolidadora` anotada en otro producto, **advierte** (no bloquea): probablemente se está registrando la cuenta consolidadora como si fuera una tarjeta — el camino a duplicar la deuda (`[R39]`). Invocable desde la resolución de la bandeja de pendientes de atribución (`[R41]`): registrar el producto que faltaba y continuar la radicación. |
| `ActualizarProductoFinanciero` | Actualización explícita de datos (emisor, tipo, configuración de cargos, plazo de alerta, `cuentaConsolidadora`). El **número de tarjeta no se actualiza** — es la clave natural (I26); su corrección es inactivar y registrar de nuevo. El evento captura **delta**. Las radicaciones futuras copian el dato corregido; los extractos históricos no se reescriben. |
| `InactivarProductoFinanciero` / `ReactivarProductoFinanciero` | Con motivo. La inactivación no bloquea extractos rezagados — genera advertencia al radicar. |

> **Bandeja de pendientes de atribución (`[R41]`/`[R43]`)** — capa de aplicación, no parte del agregado: recibe el extracto cuyo número de tarjeta no está registrado y el archivo con partidas de varios números de tarjeta (salvaguarda de consolidado). Cuando las tarjetas del archivo están anotadas con la misma `cuentaConsolidadora`, la alerta incluye el **diagnóstico**: el archivo es probablemente el consolidado de esa cuenta (`[R43]`). Se resuelve registrando el producto, corrigiendo la atribución o gestionando los extractos individuales; el documento continúa después a la radicación normal. La resolución **pre-llena** el registro con los datos interpretados del archivo (número de tarjeta; emisor si el archivo lo trae) — **asistencia de captura**, como la del `Proveedor`: el usuario confirma o corrige, el registro nunca se crea solo. No es una cadena de inferencia — el sistema no decide nada: detiene y pregunta **una sola vez**; desde entonces los extractos de esa tarjeta se atribuyen solos. El extracto **nunca se pierde ni bloquea el flujo** — mismo criterio detecta-no-exige del canal automático de `[R38]`.

### Value Objects compartidos

`InformacionTercero` y `ValorMonetario` son Value Objects reutilizados por los cuatro agregados transaccionales. Desde `[D31]`, en OxpComercio, Anticipo y Devolucion el `InformacionTercero` **se copia del agregado `Proveedor` al radicar** (referencia `proveedorId`) — una sola fuente del dato dentro de OXP; en OxpExtracto identifica al emisor/banco, que no es un Proveedor. `MedioDePago` aplica a OxpComercio y Anticipo (Devolucion no lo requiere — hereda implícitamente el medio de pago del agregado OXP origen; OxpExtracto tampoco lo lleva: el extracto **es** la deuda de la tarjeta y se paga por tesorería — su origen lo modela la referencia `productoFinancieroId`, no un medio de pago). Cada agregado los incluye en su composición pero la definición es la misma — evita duplicación de estructuras de datos sin acoplar los agregados. Desde `[D39]`, en OxpExtracto el `InformacionTercero` del emisor y el `tipoTarjeta` **se copian del `ProductoFinanciero` al radicar** (referencia `productoFinancieroId`); en OxpComercio el `MedioDePago` se resuelve por la **cadena de resolución** con origen rastreado (`[R44]`/`[D40]`) — la tarjeta queda **referenciada** (`productoFinancieroId` + últimos 4 copiados) cuando se resolvió contra el registro, y la establece en firme la conciliación.
...
**Priorización por tarjeta (`[R10]` `[R44]`):** en el modo asistido, la candidata cuya **tarjeta referenciada** (`productoFinancieroId`) coincide con el producto del extracto se prioriza primero; la coincidencia de **últimos 4 sueltos**, después — nunca excluyen (la mayoría de compras no traen el dato; puede venir mal extraído; una tarjeta reemplazada cambia de número). **Coherencia del medio de pago:** no requiere evento ni comportamiento adicional del servicio — la vinculación de una OXP causada como no-tarjeta se hace visible sola: su saldo quedó **neto de retenciones** frente al **total** de la partida, y esa diferencia guía la corrección operativa; el doble pago no puede consumarse porque los pagos por extracto y por tesorería compiten por el mismo `saldoPorPagar()`, que solo se consume una vez (`[D40]`).
...
### [SI9] Aprendizaje de la unidad organizacional → proyección "unidad frecuente por combinación"

Materializa el **Nivel B** de la cadena de resolución de la unidad (`[D35]`) — la sugerencia por aprendizaje.
...
### [SI10] Unicidad del extracto por producto y período → proyección con constraint compuesto

Materializa `[R42]`/I27. Una proyección indexa los extractos radicados por (`productoFinancieroId`, período) con constraint único — el reintento del mismo documento (reenvío del correo, recarga del archivo) se reconoce y se descarta **sin alerta**, antes de crear el stream. Análoga a `[SI4]` (unicidad de obligación). La detección de la salvaguarda de consolidados (`[R43]`) no requiere proyección: se evalúa sobre el propio archivo (partidas con más de un número de tarjeta) en la capa de aplicación, antes de radicar.
...
**ProductoFinanciero:** OxpExtracto lo referencia (N:1) vía `productoFinancieroId` — es la fuente del `InformacionTercero` del emisor y del `tipoTarjeta` que copia al radicar, y de la configuración de radicación/conciliación (cargos `[R06]`, plazo de alerta `[R07]`). Espeja el patrón Proveedor → transacciones (`[D31]`): referencia a la raíz + copia del dato al evento — el hecho económico queda completo e inmutable.
...
> **Eventos de progreso del control de doble pago (`[R38]`):** `ConstanciaAnticipoNoAplicableRegistrada` aplica en Pendiente, Confirmada y Causada (con `saldoPorPagar()` > 0); `DoblePagoPotencialDetectado` aplica en Causada. Ninguno cambia el estado.
...
- `PlazoAnticipoVencido` es evento informativo sin cambio de estado `[R04b]`. Aplica en estados Causada o Pagado (`saldoPorRegularizar()` > 0).
...
### 4.6. ProductoFinanciero

Nace `Activo` (vía `RegistrarProductoFinanciero` — registro deliberado, previo a los extractos). Los datos se siguen actualizando en cualquier estado. `Inactivo` no bloquea la radicación de extractos rezagados — genera advertencia (`[R41]`).

```
             ProductoFinancieroRegistrado
                        │
                        ▼
                 ┌─────────────┐   ProductoFinancieroInactivado    ┌─────────────┐
                 │   ACTIVO    │ ────────────────────────────────► │  INACTIVO   │
                 │             │ ◄──────────────────────────────── │             │
                 │             │   ProductoFinancieroReactivado    │             │
                 │ · ProductoFinancieroModificado                  │ · ProductoFinancieroModificado
                 └─────────────┘                                   └─────────────┘

  Un producto Inactivo genera advertencia (no bloqueo) al radicar extractos nuevos —
  el extracto rezagado de una tarjeta cancelada debe poder procesarse.
```
...
| **Información capturada** | `proveedorId` (referencia al Proveedor `[D31]`); tercero (NIT, razón social — copiado del Proveedor), fecha de transacción, valor en moneda original, moneda, TRM del día si aplica `[R05b]`, valor en moneda funcional, número de soporte/factura, medio de pago resuelto por la cadena (`[R44]`/`[D40]`: tipo canónico + origen de la resolución + tarjeta — { `productoFinancieroId`, últimos 4 } si se resolvió contra el registro, últimos 4 sueltos como evidencia si no), conceptos (gasto/costo + impuestos + retenciones — con su marca de asunción y origen `[D37]` — + provisiones `[D41]`) con clasificacionTributaria y conceptoPago por cada concepto (resueltos desde el catálogo del sub-dominio origen o del catálogo de gasto directo de OXP), subDominioOrigen `[SI5]`, distribución de costos si aplica `[R05c]`, soportes documentales adjuntos. |
| **Efectos** | Solicitud de cálculo al sub-dominio de Impuestos con el contexto transaccional completo (conceptos con clasificacionTributaria y conceptoPago, entidades fiscales, ubicaciones, fecha, moneda, direccionFiscal = gasto). El DesgloseFiscal propuesto se asigna a cada ConceptoDeGasto. Si el soporte trae tributos del proveedor, se validan contra el cálculo de Impuestos `[R37]` — las discrepancias se presentan al usuario para decisión. Si XML: extracción automática de datos desde SincoRE. Si requiere formalización: notificación a SincoADPRO `[R20]`. Si supera monto máximo: alerta informativa `[R05]`. Si compra del exterior o sujeto no obligado a facturar: alerta de plazo DIAN para Documento Soporte en Adquisiciones (6 días hábiles) `[R01]` — el documento lo emite SincoFE; OXP controla que haya sido emitido. Si `[R02]` automática **y la resolución del medio de pago es fuerte** (`capturado` \| `deducido_soporte` \| `default_proveedor` — I28): emite `OxpComercioConfirmada`; con resolución **débil** (`sugerido_xml` \| `asumido_por_defecto`) la OXP queda **retenida en Pendiente** con motivo "medio de pago por confirmar" — bandeja de pendientes de confirmación, pre-llenada para resolver con un clic (`[R44]`). |
...
| **Precondiciones** | Archivo de extracto válido (PDF o CSV). Producto financiero **atribuido** por número de tarjeta (`[R41]` — no atribuible → bandeja de pendientes de atribución, no radica). Partidas de **una sola tarjeta** (varios números de tarjeta ⇒ salvaguarda de consolidado `[R43]`: bandeja + alerta, no radica). Unicidad (producto financiero, período) superada — el reintento del mismo documento se descarta sin alerta (`[R42]`, I27, `[SI10]`). Producto Inactivo ⇒ **advertencia**, no bloqueo. |
| **Información capturada** | `productoFinancieroId` (referencia al `ProductoFinanciero` atribuido `[R41]`); entidad bancaria emisora (`InformacionTercero`) **copiada del producto** (`[D39]`, `[R39]`); tarjeta (número — llave de la atribución); `tipoTarjeta` (`credito` \| `debito_prepago`) **copiado del producto** — alimenta la clasificación de la línea `contrapartida` (`[D38]`) y queda inmutable en el hecho (las correcciones del producto no reescriben extractos históricos); período; **fecha de pago** (del archivo — sobre ella opera la alerta de plazo `[R07]`); moneda del extracto `[R05d]`; partidas del extracto (descripción, valor en moneda del extracto, moneda original, valor original, TRM si aplica `[R05d]`, fecha por cada una); cargos adicionales detectados según la configuración del producto `[R06]` `[R19]`. Distribución de costos: se establece por componente individual (`CargoFinanciero`, `AjustePorDiferenciaCambio`, `AjustePorTolerancia`) usando preferencia de empresa o instrucción explícita — sin herencia entre componentes (ver I10). Los ajustes se distribuyen cuando se generan durante la conciliación. |
...
| **Precondiciones** | Usuario con rol de Confirmador `[R23]`. Confirmador diferente al Radicador `[R25]` (excepto Ramas B/C — confirmación automática). Anticipo en estado Vigente. **Distribución por unidad organizacional asignada (I29):** sin asignación no hay confirmación — en canal automático el anticipo queda retenido en Vigente y la confirmación se re-dispara al resolverse; el pendiente de propagación de la copia local **no bloquea** (`[D34]` — difiere la causación). |
...
| **Efectos** | La partida cuenta como conciliada para alcanzar el 100% `[R06]`. Permite avanzar sin generar anticipos. Resolución posterior vía `PartidaEnDisputaDescartada` o `PartidaEnDisputaReclasificada`. Contablemente no emite nada en este momento: la línea `partida_por_aclarar` viaja cuando el extracto se causa (`ExtractoCausado`, `[D36]`). |
...
| **Efectos** | Cierra el ciclo de la partida en disputa. Contablemente, la cancelación de la transitoria viaja como línea `partida_aclarada` dentro de la causación del extracto donde llegó el reverso (`[D36]`) — este evento no emite integración propia. |
...
| **Información capturada** | Referencia a la nueva OxpComercio, líneas de traducción de la reclasificación (`[D36]`), fecha de resolución, usuario. |
| **Efectos** | Sistema vincula la partida del extracto original con la nueva OxpComercio. Sin generar documentos duplicados ni nueva deuda `[R06b]`. **Integración saliente (`[D36]`):** emite `tipoTransaccion = reclasificacion_partida` con dos líneas — Db CxP del proveedor de la nueva OxpComercio (`cruce_obligacion`, tercero = proveedor) y Cr transitoria de partidas por aclarar (`partida_aclarada`, tercero = banco/emisor); `terceroPrincipal` = banco/emisor; sin contrapartida del motor. Espera `EntregaAceptada` para registrar la `referenciaDestino`, como toda causación (`[D28]`/`[SI6]`). |

#### PlazoConciliacionVencido
...
| **Estado previo** | Parcialmente Conciliada. |
| **Estado resultante** | (sin cambio de estado; es evento informativo). |
| **Precondiciones** | Plazo configurado excedido `[R07]` (default 3 días previos a fecha de pago, configurable por producto financiero). |
...
| **Estado previo** | Pendiente. |
| **Estado resultante** | Confirmada. |
| **Precondiciones** | Usuario con rol de Confirmador `[R23]`. Confirmador diferente al Radicador `[R25]`. OXP en estado Pendiente con soportes completos. **Verificación del medio de pago (`[R44]`, I28):** si el origen de la resolución es débil (`sugerido_xml` \| `asumido_por_defecto`), la confirmación exige la decisión del usuario según el modo configurado por empresa (bloqueo / decisión obligatoria); en el canal automático la OXP permanece retenida en Pendiente hasta la decisión. **Verificación temprana de doble pago (`[R38]` `[I23]`):** si el Proveedor tiene anticipos con `saldoPorRegularizar()` > 0 no resueltos para esta OXP (sin regularización aplicada ni constancia registrada), el sistema los destaca y exige la decisión según el modo configurado por empresa — bloqueo (no confirma sin resolver) o decisión obligatoria (aplicar la regularización, que lleva a la amortización embebida `[D26]`, o dejar constancia). **Verificación de la distribución (I29):** ningún componente con destino único pendiente **de asignación** — en canal automático la OXP queda retenida en Pendiente y la confirmación se re-dispara al resolverse; el pendiente de **propagación** de la copia local no bloquea (`[D34]` — difiere la causación de esa parte). |
...
| **Estado resultante** | Confirmada. |
| **Precondiciones** | Extracto en estado Conciliada (100%) `[R11]`. Usuario con rol de Confirmador `[R23]`. Distribución de los componentes asignada (I29 — el pendiente de propagación de la copia local no bloquea, `[D34]`). |
| **Información capturada** | Usuario confirmador, fecha y hora de confirmación. |
...
| **Información capturada** | Referencia de pago del sistema contable (incluye identificador del destino físico que originó el pago, ej: número de transacción SincoA&F), valor pagado, fecha. |
| **Efectos** | Crea entidad `PagoAplicado` (tipo: pago_directo). Reduce `saldoPorPagar()`. **Detección de doble pago (`[R38]` `[I23]`):** el pago directo es un hecho consumado (el dinero ya salió del sistema contable) — el control no puede prevenir en este canal: si el Proveedor tiene anticipos abiertos sin resolver, el pago se aplica igual y se emite `DoblePagoPotencialDetectado` (derivado por configuración). La prevención de este canal vive en la verificación temprana de la confirmación. |
...
| **Agregado** | OxpComercio |
| **Estado previo** | Causada. |
| **Estado resultante** | Sin cambio (progreso — evento informativo, mismo patrón de `PlazoAnticipoVencido`). |
...
| **Estado previo** | Pendiente. |
| **Estado resultante** | Confirmada. |
| **Precondiciones** | Devolucion en estado Pendiente. Agregado OXP origen existe y cumple precondiciones según tipo (ver `DevolucionRadicada`). Distribución asignada (I29 — el pendiente de propagación de la copia local no bloquea, `[D34]`). |
...
| **Información capturada** | Campos modificados (delta): de `razonSocial`, `tipoPersona`, `identificacionLegal`, `direcciones`, `contactos`, `medioPagoHabitual` — solo los que cambiaron, con identificadores. (El `medioPagoHabitual` es configuración operativa local: el evento estándar de rol hacia la bodega no lo incluye.) |
...
Eventos del agregado de configuración que materializa el **Nivel A** de la cadena de resolución de la unidad organizacional (`[D35]`). Ciclo de vida CRUD, sin FSM.
...
> Una regla **promovida desde el aprendizaje** (Nivel B → Nivel A, `[D35]`/`[SI9]`) entra por `ReglaDeDistribucionAgregada` como cualquier otra regla. La invalidación de un aprendizaje vive en la proyección de aprendizaje (`[SI9]`), no aquí.

### 5.10. ProductoFinanciero (registro)

Eventos del registro de productos financieros — el componente Medios de pago del alcance (`[R39]`, `[D39]`).

#### ProductoFinancieroRegistrado

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Una tarjeta de la empresa ha sido registrada como producto financiero, con su emisor y su configuración. |
| **Agregado** | ProductoFinanciero |
| **Estado previo** | (nuevo) — no existía previamente. |
| **Estado resultante** | Activo. |
| **Precondiciones** | Número de tarjeta único en el registro (I26). Emisor capturado como **tercero (entidad financiera)** — no es un `Proveedor` (`[D31]`). Número coincidente con una `cuentaConsolidadora` anotada en otro producto ⇒ **advertencia**, no bloqueo (`[R39]`). |
| **Información capturada** | Número de tarjeta (clave natural), emisor (`InformacionTercero`), tipo (`credito` \| `debito_prepago`), configuración de cargos adicionales `[R06]`, plazo de alerta de conciliación `[R07]`, `cuentaConsolidadora` (opcional — identificación de la cuenta a la que la tarjeta está asociada, `[R39]`). |
| **Efectos** | El producto queda disponible como destino de atribución de extractos (`[R41]`) y como fuente del emisor y la configuración al radicar (`[R39]`). |

#### ProductoFinancieroModificado

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | Los datos del producto financiero han sido actualizados. |
| **Agregado** | ProductoFinanciero |
| **Estado previo** | Activo o Inactivo (los datos se actualizan en cualquier estado). |
| **Estado resultante** | (sin cambio de estado). |
| **Precondiciones** | El número de tarjeta **no cambia** — es la clave natural (I26) e identidad del registro; la corrección de un número mal digitado es **inactivar el producto y registrar uno nuevo**. |
| **Información capturada** | **Delta:** `productoFinancieroId` + campos modificados (emisor, tipo, configuración de cargos, plazo de alerta, `cuentaConsolidadora`). |
| **Efectos** | Las radicaciones futuras copian el dato corregido (`[D39]`); los extractos históricos no se reescriben — el hecho queda como fue. |

#### ProductoFinancieroInactivado

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | La tarjeta dejó de usarse (cancelada, reemplazada, vencida). |
| **Agregado** | ProductoFinanciero |
| **Estado previo** | Activo. |
| **Estado resultante** | Inactivo. |
| **Precondiciones** | — |
| **Información capturada** | `productoFinancieroId`, motivo. |
| **Efectos** | Los extractos nuevos del producto generan **advertencia** al radicar — no bloqueo: el extracto rezagado de una tarjeta cancelada debe poder procesarse. |

#### ProductoFinancieroReactivado

| Aspecto | Detalle |
|---------|---------|
| **Descripción** | La tarjeta vuelve a estar en uso. |
| **Agregado** | ProductoFinanciero |
| **Estado previo** | Inactivo. |
| **Estado resultante** | Activo. |
| **Precondiciones** | — |
| **Información capturada** | `productoFinancieroId`, motivo. |
| **Efectos** | La radicación de extractos del producto deja de generar advertencia. |
...
| **Tercero (traducción)** | El **proveedor de la OxpComercio de origen** — el ajuste es la diferencia sobre esa obligación, como el cruce que la salda; no el banco/emisor del extracto `[D42]`. |
| **Traducción contable (frontera)** | **OXP decide el sentido** y emite un solo componente: `diferencia_en_cambio_perdida` (Db) si la TRM subió o `diferencia_en_cambio_ganancia` (Cr) si bajó. El servicio de Traducción Contable resuelve la cuenta dentro del grupo del rol correspondiente (gasto o ingreso financiero); no infiere el sentido `[D42]`. |
...
| **Tercero (traducción)** | El **proveedor de la OxpComercio de origen** — la diferencia es sobre esa obligación; no el banco/emisor del extracto `[D42]`. |
| **Traducción contable (frontera)** | **OXP decide el sentido** y emite un solo componente: `ajuste_tolerancia_perdida` (Db, gastos bancarios) si extracto > OxpComercio o `ajuste_tolerancia_ganancia` (Cr, aprovechamientos) si extracto < OxpComercio. El servicio de Traducción Contable resuelve la cuenta dentro del grupo del rol correspondiente; no infiere el sentido `[D42]`. |
...
| I10 | **Consistencia de distribución:** Toda `InstruccionDistribucion` referencia un componente existente del agregado. **OxpComercio:** `ConceptoDeGasto` o `Tributo`; cadena de resolución (`[D35]`): instrucción explícita → herencia del gasto padre → reglas de preferencia de distribución (Nivel A, determinístico) → sugerencia por aprendizaje (Nivel B, no vinculante) → destino único pendiente. **Devolucion tipo Comercio:** `ConceptoDevuelto` o `Tributo`; misma cadena de resolución. **OxpExtracto:** `CargoFinanciero`, `AjustePorDiferenciaCambio` o `AjustePorTolerancia`; sin herencia (cada componente tiene instrucción propia o resuelve por reglas de preferencia `[D35]`). **Anticipo:** instrucción única sobre el valor global (sin desglose `[P1]`); reglas de preferencia `[D35]` o destino único pendiente. Al agregar, eliminar o recalcular componentes, el agregado mantiene la coherencia. **Dos pendientes distintos:** el pendiente **de asignación** (ninguna regla resolvió y el usuario no ha asignado) se resuelve a más tardar en la **confirmación** — sin asignación no hay confirmación (I29); el pendiente **de propagación** (`I24`, `[D34]`: la unidad asignada existe en la fuente de verdad pero su evento aún no llega a la copia local, o está inactiva en la copia) **no bloquea la confirmación** — la causación de esa parte se difiere y se libera sola cuando llega `UnidadCreada`/`UnidadActivada` (sin aproximar con unidad provisional). | OxpComercio, OxpExtracto, Anticipo, Devolucion | `[R05c]` |
...
| I23 | **Control de doble pago con anticipos abiertos** — condicionada por configuración de empresa (como I6; modos: bloqueo / decisión obligatoria): **(a) al confirmar** una OxpComercio cuyo Proveedor tiene anticipos con `saldoPorRegularizar()` > 0 **no resueltos** para esta OXP, el sistema exige la decisión según el modo; **(b) al pagar:** la vinculación con extracto (iniciada por usuario) no procede mientras exista anticipo abierto sin resolver; el pago directo (hecho consumado, automático) se aplica siempre y emite `DoblePagoPotencialDetectado`. **"Resuelto"** = regularización aplicada a esta OXP **o** `ConstanciaAnticipoNoAplicable` registrada para ese anticipo en esta OXP. | OxpComercio + Anticipo (lectura) | `[R38]` `[D33]` |
...
| I25 | **Coherencia de las reglas de distribución:** En `CatalogoReglasDistribucion`, la `distribucion` de toda `ReglaDeDistribucion` suma exactamente 100% entre sus `DestinoDeNegocio` (gemela de `I2` a nivel de regla), y no existen dos reglas **activas** con la misma combinación exacta de criterios (`criterioProveedor`, `criterioTipoGasto`, `criterioLugarEjecucion`) — así el desempate por especificidad/prioridad de `[D35]` siempre tiene una ganadora única. | CatalogoReglasDistribucion | `[D35]` |
| I26 | **Unicidad del producto financiero (eventual):** no existen dos `ProductoFinanciero` con el mismo `numeroTarjeta` — la atribución de extractos (`[R41]`) siempre tiene a lo sumo un destino. Proyección con constraint único sobre la clave natural, análoga a `[SI7]`. | ProductoFinanciero | `[R39]` `[R41]` |
| I27 | **Unicidad del extracto por producto y período (eventual):** solo puede existir un extracto radicado por (producto financiero, período); el mismo documento reenviado o recargado se descarta como reintento idempotente, sin alerta (`[SI10]`). | OxpExtracto + ProductoFinanciero (lectura) | `[R42]` |
| I28 | **Ninguna confirmación con medio de pago de resolución débil** — condicionada por configuración de empresa (modos: bloqueo / decisión obligatoria, como I23): una OxpComercio cuyo medio de pago tiene origen `sugerido_xml` o `asumido_por_defecto` no se confirma sin decisión humana — en el canal automático queda **retenida en Pendiente** (motivo "medio de pago por confirmar", bandeja de pendientes de confirmación); en el canal manual la confirmación exige el pronunciamiento. Cierra estructuralmente el doble pago del caso "tarjeta asumida como crédito": sin confirmación no hay causación, y el pago directo exige Causada (`[R35]`). | OxpComercio | `[R44]` `[D40]` |
| I29 | **Ninguna confirmación con distribución sin asignar.** La confirmación es la última puerta humana antes de la causación: si algún componente del documento tiene su distribución por unidad organizacional **sin asignar** (ninguna regla de la cadena `[D35]` resolvió y el usuario no la ha asignado — destino único pendiente **de asignación**, ver I10), el documento **no se confirma**. En el canal manual la confirmación exige resolver la distribución; en el canal automático el documento queda **retenido** en su estado previo y la confirmación automática (`[R02]`/`[R12]`) se **re-dispara** al resolverse la distribución. Mismo patrón de I28 (medio de pago de resolución débil). **No aplica al pendiente de propagación (`[D34]`):** una unidad **asignada** que existe en la fuente de verdad pero cuyo evento aún no llega a la copia local (o está inactiva en la copia) **no bloquea la confirmación** — la causación de esa parte se difiere y se libera sola al llegar `UnidadCreada`/`UnidadActivada`. | OxpComercio, OxpExtracto, Anticipo, Devolucion | `[R11]` |
...
| D33 | **El control de doble pago (`[R38]`) se materializa con constancia humana y comportamiento por canal** (issue #30, descubierto al implementar y validar el #26 con tests E2E). (1) **La pertenencia anticipo↔OXP nunca es calculable por el sistema**: el registro del anticipo no referencia factura, orden ni compra esperada — solo Proveedor, valor, medio de pago y fecha; la regularización es el único vínculo y su único filtro de sistema es el mismo Proveedor (el emparejamiento lo decide el humano, `[R29]` 1:N). Por eso la **constancia** ("este anticipo no corresponde a esta OXP") es el único mecanismo válido cuando R38 dispara, y forzar "aplicar" sería peligroso (aplicar el anticipo equivocado es trivial y el sistema no puede impedirlo por datos). (2) **La constancia se puede registrar en cualquier estado antes de saldarse** (Pendiente, Confirmada, Causada con saldo) — sin esto, el anticipo que aparece después de causar dejaba a la OXP bloqueada de forma permanente al pagar, sin salida. (3) **El refuerzo al pagar es por canal**: la conciliación vía extracto la inicia un usuario — hay humano para resolver en el acto y el cruce se bloquea hasta resolver; el pago directo es automático y registra un pago que **ya ocurrió** en el sistema contable (`[R35]`, `PagoAplicado` tipo pago_directo) — rechazarlo dejaría a OXP negando la realidad, así que el canal **detecta y alerta** (`DoblePagoPotencialDetectado`), y su prevención real vive en la verificación temprana de la confirmación. | El #26 entregó la regla en el alcance pero el modelo nunca recibió el diseño (0 eventos / 0 invariantes); la implementación lo construyó sin respaldo y los tests E2E destaparon el escenario sin salida y la asimetría de canales. | DDD: el juicio humano se modela como hecho explícito e inmutable (la constancia), no como bandera; EDA: los hechos consumados no se rechazan — se registran y se compensa operativamente. |
| D34 | **La unidad organizacional es un dato gobernado por Estructura Organizacional (dueño único); OXP la consume como copia local, no como agregado** (replanteamiento #45, issue #48). OXP no crea, modifica ni gobierna unidades — solo las usa para distribuir e imputar. Mantiene una **copia local** (read model) por suscripción a los eventos de ciclo de vida de EO (`[SI8]`), opera y valida siempre contra ella (`I24`) y **nunca consulta a EO en caliente**. La copia es para **validación del dominio, no una API de lectura para la UI** (la interfaz lee unidades directamente de EO, fuente de verdad). Como la unidad se elige de la fuente de verdad, una unidad resuelta por la cadena (`I10`) existe en EO; si su evento aún no llegó a la copia (desfase de propagación) o está inactiva, OXP **difiere** la causación de esa parte vía *destino pendiente* hasta que llegue `UnidadCreada`/`UnidadActivada` — sin aproximar con unidad provisional ni de tránsito (desconciliaría con contabilidad). El diferir aplica al pendiente de **propagación** — nunca bloquea la confirmación; el pendiente de **asignación** humana es otro caso: se exige a más tardar en la confirmación (I29). Reacciona a las reestructuraciones de EO (fusión/división/traslado) reasignando referencias futuras y conservando el histórico. **OXP no emite eventos salientes hacia EO** (la señal de demanda `DemandaDeUnidadSenalada` se retiró en el #72: con la unidad elegida de la fuente de verdad, referenciar una inexistente no ocurre en operación). **Contraste con el `Proveedor` (`[D30]`):** el Proveedor es un rol que OXP co-gobierna (agregado, `AsegurarProveedor`); la unidad tiene dueño único externo → solo copia local. **Nota:** la *asignación automática* de la unidad (qué unidad le corresponde a cada gasto) se resuelve con la cadena de niveles de `[D35]` (instrucción → herencia → reglas configurables → sugerencia por aprendizaje → pendiente). | Sin copia local, OXP dependería de la disponibilidad de EO en cada distribución (acoplamiento de ejecución que el replanteamiento #45 elimina) o re-gobernaría un dato ajeno (el consumidor volviéndose dueño). El diferir preserva la consistencia exacta operación↔contabilidad sin bloquear la radicación. | DDD: un dato con dueño único externo se consume por réplica local (read model), no se modela como agregado. EDA: copia local por eventos, nunca consulta ni escritura remota. Fundamento en `guias-de-modelado/datos-entre-dominios.md` y `[D13]` de Estructura Organizacional. |
| D35 | **Asignación de la unidad organizacional por una cadena de resolución de niveles** (issue #51). La distribución por unidad deja de resolverse solo con una preferencia global de empresa (gruesa) y pasa a una cadena con dos niveles nuevos: **Nivel A — reglas de preferencia de distribución** (configurables en `CatalogoReglasDistribucion`, **determinísticas**): criterios combinables `proveedor`/`tipo de gasto`/`lugarEjecucion`, resueltas por **especificidad** (más criterios gana) con desempate por prioridad (proveedor > tipo > lugarEjecucion); la regla sin criterios es la preferencia general (preserva el comportamiento anterior). Resuelven **automáticamente** sin confirmación — el usuario las configuró — y **cubren el arranque en frío** (un tenant nuevo configura reglas desde el día 1, sin esperar histórico). **Nivel B — sugerencia por aprendizaje** (`[SI9]`, **no vinculante**): cuando ninguna regla casa, se pre-llena con la unidad más frecuente del histórico para esa combinación; el usuario confirma → pasa a instrucción explícita; promovible a regla e invalidable. La cadena completa: instrucción explícita → herencia del gasto padre → Nivel A → Nivel B (sugerencia) → destino pendiente. Cada distribución registra **con qué nivel** se resolvió (trazabilidad). Todo opera sobre la **copia local de unidades activas** (`[SI8]`, `[D34]`): nunca se propone una unidad inexistente/inactiva; si la resuelta no existe aún, se difiere. **Límite (lo determinístico vs lo inferido):** lo vinculante/automático es solo lo determinístico (explícita, herencia, reglas configuradas); lo inferido por aprendizaje **siempre** pasa por confirmación humana — una inferencia no pisa una regla que el negocio configuró. | El aprendizaje por sí solo no resuelve el arranque en frío (sin histórico no sugiere) y es inferencia (riesgoso para un dato que debe coincidir exacto con la contabilidad); las reglas determinísticas dan certeza y resolución desde el día 1. La estructura de dos niveles (determinístico + aprendizaje) sigue el mismo patrón de la cadena de resolución de cuentas del sub-dominio Contabilidad. | DDD: la asignación es lógica de OXP (capa de aplicación para la sugerencia, agregado de configuración para las reglas). Extiende `[D23]` (clasificación inteligente) a la dimensión unidad. |
| D36 | **Ciclo contable de la partida en disputa mediante cuenta transitoria de "partidas por aclarar"** (issue #90; complementa `[D29]`). El banco cobra también el valor de la partida en disputa: es parte de la deuda real del extracto. **(1) Al causar el extracto**, cada partida en disputa emite una línea `partida_por_aclarar` (Db a la transitoria; grupo tentativo 1360 Reclamaciones — recomendada: la disputa es materialmente una reclamación al banco/emisor — o 1380 Deudores varios) para que la contrapartida (CxP del banco/emisor) refleje el **total real**; sin ella el pasivo queda subvalorado — el mismo hueco de partida doble que el #18 cerró para el cruce. **(2) Resolución por descarte** (`R10c`): el reverso bancario llega en un extracto futuro y la cancelación viaja como línea `partida_aclarada` (Cr transitoria) dentro de la causación de ese extracto. **(3) Resolución por reclasificación** (`R06b`): `PartidaEnDisputaReclasificada` emite `tipoTransaccion` propio `reclasificacion_partida` — Db CxP del proveedor identificado (`cruce_obligacion`; su compra ya fue pagada vía extracto) · Cr transitoria (`partida_aclarada`) — **sin contrapartida del motor** (sus dos líneas viajan explícitas), mismo criterio del Caso B de `[D26]`: el hecho ocurre después de las causaciones y no hay dónde embeberlo. **Terceros:** la transitoria lleva siempre el **banco/emisor** (la reclamación es contra él — abre y cierra contra el mismo tercero, conciliable por tercero); las CxP llevan su **proveedor**; `terceroPrincipal` de la reclasificación = banco/emisor (informativo). Los nombres del contrato son **generales de contabilidad** ("por aclarar"/"aclarada"), no "disputa" (vocabulario interno de OXP): cualquier dominio futuro que necesite transitorias reutiliza los mismos roles. En ambos caminos la transitoria **abre y cierra**, auditable por partida y por tercero. Cuentas exactas `porValidar` por consultor contable (ítems 11-12 del catálogo de plantillas). | Sin la línea, la CxP del banco queda subvalorada frente a lo que el banco cobra y el pago del extracto no coincide con el pasivo registrado; y las resoluciones dejaban el valor en el limbo (la reclasificación declaraba "reclasificación contable aplicada" sin emitir nada). La cuenta transitoria de partidas por aclarar es la práctica contable estándar para valores en aclaración. | Coordinación cruzada con Contabilidad: catálogo de plantillas v1.7 (roles `PARTIDA_POR_ACLARAR`/`PARTIDA_ACLARADA` + plantilla `reclasificacion_partida`) y anexo de ejemplos v1.4 (Ejemplo 5). Análogo al #18 (`cruce_obligacion`/`[D29]`). |
| D37 | **Retenciones asumidas cuando el pago ya salió completo — la responsabilidad es de OXP y el criterio es el medio de pago** (issue #94). La retención se practica según las reglas tributarias **independientemente del medio de pago** (doctrina de concurrencia — conceptos DIAN 2023-2025; la retención del 1,5% que la entidad emisora le practica al comercio es del proveedor y no sustituye ni afecta este registro). Lo que cambia con el medio de pago es **quién la financia**: cuando el pago del documento ya se realizó completo por el sistema financiero (medio de pago de tipo tarjeta — se pagó el valor total en el datáfono y no hubo de dónde descontar), las retenciones del desglose (**todas** las sustractivas a cargo del comprador) **no se descuentan del valor a pagar**: la empresa las **asume**. **Mecánica en `lineasParaTraduccion()`** (el responsable es OXP — único que conoce el medio de pago; Impuestos liquida sin conocerlo y Contabilidad solo mapea): la línea de cada retención viaja **idéntica** al caso normal (Cr, mismo componente y cuenta, tercero = proveedor — se declara y certifica igual); cuando es asumida se emite **además** una línea `retencion_asumida` (Db, espejo del valor — gasto propio) por cada retención, y el valor a pagar queda por el total. Rol espejo `IMPUESTO_ASUMIDO` en la plantilla `causacion_gasto` de Contabilidad (tentativo `["5315"]`, `porValidar`). El proveedor autorretenedor (no se le deduce nada) ya lo resuelve el catálogo de Impuestos (`esAutorretenedora → noAplicar`). **Lo diferido aquí (canonización del enum, captura al radicar con sugerencia del XML, alerta de coherencia con el extracto) quedó resuelto en `[D40]` (issue #96).** **Segunda causa de asunción (v4.16, issue #143):** la **retención a beneficiarios del exterior** (`RETEFUENTE_EXTERIOR` de Impuestos, `[D17]` de su modelo — proveedor sin residencia ni domicilio fiscal en el país, 20 % general o tarifa de convenio) nace **asumida por defecto**, cualquiera que sea el medio de pago: en la práctica el proveedor del exterior recibe el total. El usuario puede **desmarcarla** en el documento antes de causar cuando el contrato permite descontarla — entonces viaja como cualquier retención y sí resta del valor a pagar. La marca de asunción vive en el `Tributo` del desglose con su origen (`medio_de_pago` / `beneficiario_exterior` / `manual`). Misma mecánica de líneas; ningún componente nuevo salvo `retefuente_exterior` (el código del tributo). Se descartó modelarla en Impuestos como tributo de provisión "siempre asumido": la asunción no es obligación normativa sino práctica comercial, y esta decisión ya le asignó a OXP quién financia la retención. | Sin el par de líneas, el asiento real de una compra con tarjeta no es registrable: la CxP quedaría neta de retenciones que ya se pagaron completas al comercio, o se perdería la declaración/certificación de la retención. Asumir es la práctica contable estándar, validada con la consultoría fiscal (jul-2026). | Coordinación con el catálogo de plantillas de Contabilidad v1.8. El mismo patrón servirá para legalizaciones futuras (cajas menores) con otro medio de pago. |
| D38 | **Clasificación semántica, línea de contrapartida y espejo del hecho relacionado hacia Contabilidad** (issue #104, `[D15]` de Contabilidad). **(1)** Toda línea de traducción viaja con **`clasificacion` obligatoria**: texto semántico que OXP compone **mecánicamente** desde sus catálogos según el componente (tabla de recetas en la subsección "Campos que OXP puebla en el contrato"); Contabilidad lo empareja por similitud para resolver la cuenta (`[R52]` de Contabilidad) — no es un código ni lo digita un usuario. **(2)** OXP emite la **línea `contrapartida`** en todo hecho cuya plantilla tiene rol CONTRAPARTIDA: lleva el tercero del documento y su clasificación (medio de pago + observación general), sin valor ni unidad organizacional (los rinde el motor — `[R54]`/`[I33]` de Contabilidad). `terceroPrincipal` queda **informativo** (el caso del extracto que motivó el #28 lo resuelve ahora la línea). **(3)** Los componentes que saldan un hecho anterior (`cruce_obligacion`, `partida_aclarada`, `amortizacion_anticipo`, `reversa_anticipo` y toda `nota_credito_gasto`) viajan con **`referenciaHechoRelacionado`** para que Contabilidad copie la cuenta del rol espejado (`[R53]`) — el cruce contable salda en la **misma cuenta**, no en una parecida. Catálogo canónico: 17 → **18** componentes (+`contrapartida`). | Sin definición, el equipo de desarrollo no podía implementar las reglas de derivación ni las sugerencias por movimientos recientes (¿texto abierto o código?); un código sería inútil para el emparejamiento contra la descripción de la cuenta, y sin espejo los cruces dependerían de que la similitud "adivinara" la misma cuenta. | Coordinación cruzada: Contabilidad alcance v1.11 (`[R52]`-`[R54]`), modelo v1.12 (`[D15]`/`[I34]`/`[I35]`/`[SI8]`), catálogo de plantillas v1.9 (`resolucionPorEspejo`), anexo de ejemplos v1.5. |
| D39 | **Registro de productos financieros — el emisor y la configuración del extracto provienen de un registro definido por el usuario; la conciliación es por tarjeta** (issue #106; reemplaza el planteamiento de la decisión de inferencia del #57, retirada en v4.9). Casos de uso del equipo de desarrollo mostraron que casi ninguna compra permite deducir la forma de pago, que en algunos extractos el emisor no es determinable, y que existen **cuentas consolidadoras** que agrupan varias tarjetas con extracto consolidado (caso real Bancolombia). Nuevo agregado **`ProductoFinanciero`** — la tarjeta de la empresa: número (clave natural, I26), emisor, tipo, configuración `[R06]`/`[R07]`, `cuentaConsolidadora` opcional, estado: **el usuario define sus productos y el sistema verifica contra lo definido** — no adivina el comportamiento de cada entidad financiera. El emisor y el `tipoTarjeta` **se copian al radicar** el OxpExtracto (referencia `productoFinancieroId` — patrón `[D31]`); el extracto **no lleva medio de pago** — es la deuda de la tarjeta y se paga por tesorería: el campo que dice *cómo se paga* no puede cargar el *origen* de la deuda, que ya modela la referencia; la atribución tiene **bandeja de pendientes** (`[R41]`, canal usuario o proceso automático de correo — el extracto nunca se pierde ni bloquea); la unicidad por (producto, período) es **idempotente** (`[R42]`/I27/`[SI10]`). **La conciliación y la deuda se manejan por tarjeta:** el extracto consolidado se difiere como evolución — evidencia operativa: fechas de corte y pago distintas por tarjeta, contabilización y gestión independientes — con **salvaguarda de detección** (`[R43]`: archivo con varios números de tarjeta → bandeja + alerta, no radica automáticamente) y la `cuentaConsolidadora` como **dato agrupable**: el mismo valor agrupa las tarjetas de una misma cuenta — alimenta el diagnóstico de la alerta de `[R43]` y la advertencia al registrar de `[R39]`, y es semilla real de la migración futura. **Dueño provisional:** OXP, mientras existe el dueño definitivo (Tesorería o un contexto global) — entonces el registro se extrae y OXP lo consume por copia local (`[D34]`/`[SI8]`); el argumento anti-apropiación del planteamiento anterior se responde con extracción futura, no con inferencia. | La inferencia desde el histórico re-digitaba el error del primer período, no resolvía el primer extracto de una tarjeta y era incorrecta para el consolidado (no tiene *un* número de tarjeta); la configuración por tarjeta de `[R06]`/`[R07]` no tenía hogar en el modelo; y el componente "Medios de pago" del alcance (v1.3) nunca tuvo contraparte modelada. | DDD: registro con identidad y comportamiento propios, frontera mínima alineada con lo que la evidencia soporta. ES: la ampliación al consolidado es aditiva (eventos nuevos, proyecciones reconstruibles). Coordinación: alcance v1.18 (glosario Producto Financiero, R39 reescrita, R41-R43 nuevas). |
| D40 | **Medio de pago canónico con cadena de resolución, retención en la confirmación y coherencia con el extracto** (issue #96; cierra lo diferido por `[D37]`). **(1) Enum canónico** de 7 tipos (`tarjeta_credito \| tarjeta_debito \| tarjeta_prepago \| transferencia \| efectivo \| cheque \| otro`) — una compra por transferencia ya no llega con `medioPago.tipo = 0`. **(2) Cadena de resolución al radicar** (`[R44]`, molde de la cadena de unidades `[D35]` — señales fuertes deciden, débiles sugieren, la ausencia no inventa): capturado (incluye **selección de la tarjeta del registro** — la compra queda con la referencia `productoFinancieroId` + últimos 4 copiados, patrón `[D31]`) → deducido del soporte (últimos 4 con coincidencia única en el registro → referencia; sin coincidencia → **evidencia suelta**) → **`medioPagoHabitual` del Proveedor** (tipo + tarjeta habitual opcional; sin configuración el sistema no asume; pesa más que el XML del emisor) → sugerencia del XML FE (pre-llena, no decide sola) → conservador no-tarjeta **marcado**. Origen rastreado por resolución. **(3) Verificación en la confirmación (I28):** resoluciones débiles (`sugerido_xml`/`asumido_por_defecto`) exigen decisión humana según modo por empresa (bloqueo/decisión obligatoria, esquema `[R38]`/I23); en canal automático la OXP queda **retenida en Pendiente** (bandeja de pendientes de confirmación) — sin confirmación no hay causación y el pago directo exige Causada (`[R35]`): el doble pago "tarjeta asumida como crédito" queda estructuralmente cerrado. **(4) Conciliación:** la tarjeta referenciada/dígitos **priorizan** las sugerencias (nunca excluyen). **La coherencia del medio de pago no requiere evento ni regla propia** (se evaluó y retiró un evento de alerta): la garantizan **I28** (prevención — sin decisión no hay causación), el **control de saldos existente** (los pagos por extracto y por tesorería compiten por el mismo `saldoPorPagar()`, que solo se consume una vez — el doble pago no puede consumarse en ningún orden) y la **diferencia neto-vs-total** que el propio cruce hace visible cuando una compra causada como no-tarjeta aparece en el extracto (corrección operativa; antes de causar, el medio de pago se corrige por edición directa — nivel 1 de la cadena). La asociación compra↔tarjeta **en firme** la establece la conciliación (`Vinculacion` → partida → extracto → producto). | El XML FE declara lo del emisor, no cómo pagó la empresa; el dato decide el tratamiento de las retenciones asumidas (`[D37]`) y no tenía resolución definida; una compra pagada con datáfono y asumida como crédito quedaba expuesta a doble pago por pago directo. | DDD: el registro `ProductoFinanciero` como dato común referenciado por OxpComercio y OxpExtracto (referencia a la raíz + copia, `[D31]`); cadena de niveles con trazabilidad (patrón `[D35]`); detecta-no-exige en los hechos consumados y retención solo donde el dinero aún no ha salido. Coordinación: alcance v1.19 (glosario Medio de Pago, R44, R10 ampliada). |
| D41 | **Tributos de provisión en la causación — par de líneas explícito con componentes propios por tributo** (issue #128; espejo de la naturaleza `provision` de Impuestos v2.0.8). Un tributo autoliquidado (IVA_IMPORTACION_SERVICIOS: el adquiriente asume el IVA de servicios contratados con proveedores sin domicilio fiscal en el país — el proveedor recibe el total) se reconoce sin afectar el valor a pagar ni los saldos: un débito y un crédito del mismo valor. **Regla general por naturaleza:** todo `Tributo` de naturaleza `provision` del desglose fiscal emite en `causacion_gasto` un **par de líneas explícito** — **reconocimiento** (Db, `tipoComponente` = código del tributo; su destino —impuesto descontable o mayor valor del gasto— queda pendiente del refinamiento del IVA descontable de Contabilidad) y **provisión por pagar** (Cr, `tipoComponente` = código con sufijo `_por_pagar`) — ambas con tercero = proveedor y el valor de la línea del desglose. El `DesgloseFiscal` de OXP incorpora la lista de provisiones y `valorNeto()` las excluye (espejo de la aritmética de Impuestos). En el catálogo de plantillas de Contabilidad: roles genéricos **IMPUESTO_AUTOLIQUIDADO** (Db) y **AUTOLIQUIDADO_POR_PAGAR** (Cr) — un rol por lado; las cuentas se acotan **por componente**, no por rol (como RETENCION acota retefuente/reteiva/reteica): cada autoliquidado nuevo agrega su par de componentes con sus grupos, sin tocar la regla ni el motor. La **nota crédito reversa el par por espejo** del rol homólogo de la causación devuelta (patrón de toda `nota_credito_gasto`). | Sin definición, la línea de la provisión caía en rechazo `LINEA_SIN_ROL_EN_PLANTILLA` (I27 de Contabilidad): el catálogo canónico no declaraba componente alguno para provisiones y la plantilla no tenía roles que las recibieran. El motor no desdobla una línea en dos partidas (una línea = una partida) y las dos partidas aterrizan en cuentas distintas — se requieren dos componentes. | Mismo molde del par explícito de `[D37]` (retención asumida) y de las autorretenciones del sistema legado (par Db/Cr). Las autorretenciones (dirección ingreso) no tocan OXP — les llegará su turno con el sub-dominio de facturación. Coordinación cruzada: catálogo de plantillas de Contabilidad v1.10 (+2 roles en `causacion_gasto` y `nota_credito_gasto`; grupos del PUC `porValidar` con consultor contable). |
| D42 | **Los ajustes de conciliación viajan con el signo decidido por OXP — un componente por sentido — y con el tercero del proveedor de la compra cruzada.** La diferencia en cambio (`[R10b]`) y el ajuste por tolerancia (`[R10]`) nacen al cruzar la partida del extracto con la compra; OXP ya conoce el sentido (la clasificación de `AjustePorDiferenciaCambio`, la dirección de `AjustePorTolerancia`) y lo materializa **eligiendo el componente**: `diferencia_en_cambio_perdida` / `ajuste_tolerancia_perdida` (Db) o `diferencia_en_cambio_ganancia` / `ajuste_tolerancia_ganancia` (Cr) — nunca ambos para un mismo ajuste. La naturaleza contable vive en el rol de la plantilla, por eso cada sentido tiene rol y componente propios (molde del par de `[D41]`: dos partidas de naturaleza distinta exigen dos roles). La línea lleva el **tercero del proveedor de la compra cruzada** — el ajuste es la diferencia sobre esa obligación, como el `cruce_obligacion` que la salda; el banco/emisor solo va en el cargo financiero y en la contrapartida. El **extracto es el único emisor**: al radicar una compra en moneda extranjera no hay dos tasas que comparar. No hay compensación explícita: el cruce viaja a TRM de radicación y la contrapartida la balancea el motor por el total real del extracto. | Antes el sentido viajaba solo embebido en el texto de la clasificación, obligando a Contabilidad a inferir por similitud una decisión estructural; los roles duales (`debito_credito`) no existen en el agregado de plantillas de Contabilidad y el grupo mixto de gasto e ingreso dejaba la inferencia sin acotar. La naturaleza dual no era una regla de negocio sino una omisión del emisor: el dato existía en la entidad y no se comunicaba. Alternativa descartada: enviar el signo en el valor o en un campo nuevo del contrato — rompería la regla de que la dirección la fija la plantilla y no la línea. | DDD: el contexto que conoce el hecho decide; contrato explícito entre bounded contexts (`[D29]`, `[D37]`, `[D41]`). Coordinación cruzada: catálogo de plantillas de Contabilidad v1.11 (roles `*_PERDIDA`/`*_GANANCIA` con naturaleza fija y un solo grupo del PUC; grupos `porValidar`). |
...
| 4.5 | Julio 2026 | **Referencias a Estructura Organizacional actualizadas por la renumeración de EO (issues #85-#89, PR #91).** EO cerró los saltos de numeración de sus reglas, decisiones, invariantes y sugerencias de implementación; las referencias vivas de este modelo se ajustan: `[D15]` de EO → **`[D13]`** (dato gobernado / copia local / diferir) y `[SI12]` de EO → **`[SI09]`** (punto de resincronización). Sin cambios de contenido en OXP. |
| 4.6 | Julio 2026 | **Tratamiento contable de la partida en disputa — ciclo por cuenta transitoria de "partidas por aclarar" (issue #90).** Cierra el hueco de partida doble en la causación del extracto: la partida en disputa es parte de la deuda real que el banco cobra, pero no viajaba en ninguna línea → la CxP del banco/emisor quedaba subvalorada (mismo hueco que el #18 cerró para el cruce); y las resoluciones no cerraban contablemente. **Nueva decisión `[D37]`:** (1) al causar, cada partida en disputa emite la línea `partida_por_aclarar` (Db transitoria, tentativo 1360 Reclamaciones/1380 Deudores varios); (2) el descarte viaja como `partida_aclarada` dentro de la causación del extracto donde llega el reverso (`R10c`); (3) la reclasificación es `tipoTransaccion` propio **`reclasificacion_partida`** desde `PartidaEnDisputaReclasificada` (Db CxP del proveedor identificado · Cr transitoria, sin contrapartida del motor — criterio del Caso B de `[D26]`; se descartó reutilizar plantillas existentes: `causacion_gasto` dejaría la contrapartida del motor en cero y `amortizacion_anticipo` prestaría su semántica a otro hecho). **Terceros definidos por línea:** la transitoria siempre contra el banco/emisor; las CxP contra su proveedor; `terceroPrincipal` de la reclasificación = banco/emisor. Nombres **generales de contabilidad** (sin "disputa" — vocabulario interno de OXP), decididos con el usuario. **Cambios:** catálogo canónico de `tipoComponente` 14 → **16** (+`partida_por_aclarar`, +`partida_aclarada`; `cruce_obligacion` también en `reclasificacion_partida`); mapeo de eventos → `tipoTransaccion` +1 fila; `lineasParaTraduccion()` del OxpExtracto emite las dos líneas nuevas; efectos de `ExtractoCausado` y de los 3 eventos de disputa (la reclasificación pasa a integración saliente con `EntregaAceptada`/`referenciaDestino`); párrafo de `terceroPrincipal` ampliado. Decisiones 36 → **37**. **Coordinación cruzada:** catálogo de plantillas de Contabilidad v1.7 (+2 roles en `causacion_gasto`, +plantilla `reclasificacion_partida`; 5 → 6), anexo de ejemplos v1.4 (**Ejemplo 5** con los tres momentos), modelo de Contabilidad v1.10. Cuentas `porValidar` por consultor contable (ítems 11-12 del catálogo). Acompaña al alcance v1.17. |
| 4.7 | Julio 2026 | **Retenciones asumidas — nueva decisión `[D38]` (issue #94).** El issue partía de una premisa desactualizada ("con tarjeta el comprador no retiene"); la doctrina DIAN vigente (concurrencia, conceptos 2023-2025, validada con consultoría fiscal) establece que la retención se practica **independientemente del medio de pago** — el motor de Impuestos no cambia (liquida igual, sin conocer el medio de pago) y el 1,5% del emisor al comercio es asunto del proveedor. Lo que sí faltaba: cuando el pago ya salió completo (tarjeta), las retenciones **no se descuentan del valor a pagar** — la empresa las **asume**. **Cambios (alcance compacto acordado):** `[D38]` (responsabilidad de OXP, mecánica del par de líneas: la retención viaja idéntica —misma cuenta, tercero proveedor, certificable— y se agrega la línea Db espejo solo cuando es asumida), componente **`retencion_asumida`** en el catálogo canónico (16 → **17**), nota en `lineasParaTraduccion()` de OxpComercio. Rol espejo `IMPUESTO_ASUMIDO` en el catálogo de plantillas de Contabilidad v1.8. Decisiones 37 → **38**. **Diferido a issue propio:** canonización del enum `MedioDePago`, captura obligatoria al radicar (sugerencia del XML DIAN) y alerta de coherencia con el extracto. |
| 4.8 | Julio 2026 | **Clasificación semántica, línea de contrapartida y espejo del hecho relacionado — nueva decisión `[D39]` (issue #104).** Cierra la zona gris del campo `clasificacion` del contrato `LineaTraduccion` (los desarrolladores no sabían si era texto abierto o código): es **texto semántico obligatorio** que OXP compone mecánicamente por componente — **tabla de recetas** nueva en la subsección de integración (gasto: descripción del concepto + concepto de pago + clasificación tributaria; tributos: concepto de pago + nombre y % del tributo; etc.). **Línea `contrapartida` nueva** en todo hecho con rol CONTRAPARTIDA (tercero del documento + medio de pago/observación; sin valor ni unidad) — `terceroPrincipal` pasa a **informativo**. **`referenciaHechoRelacionado` por línea** en los componentes de espejo (`cruce_obligacion`, `partida_aclarada`, `amortizacion_anticipo`, `reversa_anticipo`, toda `nota_credito_gasto`): Contabilidad copia la cuenta del rol espejado del hecho original (`[R53]` de Contabilidad). **Cambios:** catálogo canónico de `tipoComponente` 17 → **18** (+`contrapartida`); subsección "Campos de narración" renombrada a "Campos que OXP puebla en el contrato" y ampliada; `lineasParaTraduccion()` de OxpComercio, OxpExtracto y Devolucion actualizados (retirada la frase "solo necesita mapear (tipoComponente + unidad organizacional) → cuenta", que omitía la clasificación); párrafo de `terceroPrincipal` ajustado. Decisiones 38 → **39**. **Coordinación cruzada:** Contabilidad alcance v1.11, modelo v1.12, catálogo de plantillas v1.9, anexo de ejemplos v1.5. Sin cambios en eventos, invariantes ni estados. |
| 4.9 | Julio 2026 | **Registro de productos financieros — nuevo agregado `ProductoFinanciero`; retiro de la inferencia del emisor con renumeración (issue #106; reemplaza el planteamiento del #57).** Casos de uso del equipo de desarrollo: casi ninguna compra permite deducir la forma de pago; en algunos extractos el emisor no es determinable; existen **cuentas consolidadoras** que agrupan varias tarjetas (caso Bancolombia). La evidencia operativa (fechas de corte y pago distintas por tarjeta, contabilización y gestión independientes) fija que **la conciliación es por tarjeta** — el extracto consolidado se difiere como evolución (#106), con salvaguarda de detección. **Cambios:** **(1)** Nuevo agregado **`ProductoFinanciero`** [F1] (octavo del BC): número de tarjeta (clave natural), emisor (`InformacionTercero` entidad financiera), tipo, configuración de cargos `[R06]` y plazo de alerta `[R07]`, marca informativa `asociadaA`, estados Activo/Inactivo; comandos Registrar/Actualizar/Inactivar/Reactivar; bandeja de pendientes de atribución (capa de aplicación). Omisiones deliberadas: cuentas de ajuste (las resuelve la traducción contable — frontera de `AjustePorTolerancia`), período/fecha de pago (datos de cada extracto), límites (dueño futuro). **(2)** OxpExtracto: referencia `productoFinancieroId`; emisor y `MedioDePago` **copiados del producto al radicar** (patrón `[D31]`); ficha `ExtractoRadicado` reescrita (atribución `[R41]`, unicidad idempotente `[R42]`, salvaguarda de consolidado `[R43]`, advertencia por producto Inactivo, **fecha de pago** capturada — hueco preexistente de la alerta `[R07]`); desaparece el `origen` del emisor (`del_archivo \| inferido_historico \| capturado_usuario`). **(3)** FSM 4.6, eventos 5.10 (`ProductoFinancieroRegistrado`/`Modificado`/`Inactivado`/`Reactivado` — 4 fichas nuevas), fila nueva en Clasificación de capacidades, relación OxpExtracto→ProductoFinanciero, nota en Value Objects compartidos (en OxpComercio se captura el **tipo** de medio de pago; canonización diferida al #96). **(4)** Invariantes **I26** (unicidad del producto) e **I27** (unicidad extracto por producto y período): 25 → **27**. **(5)** **Retiro con renumeración sin saltos** (precedente PR #91): decisión de inferencia del emisor (#57) **retirada** — mapeo **D36→D35, D37→D36, D38→D37, D39→D38**; nueva **D39** (registro de productos financieros) — total se mantiene en 39. Sugerencia de proyección "último emisor por número de tarjeta" **retirada** — mapeo **SI10→SI9**; nueva **[SI10]** (unicidad extracto por producto y período, análoga a `[SI4]`) — total se mantiene en 10. Los changelogs conservan la numeración histórica. **(6)** D2 actualizada (8 agregados). Acompaña al alcance v1.18 (glosario Producto Financiero, R39 reescrita, R41-R43 nuevas, R10 corregida). |
| 4.10 | Julio 2026 | **Medio de pago canónico — cadena de resolución, retención en la confirmación y coherencia con el extracto; nueva decisión `[D40]` (issue #96).** Análisis de canales con el equipo: las compras entran por carga manual de PDF o carga automática (XML FE, o PDF de proveedores del exterior sin FE); el XML FE **declara** el medio de pago del emisor pero no certifica cómo pagó la empresa; los PDF del exterior a veces traen los **últimos 4 dígitos** de la tarjeta. **Cambios:** **(1)** VO `MedioDePago` canonizado en los 3 agregados (7 tipos; en OxpComercio con **origen de la resolución** y tarjeta opcional { `productoFinancieroId`, últimos 4 } cuando se resolvió contra el registro — referencia validada vs **evidencia suelta**; en OxpExtracto copiado del producto; en Anticipo el enum). **(2)** `Proveedor` + **`medioPagoHabitual`** opcional (tipo + `tarjetaHabitual` referencia al registro; no viaja a la bodega) — nivel 3 de la cadena; `ActualizarProveedor`/`ProveedorActualizado` ampliados. **(3)** Ficha `OxpComercioRadicada` (medio de pago resuelto por la cadena `[R44]`; la confirmación automática `[R02]` solo procede con resolución fuerte) y `OxpComercioConfirmada` (verificación del medio de pago). **(4)** Nueva invariante **I28** (ninguna confirmación con resolución débil — retenida en Pendiente en canal automático; condicionada por modos bloqueo/decisión obligatoria como I23; cierra estructuralmente el doble pago vía `[R35]`): 27 → **28**. **(5)** `ServicioDeConciliacion` con **priorización por tarjeta** (referencia primero, dígitos después — nunca excluyen). **La coherencia con el extracto se resolvió sin evento ni regla propia** — se evaluó y retiró un evento de alerta (sin efectos que gobernar, señal redundante con la diferencia neto-vs-total del propio cruce): la garantizan I28, el control de saldos existente y la corrección operativa (ver `[D40]`); sin cambios en el conteo de eventos (57). **(6)** Nueva decisión **`[D40]`**; el "diferido a issue propio" de `[D37]` queda cerrado. Decisiones 39 → **40**. Acompaña al alcance v1.19 (glosario Medio de Pago, R44 nueva, R10 ampliada). |
| 4.11 | Julio 2026 | **Renombre semántico de la familia de eventos de alerta (issue #113).** Al revisar la semántica del evento de coherencia del #96 se detectó que los eventos de alerta nombraban la **notificación** (el sustantivo "Alerta") en vez del **hecho ocurrido en pasado**, violando la convención de nomenclatura de la Sección 2 (`OxpComercioRadicada`, `VinculacionRealizada`) — el defecto nació en el #30 (v3.9), cuando la prosa de negocio de `[R38]` ("emite una alerta de doble pago potencial") se coló directo al nombre del evento. La alerta es el **efecto** (read model de alertas / panel de trabajo) y vive en la ficha de cada evento, no en su nombre. **Tabla de mapeo:** `AlertaDoblePagoPotencial` → **`DoblePagoPotencialDetectado`** · `AlertaPlazoAnticipoVencido` → **`PlazoAnticipoVencido`** · `AlertaConciliacionPlazoVencido` → **`PlazoConciliacionVencido`**. Referencias vivas ajustadas (notas de FSM, efectos de `PagoOxpComercioDirectoAplicado` y `ConstanciaAnticipoNoAplicableRegistrada`, I23, D33); los changelogs conservan los nombres históricos. Sin cambios de conteos (57 eventos — mismos hechos, nombres correctos) ni de artefactos externos (el alcance habla de las alertas en prosa de negocio, sin nombrar eventos). |
| 4.12 | Agosto 2026 | **Lote del equipo de desarrollo — el extracto no lleva medio de pago; la cuenta consolidadora como dato agrupable (issues #126 y #127; residuos del #106/#96).** **(1) OxpExtracto sin `MedioDePago` (#126):** el VO estaba declarado en la composición pero el payload de `ExtractoRadicado` nunca lo llevó — y el dato no significaba lo que el nombre dice: el extracto **es** la deuda de la tarjeta y se paga por tesorería (su único cruce de pago externo es `pago_sincoa`); el **origen** de la deuda ya lo modela la referencia `productoFinancieroId`, y guardarlo en el campo que dice *cómo se paga* mezclaba las dos cosas. El extracto lleva copiados al radicar el emisor, el número de tarjeta y el **`tipoTarjeta`** (dato simple, no VO) — el único ingrediente del VO con consumidor real: alimenta la clasificación de la línea `contrapartida` (`[D38]` — permite que crédito y débito prepago aterricen en cuentas distintas del grupo CxP del banco) y queda inmutable en el hecho (con solo la referencia, la causación leería el estado actual del producto, contra la promesa de `ActualizarProductoFinanciero`). Cambios: encabezado, composición y diagrama de OxpExtracto; ficha `ExtractoRadicado`; Value Objects compartidos (`MedioDePago` pasa a **2 agregados**: OxpComercio y Anticipo); relación ProductoFinanciero → OxpExtracto; **receta de `contrapartida` corregida** — nombra el enum real (`credito \| debito_prepago`); la mención "cuenta" era del consolidado, evolución anotada en el #106; `[D39]` ampliada; guía de modelado de agregados alineada. **(2) `asociadaA` → `cuentaConsolidadora` (#127):** la marca `{ esAsociada, datosCuentaConsolidadora (texto libre) }` admitía estados contradictorios, ningún control la leía y el texto libre la inutilizaba como semilla (cinco tarjetas de la misma cuenta = cinco textos que el sistema no puede agrupar). Queda **un solo campo opcional** — si está, la tarjeta es asociada — cuyo mismo valor **agrupa** las tarjetas de una cuenta, con **dos lectores**: el **diagnóstico** de la alerta de la salvaguarda de consolidado (`[R43]`: la alerta dice por qué sospecha — las tarjetas del archivo están anotadas a la misma cuenta) y la **advertencia al registrar** un producto cuyo número coincida con una cuenta anotada (`[R39]` — advertencia, no bloqueo, criterio detecta-no-exige: cierra el camino a la doble deuda por registro de la cuenta consolidadora como si fuera una tarjeta, que `[R43]` no cubría). Cambios: estructura del `ProductoFinanciero`, comandos `Registrar`/`Actualizar`, nota de la bandeja de pendientes, eventos `ProductoFinancieroRegistrado`/`ProductoFinancieroModificado`, `[D39]` (la semilla pasa de informativa a dato agrupable real). **(3) Alineaciones por validación contra la implementación (ago-12):** el **número de tarjeta es inmutable** — clave natural (I26) e identidad del registro; la corrección de un número mal digitado es inactivar el producto y registrar uno nuevo (la precondición de `ProductoFinancieroModificado` prometía una "corrección excepcional" del número que el sistema no permite — se adopta la forma de la implementación por correcta: mutar la clave natural rompería la atribución histórica). Y la **entidad bancaria sale del `MedioDePago`**: no hace parte del medio de pago — es el emisor del producto financiero registrado; el número identifica el instrumento (identidad en tarjetas; opcional en los demás tipos: número del cheque, referencia de la transferencia) — composición de Anticipo y glosario ajustados. **Sin cambios de conteos** (57 eventos, 28 invariantes, 40 decisiones). Acompaña al alcance v1.20. |
| 4.13 | Agosto 2026 | **Tributos de provisión en la causación de gasto — nueva decisión `[D41]` (issue #128, planteado por el equipo de desarrollo; espejo de Impuestos v2.0.8).** El rediseño del autoliquidado dejó a `IVA_IMPORTACION_SERVICIOS` (naturaleza `provision`, dirección gasto) entrando al desglose fiscal de la compra, pero ni el catálogo canónico de OXP ni la plantilla `causacion_gasto` sabían recibirlo — la línea caía en rechazo `LINEA_SIN_ROL_EN_PLANTILLA` (I27 de Contabilidad). **Cambios:** **(1)** `DesgloseFiscal` (OxpComercio y Devolucion) incorpora la **lista de provisiones**; `Tributo` admite la tercera naturaleza; `valorNeto()` documenta que las provisiones no suman ni restan. **(2)** Regla general por naturaleza (`[D41]`): cada tributo `provision` emite un **par de líneas explícito** — reconocimiento (Db, código del tributo) y provisión por pagar (Cr, código + sufijo `_por_pagar`), mismo valor, tercero = proveedor — molde del par de `[D37]`; bullet nuevo en la integración con Contabilidad y nota en `lineasParaTraduccion()` de OxpComercio y Devolucion. **(3)** Catálogo canónico de `tipoComponente` 18 → **20** (+`iva_importacion_servicios`, +`iva_importacion_servicios_por_pagar`) con la excepción de nombre documentada (dos componentes por tributo de provisión); recetas de clasificación del par en la tabla de `[D38]`. **(4)** La **nota crédito reversa el par por espejo** del rol homólogo (patrón vigente de toda `nota_credito_gasto`). El destino del reconocimiento (descontable vs mayor valor del gasto) queda pendiente del refinamiento del IVA descontable de Contabilidad (issue #98). Decisiones 40 → **41**. **Coordinación cruzada:** catálogo de plantillas de Contabilidad v1.10 (roles genéricos `IMPUESTO_AUTOLIQUIDADO`/`AUTOLIQUIDADO_POR_PAGAR`, cuentas por componente `porValidar` — ítems 14-15). Sin cambios en eventos, invariantes ni estados. |
| 4.14 | Agosto 2026 | **Dos pendientes de distribución distintos — la asignación se exige en la confirmación, la propagación difiere la causación; nueva invariante I29 (issue #123).** Las pruebas de integración contra la implementación mostraron que la confirmación del anticipo se rechaza cuando la distribución no está resuelta, mientras I10 mandaba diferir la causación sin bloquear etapas previas. El análisis (plan del equipo de desarrollo del 30-jun-2026, "eliminar centinela PENDIENTE") reveló que I10 mezclaba **dos pendientes de naturaleza distinta**: **(a) pendiente de asignación** — ninguna regla de la cadena `[D35]` resolvió y el usuario no ha asignado: requiere decisión humana, y la **confirmación es la última puerta humana** antes de la causación (confirmar algo incausable — "validado y aprobado para causación", `[R11]` — sería semánticamente incoherente); **(b) pendiente de propagación** — la unidad asignada existe en la fuente de verdad pero su evento aún no llega a la copia local (`[D34]`): no hay nada que un humano corrija, bloquear sería un error. **Cambios:** nueva **I29** (ninguna confirmación con distribución sin asignar — bloqueo en canal manual, retención con re-disparo automático al resolverse en canal automático; patrón de I28): invariantes 28 → **29**. I10, `[D34]` y el nivel 5 de la cadena de resolución precisados con la distinción; precondiciones de las 4 fichas de confirmación (`AnticipoConfirmado`, `OxpComercioConfirmada`, `ExtractoConfirmado`, `DevolucionConfirmada`). **Para la implementación queda canonizado su guard de asignación** (confirmar exige distribución + re-disparo del consumer) **y pendiente el ajuste del caso de propagación:** hoy los handlers de confirmar y causar **lanzan error** por unidad inexistente/inactiva en la copia local — deben **diferir** (el mecanismo de destino pendiente con liberación por `UnidadCreada`/`UnidadActivada` de `[D34]` no existe en código). Acompaña al alcance v1.21 (R11 precisada). |
| 4.15 | Septiembre 2026 | **Ajustes de conciliación con signo decidido por OXP y tercero del proveedor cruzado — nueva decisión `[D42]` (issue #132, planteado por el equipo de desarrollo; coordinación cruzada con el catálogo de plantillas de Contabilidad v1.11).** Los roles `DIFERENCIA_EN_CAMBIO` y `AJUSTE_TOLERANCIA` del catálogo declaraban una naturaleza dual (`debito_credito`) inexistente en el agregado de plantillas y un grupo del PUC mixto; el sentido del ajuste solo viajaba embebido en el texto de la clasificación. **Cambios:** **(1)** Cada ajuste se emite en **uno de dos componentes según su signo**, decidido por OXP con el dato que la entidad ya tiene: `diferencia_en_cambio` → `diferencia_en_cambio_perdida` (Db, clasificación gasto financiero) / `diferencia_en_cambio_ganancia` (Cr, ingreso financiero); `ajuste_tolerancia` → `ajuste_tolerancia_perdida` (Db, extracto mayor) / `ajuste_tolerancia_ganancia` (Cr, extracto menor). Catálogo canónico de `tipoComponente` 20 → **22**; segunda excepción de nombre documentada (el signo va en el nombre porque la plantilla no puede saberlo). **(2)** Las líneas de ambos ajustes llevan el **tercero del proveedor de la `OxpComercio` de origen** (antes no se decía y se entendía el banco/emisor) — el ajuste es la diferencia sobre esa obligación, como el cruce. **(3)** El **extracto queda como único emisor** de la diferencia en cambio: se retira la mención de `OxpComercioCausada` "si se generó al radicar en moneda extranjera" (la entidad, `[R10b]` y el evento solo existen en la conciliación). **(4)** Fichas de `AjustePorDiferenciaCambio` y `AjustePorTolerancia` (Sección 6): fila nueva "Tercero (traducción)" y frontera reescrita — OXP decide el sentido, Contabilidad resuelve la cuenta dentro del grupo del rol sin inferirlo; tabla de entidades de OxpExtracto, `lineasParaTraduccion()` de OxpExtracto y OxpComercio, recetas de clasificación de `[D38]` (el sentido en la clasificación pasa a texto de apoyo), lista de componentes sin `descripcionConcepto`, nota de coordinación cruzada, omisiones deliberadas del `ProductoFinanciero` y `[D29]` actualizados. Decisiones 41 → **42**. Sin cambios en eventos, invariantes ni estados. Acompaña al alcance v1.22 (R10 y R10b precisadas). |
| 4.16 | Septiembre 2026 | **Retención a beneficiarios del exterior: segunda causa de asunción en `[D37]` (issue #143; espejo del `[D17]` de Impuestos v2.1.0).** La revisión con el contador y las consultoras fiscales definió la retención a proveedores sin domicilio fiscal en el país como tributo propio de Impuestos (`RETEFUENTE_EXTERIOR`, sustractivo, 20 % o tarifa de convenio); en la práctica el proveedor recibe el total, así que la retención **nace asumida** sin depender del medio de pago. **Cambios:** **(1)** `[D37]` ampliada — dos causas de asunción (medio de pago tarjeta; beneficiario del exterior por defecto, desmarcable por documento); **(2)** `Tributo` del `DesgloseFiscal` lleva la marca **asumida** con su origen (`medio_de_pago` / `beneficiario_exterior` / `manual`), editable antes de causar sin recalcular; **(3)** catálogo canónico de `tipoComponente` 22 → **23** (+`retefuente_exterior`); `retencion_asumida` se emite también en `nota_credito_gasto` (espejo — hueco previo: la devolución de una compra con retención asumida no reversaba el Db); receta de clasificación con la causa de la asunción; **(4)** `lineasParaTraduccion()` generaliza la condición "medio de pago tarjeta" a "retención marcada como asumida". Se descartó recibir la retención del exterior como tributo de provisión (`[D41]`): la asunción es decisión del consumidor, no del catálogo fiscal. Sin cambios en eventos, invariantes ni estados. **Coordinación cruzada:** plantillas de Contabilidad v1.12 (componente `retefuente_exterior` en el rol RETENCION; rol `IMPUESTO_ASUMIDO` en la nota crédito). Acompaña al alcance v1.23 (R45). |
