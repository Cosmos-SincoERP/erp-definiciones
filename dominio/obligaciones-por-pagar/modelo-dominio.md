...
- **Retenciones asumidas (`[D37]`):** cuando el pago del documento ya se realizó completo (medio de pago tarjeta), las retenciones del desglose viajan **idénticas** al caso normal (misma línea Cr, misma cuenta, tercero = proveedor — se declaran y certifican igual) y cada una emite **además** una línea `retencion_asumida` (Db espejo, gasto propio de la empresa); el valor a pagar no las descuenta. La decisión es de OXP — el único que conoce el medio de pago.
...
| `retencion_asumida` | retención del desglose marcada como **asumida** (medio de pago tarjeta, `[D37]`) — una línea Db espejo por cada retención asumida | `causacion_gasto` | OxpComercio |
...
| `iva`, `inc`, `retefuente`, `reteiva`, `reteica` | concepto de pago del concepto al que pertenece + nombre y % del tributo (desglose fiscal) |
| `retencion_asumida` | nombre y % del tributo asumido + medio de pago que origina la asunción (`[D37]`) |
...
| `Tributo` | Cálculo fiscal individual (impuesto, retención o provisión). Tipo, base, tarifa, valor. Inmutable — es el resultado de aplicar reglas fiscales al gasto. |
...
| **Información capturada** | `proveedorId` (referencia al Proveedor `[D31]`); tercero (NIT, razón social — copiado del Proveedor), fecha de transacción, valor en moneda original, moneda, TRM del día si aplica `[R05b]`, valor en moneda funcional, número de soporte/factura, medio de pago resuelto por la cadena (`[R44]`/`[D40]`: tipo canónico + origen de la resolución + tarjeta — { `productoFinancieroId`, últimos 4 } si se resolvió contra el registro, últimos 4 sueltos como evidencia si no), conceptos (gasto/costo + impuestos + retenciones + provisiones `[D41]`) con clasificacionTributaria y conceptoPago por cada concepto (resueltos desde el catálogo del sub-dominio origen o del catálogo de gasto directo de OXP), subDominioOrigen `[SI5]`, distribución de costos si aplica `[R05c]`, soportes documentales adjuntos. |
