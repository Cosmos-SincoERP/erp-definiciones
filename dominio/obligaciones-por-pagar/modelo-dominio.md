...
**Diferir cuando la unidad no existe aún (alineado con `[D15]` de EO):** si la cadena de resolución (`I10`/`D6`/`D7`) resuelve una unidad que **todavía no existe o no está activa** en la copia local, ese componente cae en **destino único pendiente** — OXP **registra la obligación pero no causa esa parte** hasta que la unidad exista. La resolución es automática: cuando llega `UnidadActivada` a la copia local, el pendiente se libera y la causación procede (consistencia eventual, sin espera humana ni consulta en caliente). OXP **nunca aproxima** con una unidad provisional o de tránsito: la unidad debe coincidir exacto con la contabilidad, y aproximarla desconciliaría operación y contabilidad (anti-patrón de la guía). Esto reusa el mecanismo de *destino pendiente* que OXP ya tiene — es la misma "incompletitud que bloquea la causación", ahora también disparada por una unidad que aún no llega.

**Evento saliente (OXP → EO):**

| Evento de OXP | Propósito en EO |
|---|---|
| `DemandaDeUnidadSenalada` (señal de demanda de unidad inexistente, no bloqueante) | Cuando OXP difiere por una unidad que no existe, hace **visible** la necesidad en la bandeja de sugerencias de EO (`[SI11]` de EO) para que el administrador la cree si corresponde. Es informativa: no crea unidades, no bloquea a OXP, y la corrección de OXP no depende de ella (la sostiene el diferir). |

> El diagrama del flujo completo (copia local + diferir + señal) vive en el modelo de EO, sub-sección 3.8 — no se duplica aquí.
...
Al consultar la distribución efectiva de cualquier componente, el agregado aplica la siguiente cadena (en orden de prioridad). Está inspirada en la **cadena de resolución de cuentas del sub-dominio Contabilidad** (su término 23 + `[R12]`/`[R35]`/`[R36]`): mismos principios de niveles determinísticos vs. aprendizaje, aprendizaje promovible a regla y trazabilidad del nivel — ver `[D36]`.
...
4. **Sugerencia por aprendizaje (Nivel C — no vinculante, `[D36]`/`[SI10]`)** → Si ninguna regla aplica, el sistema **pre-llena** con la unidad más frecuente del patrón histórico para esa combinación (`proveedor + tipo + lugarEjecucion`); el usuario **confirma o corrige** → pasa a instrucción explícita (nivel 1). No resuelve sola. (No hay un "Nivel B" de inferencia análogo al de Contabilidad: aquí no existe una estructura tipo PUC que inferir; el aprendizaje cubre ese rol.)
...
Los niveles 1–3 y 5 son **determinísticos y propios del agregado**; el nivel 4 es **asistencia de la capa de aplicación** (no vinculante, `[D23]`/`[D36]`). Cada distribución resuelta registra **con qué nivel** se asignó (trazabilidad, espejo de `[R36]` de Contabilidad). La sugerencia (nivel 4) y las reglas (nivel 3) operan siempre sobre la **copia local de unidades activas** (`[SI8]`); nunca proponen una unidad inexistente o inactiva.
...
Materializa el **Nivel C** de la cadena de resolución de la unidad (`[D36]`) — la sugerencia por aprendizaje. Espeja el aprendizaje de cuentas del sub-dominio Contabilidad (su Nivel C, `[R12]`/`[R35]`).
...
- **Promovible a regla:** el usuario puede convertir una sugerencia recurrente en una `ReglaDeDistribucion` formal del Nivel A (`CatalogoReglasDistribucion`) — espejo de la promoción aprendizaje→regla de Contabilidad.
- **Invalidable:** una sugerencia aprendida errónea puede invalidarse para que no se vuelva a proponer (espejo de `[R35]` de Contabilidad). No afecta distribuciones ya resueltas.
...
> Una regla **promovida desde el aprendizaje** (Nivel C → Nivel A, `[D36]`/`[SI10]`, espejo de `[R12]` de Contabilidad) entra por `ReglaDeDistribucionAgregada` como cualquier otra regla. La invalidación de un aprendizaje vive en la proyección de aprendizaje (`[SI10]`), no aquí.
...
| D34 | **La unidad organizacional es un dato gobernado por Estructura Organizacional (dueño único); OXP la consume como copia local, no como agregado** (replanteamiento #45, issue #48). OXP no crea, modifica ni gobierna unidades — solo las usa para distribuir e imputar. Mantiene una **copia local** (read model) por suscripción a los eventos de ciclo de vida de EO (`[SI8]`), opera y valida siempre contra ella (`I24`) y **nunca consulta a EO en caliente**. Cuando la cadena de resolución (`I10`) resuelve una unidad que aún no existe o no está activa en la copia local, **difiere** la causación de esa parte vía *destino pendiente* hasta que llegue `UnidadActivada` — sin aproximar con unidad provisional ni de tránsito (desconciliaría con contabilidad). Reacciona a las reestructuraciones de EO (fusión/división/traslado) reasignando referencias futuras y conservando el histórico. Publica hacia EO un aviso no bloqueante: la señal de demanda de unidad inexistente (`DemandaDeUnidadSenalada` → bandeja de sugerencias `[SI11]` de EO). **Contraste con el `Proveedor` (`[D30]`):** el Proveedor es un rol que OXP co-gobierna (agregado, `AsegurarProveedor`); la unidad tiene dueño único externo → solo copia local. **Nota:** la *asignación automática* de la unidad (qué unidad le corresponde a cada gasto) hoy se resuelve con la cadena `I10`/`D6`/`D7` hasta la preferencia de empresa; un nivel de sugerencia inteligente (reglas finas / aprendizaje, extensión de `[D23]`) se evalúa en issue aparte. | Sin copia local, OXP dependería de la disponibilidad de EO en cada distribución (acoplamiento de ejecución que el replanteamiento #45 elimina) o re-gobernaría un dato ajeno (el consumidor volviéndose dueño). El diferir preserva la consistencia exacta operación↔contabilidad sin bloquear la radicación. | DDD: un dato con dueño único externo se consume por réplica local (read model), no se modela como agregado. EDA: copia local por eventos + injerencia por avisos, nunca consulta ni escritura remota. Fundamento en `guias-de-modelado/datos-entre-dominios.md` y `[D15]` de Estructura Organizacional. |
...
| D36 | **Asignación de la unidad organizacional por una cadena de resolución de niveles, inspirada en la cadena de cuentas del sub-dominio Contabilidad** (issue #51). La distribución por unidad deja de resolverse solo con una preferencia global de empresa (gruesa) y pasa a una cadena con dos niveles nuevos, replicando el patrón ya probado en Contabilidad (su término 23 + `[R12]`/`[R35]`/`[R36]`): **Nivel A — reglas de preferencia de distribución** (configurables en `CatalogoReglasDistribucion`, **determinísticas**): criterios combinables `proveedor`/`tipo de gasto`/`lugarEjecucion`, resueltas por **especificidad** (más criterios gana) con desempate por prioridad (proveedor > tipo > lugarEjecucion); la regla sin criterios es la preferencia general (preserva el comportamiento anterior). Resuelven **automáticamente** sin confirmación — el usuario las configuró — y **cubren el arranque en frío** (un tenant nuevo configura reglas desde el día 1, sin esperar histórico). **Nivel C — sugerencia por aprendizaje** (`[SI10]`, **no vinculante**): cuando ninguna regla casa, se pre-llena con la unidad más frecuente del histórico para esa combinación; el usuario confirma → pasa a instrucción explícita; promovible a regla e invalidable (espejo del aprendizaje de Contabilidad). **Sin Nivel B**: la inferencia de Contabilidad se apoya en la estructura del PUC; para la unidad no hay estructura análoga que inferir, así que el aprendizaje cubre ese rol. La cadena completa: instrucción explícita → herencia del gasto padre → Nivel A → Nivel C (sugerencia) → destino pendiente. Cada distribución registra **con qué nivel** se resolvió (trazabilidad, espejo de `[R36]`). Todo opera sobre la **copia local de unidades activas** (`[SI8]`, `[D34]`): nunca se propone una unidad inexistente/inactiva; si la resuelta no existe aún, se difiere. **Límite (lo determinístico vs lo inferido):** lo vinculante/automático es solo lo determinístico (explícita, herencia, reglas configuradas); lo inferido por aprendizaje **siempre** pasa por confirmación humana — una inferencia no pisa una regla que el negocio configuró. | El aprendizaje por sí solo no resuelve el arranque en frío (sin histórico no sugiere) y es inferencia (riesgoso para un dato que va a contabilidad y debe ser exacto); las reglas determinísticas dan certeza y resolución desde el día 1, y capturan el conocimiento que el negocio ya tiene. Ambos son complementarios — el mismo balance que Contabilidad resolvió con su cadena de niveles. | DDD: la asignación es lógica de OXP (capa de aplicación para la sugerencia, agregado de configuración para las reglas). Consistencia entre modelos: replica el patrón de resolución de Contabilidad en lugar de inventar uno nuevo. Extiende `[D23]` (clasificación inteligente) a la dimensión unidad. |
...
| 4.3 | Junio 2026 | **Asignación de la unidad organizacional por cadena de niveles, inspirada en Contabilidad (issue #51).** **D36 nueva**: la distribución por unidad se resuelve con Nivel A (reglas configurables, determinísticas, por especificidad sobre proveedor/tipo/lugarEjecucion) + Nivel C (sugerencia por aprendizaje, no vinculante, promovible a regla e invalidable); sin Nivel B; replica el patrón de la cadena de resolución de cuentas de Contabilidad (término 23/R12/R35/R36) en vez de inventar uno. **Nuevo agregado de configuración `CatalogoReglasDistribucion`** (entidad `ReglaDeDistribucion`, 4 eventos en §5.9) — D2 actualizada (incluye además el `Proveedor` que faltaba listar). **I25 nueva** (distribución de cada regla suma 100%; sin dos reglas activas con criterios idénticos). **`[SI10]` nueva** (proyección de aprendizaje: unidad frecuente por combinación). Cadena de resolución (§3) e `I10` actualizadas: el nivel "preferencia de empresa" se reemplaza por Nivel A + Nivel C; trazabilidad del nivel (espejo de R36). Todo sobre la copia local de unidades activas (`[SI8]`/`[D34]`). Conteos: invariantes 24 → **25**; decisiones 35 → **36**; SIs 9 → **10**; +1 agregado de configuración. Acompaña al alcance v1.15 (R40). |
