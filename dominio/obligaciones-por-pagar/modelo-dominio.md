...
| `diferencia_en_cambio` | clasificación del ajuste (gasto/ingreso financiero) + moneda original (`AjustePorDiferenciaCambio`) |
| `ajuste_tolerancia` | dirección del ajuste (extracto mayor/menor) (`AjustePorTolerancia`) |
...
> **Omisiones deliberadas (F1):** las **cuentas del ajuste por tolerancia** las resuelve la traducción contable según el sentido de la diferencia — frontera ya definida en `AjustePorTolerancia` (Sección 6) y en el rol `AJUSTE_TOLERANCIA` del catálogo de plantillas de Contabilidad, no una configuración por tarjeta. El **período de facturación y la fecha de pago** son datos de cada extracto (viajan en `ExtractoRadicado`), no del producto. Los **límites y fechas de corte/pago del plástico** son datos maestros del dueño futuro (Tesorería) — ninguna regla de F1 los usa.
