...
| 11 | **Libro contable** | N2 | Configuración que define un conjunto de registros contables bajo una normativa específica (Principal, Local, NIIF, Gerencial). Cada libro tiene un PUC asociado. Un mismo PUC puede ser compartido por varios libros. La equivalencia entre libros se resuelve a nivel de PUC. |
...
2. Para cada empresa, el analista contable vincula un **plan de cuentas (PUC)**: la jerarquía de cuentas maestras y auxiliares. N1 necesita el PUC para resolver cuentas durante la traducción.
...
5. El sistema provee los **libros contables** como contenido incluido en el producto: un libro Principal y los libros alternos (Local, NIIF). El analista contable puede configurar libros adicionales. Para cada libro se vincula un PUC. La **equivalencia entre libros** se configura a nivel de PUC: cuando dos libros usan PUCs diferentes, se establece el mapeo cuenta a cuenta.
...
|---------|------|-----------|
| **Servicio de Entrega** (componente de N1) | Borrador resuelto: partidas con cuenta, tercero, unidad organizacional, débito/crédito, documento fuente y referencia de origen | Entregar al sistema contable de destino configurado |
...
- **Libros contables** — configuración de libros (Principal, Local, NIIF) con su PUC asociado y equivalencia entre PUCs.
...
| Plan de cuentas (PUC) | Gestión de la jerarquía de cuentas maestras y auxiliares. Necesario para que N1 resuelva cuentas durante la traducción. | F1 |
...
| Libros contables | Configuración de libros (Principal, Local, NIIF) con PUC asociado y equivalencia entre PUCs. Incluidos en el producto. | F2 |
...
| Libros contables | Configuración de libros (Principal, Local, NIIF) con PUC asociado y equivalencia entre PUCs. Incluidos en el producto. |
...
| 9 | **Multi-libro sin duplicación** *(N2)* | Un solo asiento contable se refleja en múltiples libros (Principal, Local, NIIF) mediante equivalencia de PUC, sin generar asientos duplicados ni controles de cruces entre libros. |
